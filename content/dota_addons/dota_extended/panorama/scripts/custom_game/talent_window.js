...
var TALENT_TABLE_NAME = "extended_talent_manager";
var OPEN_TALENT_WINDOW_CLASS = "show_talent_window";
var EXTENDED_TALENT_BTN_ID = "ExtendedTalentBtn";
...
function InitializeEXTENDEDTalentWindow(){
    ...
    for(var i=0; i<MAX_TALENT_ROWS; i++){
        ...
        var labelLevel = newTalentRow.FindChildTraverse("EXTENDED_Talent_Level");
        if(labelLevel){
            labelLevel.text = ""+currentRowLevel;
        }

        var panelSpecials = newTalentRow.FindChildTraverse("EXTENDED_Talent_Specials_Row");

        if(panelSpecials){
            var numOfChoices;
            if((i%2)==0){
                //Extended Special talents = 2, non-extended Special talents = 8
        ...
        if(talentWindow.BHasClass("show_talent_window") && GameUI.GetClickBehaviors() == CLICK_BEHAVIORS.DOTA_CLICK_BEHAVIOR_NONE){
            ...
            if ( eventName == "pressed" ){
                ...
                if(cursorPos[0] < talentWindow.actualxoffset ||
                    ...
                    OpenExtendedTalentWindow(false);
                }
            }
        }
...
function GetGenericTalentInfoTable(){
    return CustomNetTables.GetTableValue(TALENT_TABLE_NAME, "extended_generic_talent_info");
}
...
function GetExtendedTalentButtonPanel(){
    var baseUI = GetHUDRootUI();
    baseUI = baseUI.FindChildTraverse("AbilitiesAndStatBranch");
    return baseUI.FindChildTraverse(EXTENDED_TALENT_BTN_ID);
}

function IsExtendedTalentWindowVisible(){
    var talentWindow = $.GetContextPanel();
...
function OnTalentChoiceUpdated(table_name, key, value){
    if(key.indexOf("hero_talent_choice_") == 0){
        ...
        if(IsExtendedTalentWindowVisible()){
            //Update UI
            PopulateEXTENDEDTalentWindow();
            ...
            if(currentShownUnitID == talentWindow.GetAttributeInt(ATTRIBUTE_UNIT_ID, -1)){
                if(Entities.GetAbilityPoints(currentShownUnitID) <= 1 || //Note that this is <= 1 because it takes time for server to update the abilityPoints
                    ...
                    var extendedBtnPanel = GetExtendedTalentButtonPanel();
                    //Should not be null as you need it to open talent window
                    extendedBtnPanel.RemoveClass("upgrade");
        ...
        if(bol_to_animate_btn){
            AnimateExtendedTalentButton();
        }
    }
}

function CloseEXTENDEDTalentWindowWhenDeselectUnit(){
    var talentWindow = $.GetContextPanel();
...
function RepopulateExtendedTalentWindowOnAbilityPointsChanged(){
    var talentWindow = $.GetContextPanel();
    var currentShownUnitID = Players.GetLocalPlayerPortraitUnit();
    if(currentShownUnitID == talentWindow.GetAttributeInt(ATTRIBUTE_UNIT_ID, -1)){
...
function AnimateExtendedTalentButton(){
    var extendedBtnPanel = GetExtendedTalentButtonPanel();
    ...
    if(extendedBtnPanel){
        ...
        if(Entities.IsValidEntity(currentShownUnitID) &&
            ...
            extendedBtnPanel.SetHasClass("upgrade", CanHeroUpgradeAnyTalents(currentShownUnitID));
        }else{
            extendedBtnPanel.RemoveClass("upgrade");
        }
    }
...
function FormatGenericTalentValue(talent_name, talent_value){
    switch(talent_name){
        case "extended_generic_talent_magic_resistance":
        case "extended_generic_talent_evasion":
        case "extended_generic_talent_attack_life_steal":
        case "extended_generic_talent_spell_life_steal":
        case "extended_generic_talent_spell_power":
        case "extended_generic_talent_cd_reduction":
        case "extended_generic_talent_bonus_xp":
            return "+"+talent_value+"%";
        case "extended_generic_talent_respawn_reduction":
            return talent_value;
        default:
            return "+"+talent_value;
    }
...
function GetTalentLabelText(talent_name, row_level){
    ...
    //Add more label text if it is a generic extended talent
...
function ConfigureTalentClick(panel, heroID, level, luaIndex){
    ...
    panel.SetPanelEvent("onactivate", function(){
        GameEvents.SendCustomGameEventToServer("upgrade_extended_talent", {
            "heroID" : heroID,
            "level" : level,
            "index" : luaIndex
        });
...
function WhiteWashTalentChoice(rowLevel, columnIndex, bol_enable){
    ...
    if(TalentChoicePanel){
        var imageChoiceContainer = TalentChoicePanel.FindChild("EXTENDED_Talent_Choice_Image_Container");
...
function AttachToolTip(image_container_panel, talent_name, currentRowLevel, columnIndex){
    ...
    if(ability_panel != null){
        ...
        if(talent_value){
            ...
            //Note that the localized strings are to be located in /game/dota_addons/dota_extended/panorama/localization, this is different from localization of abilities
...
function PopulateEXTENDEDTalentWindow(){
    var talentPanel = $.GetContextPanel();
                    ...
                    for(var k=0; k<numOfChoicesToProcess; k++){
                        ...
                        if(TalentChoicePanel){
                            if(TalentChoiceData == null){
                                //Hide choices for extended heroes (only allow 2 choices for non-generic talents, first 6 panels will be hidden)
                                TalentChoicePanel.style.visibility = "collapse";
                            }else{
                                TalentChoicePanel.style.visibility = "visible";
                                var labelChoiceText = TalentChoicePanel.FindChildTraverse("EXTENDED_Talent_Choice_Text");
                                if(labelChoiceText){
...
function OpenExtendedTalentWindow(bol_open){
...
function ToggleEXTENDEDTalentWindow(){
    var talentWindow = $.GetContextPanel();
    OpenExtendedTalentWindow(!talentWindow.BHasClass(OPEN_TALENT_WINDOW_CLASS));
}

function PreviewExtendedTalentWindow(bol_preview){
...
function DelayedPreviewExtendedTalentWindow(){
    //Set to null as timer has fired
    _current_hover_preview_timer = null;
    PreviewExtendedTalentWindow(true);
}

function InsertEXTENDEDTalentButton(){
    $.Msg("InsertEXTENDEDTalentButton");
    var baseUI = GetHUDRootUI();
    baseUI = baseUI.FindChildTraverse("AbilitiesAndStatBranch");
    var newButton = baseUI.FindChildTraverse(EXTENDED_TALENT_BTN_ID);
    if(newButton){
        //Remove existing button
        newButton.DeleteAsync(0);
    }
    var oldButton = baseUI.FindChildTraverse("StatBranch");
    var newButton = $.CreatePanel("Panel", baseUI, EXTENDED_TALENT_BTN_ID);
    ...
    newButton.SetPanelEvent("onmouseover", function(){
        ...
        _current_hover_preview_timer = $.Schedule(0.3, DelayedPreviewExtendedTalentWindow);
    });

    newButton.SetPanelEvent("onmouseout", function(){
        //Remove preview talent window
        //Allow preview even if it is an illusion/clone
        PreviewExtendedTalentWindow(false);
    });

    newButton.SetPanelEvent("onactivate", ToggleEXTENDEDTalentWindow);
...
$.Schedule(2, InsertEXTENDEDTalentButton);
$.Schedule(2, InitializeEXTENDEDTalentWindow);
...
function OnPlayerGainedLevel(data){
    ...
    RepopulateExtendedTalentWindowOnAbilityPointsChanged();
    AnimateExtendedTalentButton();
}
