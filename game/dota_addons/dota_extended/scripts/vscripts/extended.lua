...
-- This library used to handle custom EXTENDED talent UI (hero_selection will need to use a function in this)
require('internal/extended_talent_events')
...
function GameMode:OnFirstPlayerLoaded()
	...
	local contributor_statues = {
		"npc_extended_contributor_hjort",
		"npc_extended_contributor_martyn",
		"npc_extended_contributor_mikkel",
		"npc_extended_contributor_anees",
		"npc_extended_contributor_swizard",
		"npc_extended_contributor_phroureo",
		"npc_extended_contributor_catchy",
		"npc_extended_contributor_hewdraw",
		"npc_extended_contributor_zimber",
		"npc_extended_contributor_matt",
		"npc_extended_contributor_maxime",
		"npc_extended_contributor_poly",
		"npc_extended_contributor_firstlady"
	}
	...
	for i = 1, 8 do
		...
		statue_entity:AddNewModifier(statue_entity, nil, "modifier_extended_contributor_statue", {})
	end
...
function GameMode:ItemAddedFilter( keys )
	...
	if item_name == "item_extended_aegis" then
		...
		if unit:IsRealHero() then
			...
			unit:AddNewModifier(unit, item, "modifier_item_extended_aegis",{})
	...
	if item.IsRapier then
		...
		if unit:IsRealHero() then
			...
			if 	((item_name == "item_extended_rapier") and (rapier_amount == 2)) or 
				((item_name == "item_extended_rapier_magic") and (rapier_magic_amount == 2)) or 
				((item_name == "item_extended_rapier_2") and (rapier_magic_2_amount >= 1)) or
				((item_name == "item_extended_rapier_magic_2") and (rapier_2_amount >= 1)) then
				return true
			else
...
function GameMode:OrderFilter( keys )
	...
	------------------------------------------------------------------------------------
	if unit:HasModifier("modifier_extended_death_ward") then
		if keys.order_type ==  DOTA_UNIT_ORDER_ATTACK_TARGET then
			local death_ward_mod = unit:FindModifierByName("modifier_extended_death_ward")
	...
	if unit:HasModifier("modifier_extended_death_ward_caster") then
		if keys.order_type ==  DOTA_UNIT_ORDER_ATTACK_TARGET then
			local modifier = unit:FindModifierByName("modifier_extended_death_ward_caster")
	...
	if keys.order_type == DOTA_UNIT_ORDER_CAST_TARGET then
		...
		if ability:GetAbilityName() == "extended_riki_blink_strike" then
			ability.thinker = unit:AddNewModifier(unit, ability, "modifier_extended_blink_strike_thinker", {target = keys.entindex_target})
		end
	end
	...
	if unit:HasModifier("modifier_extended_sonic_wave_daze") then
		...
		local modifier = unit:FindModifierByName("modifier_extended_sonic_wave_daze")
	...
	if keys.order_type == DOTA_UNIT_ORDER_CAST_NO_TARGET then
		...
		if unit:HasModifier("modifier_extended_skewer_motion_controller") then
			if ability:GetAbilityName() == "extended_magnataur_skewer" then
				unit:FindModifierByName("modifier_extended_skewer_motion_controller").begged_for_pardon = true
			end
		end
	...
	-- Culling Blade leap
	if unit:HasModifier("modifier_extended_axe_culling_blade_leap") then
		return false
	end
	...
	if keys.order_type == DOTA_UNIT_ORDER_CAST_POSITION then
        ...
        if ability:GetAbilityName() == "extended_techies_focused_detonate" then                        
            unit:AddNewModifier(unit, ability, "modifier_extended_focused_detonate", {duration = 0.2})            
        end
    end
...
function GameMode:DamageFilter( keys )
	if IsServer() then
		...
		if victim:HasModifier("modifier_extended_rapier_cursed") and keys.damage > 0 and victim:GetTeam() ~= attacker:GetTeam() then
		...
		-- if victim:HasModifier("modifier_extended_return_damage_block") then
		-- 	local return_ability = victim:FindAbilityByName("extended_centaur_return")
		-- 	local stacks = victim:FindModifierByName("modifier_extended_return_damage_block"):GetStackCount()
		...
		if victim:HasModifier("modifier_extended_sacred_arrow_stun") then
			local modifier_stun_handler = victim:FindModifierByName("modifier_extended_sacred_arrow_stun")
...
function GameMode:OnAllPlayersLoaded()
	...
	for _, building in pairs(buildings) do
		...
		if string.find(building_name, "fountain") then
			...
			building:AddAbility("extended_fountain_grievous_wounds")
		...
		elseif string.find(building_name, "_rax_") then
			building:SetBaseMaxHealth(3000)
			building:SetMaxHealth(3000)
...
function GameMode:OnGameInProgress()
	...
	if TOWER_ABILITY_MODE then
		local protective_instict = "extended_tower_protective_instinct"		
