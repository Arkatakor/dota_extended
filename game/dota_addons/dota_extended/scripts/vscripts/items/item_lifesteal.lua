...
LinkLuaModifier("modifier_extended_morbid_mask_unique", "items/item_lifesteal.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_extended_morbid_mask:OnCreated()        
    ...
    if IsServer() then
        ...
        if not self.caster:HasModifier("modifier_extended_morbid_mask_unique") then
            self.caster:AddNewModifier(self.caster, self.ability, "modifier_extended_morbid_mask_unique", {})
        end
    end
end
...
function modifier_extended_morbid_mask:OnDestroy()
    if IsServer() then
        if not self.caster:HasModifier("modifier_extended_morbid_mask") then
            self.caster:RemoveModifierByName("modifier_extended_morbid_mask_unique")
        end
...
function modifier_extended_morbid_mask:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end

modifier_extended_morbid_mask_unique = class({})
function modifier_extended_morbid_mask_unique:IsHidden() return true end
function modifier_extended_morbid_mask_unique:IsPurgable() return false end
function modifier_extended_morbid_mask_unique:IsDebuff() return false end

function modifier_extended_morbid_mask_unique:OnCreated()
    -- Ability properties
    self.ability = self:GetAbility()
...
function modifier_extended_morbid_mask_unique:OnRefresh()
    self:OnCreated()
end

function modifier_extended_morbid_mask_unique:GetModifierLifesteal()
