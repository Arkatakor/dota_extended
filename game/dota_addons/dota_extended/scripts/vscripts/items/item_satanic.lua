...
LinkLuaModifier("modifier_extended_satanic_unique", "items/item_satanic.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_extended_satanic:OnCreated()        
    ...
    if not self.caster:HasModifier("modifier_extended_satanic_unique") then
        self.caster:AddNewModifier(self.caster, self.ability, "modifier_extended_satanic_unique", {})
    end
...
function modifier_extended_satanic:OnDestroy()
    if IsServer() then      
        ...
        if not self.caster:HasModifier("modifier_extended_satanic") then
            self.caster:RemoveModifierByName("modifier_extended_satanic_unique")
        end
...
function modifier_extended_satanic:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end


-- Unique Satanic modifier
modifier_extended_satanic_unique = class({})
function modifier_extended_satanic_unique:IsHidden() return true end
function modifier_extended_satanic_unique:IsPurgable() return false end
function modifier_extended_satanic_unique:IsDebuff() return false end

function modifier_extended_satanic_unique:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()    
...
function modifier_extended_satanic_unique:DeclareFunctions()
    local decFunc = {MODIFIER_EVENT_ON_ATTACK_LANDED}

    return decFunc
end

function modifier_extended_satanic_unique:GetModifierLifesteal()    
...
function modifier_extended_satanic_unique:OnAttackLanded(keys)
    if IsServer() then
...
function modifier_extended_satanic_soul_slaughter:OnRefresh()
    self:OnCreated()
end
