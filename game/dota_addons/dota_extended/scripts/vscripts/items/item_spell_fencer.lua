...
LinkLuaModifier( "modifier_item_extended_spell_fencer_unique", "items/item_spell_fencer.lua", LUA_MODIFIER_MOTION_NONE )	-- Unique passive modifier
LinkLuaModifier( "modifier_item_extended_spell_fencer_buff", "items/item_spell_fencer.lua", LUA_MODIFIER_MOTION_NONE )		-- Physical damage prevention modifier
...
function modifier_item_extended_spell_fencer:OnCreated(keys)
	if IsServer() then
		local parent = self:GetParent()
		if not parent:HasModifier("modifier_item_extended_spell_fencer_unique") then
			parent:AddNewModifier(parent, self:GetAbility(), "modifier_item_extended_spell_fencer_unique", {})
		end
	end
end

-- Removes the unique modifier from the bearer if this is the last spellfencer in its inventory
function modifier_item_extended_spell_fencer:OnDestroy(keys)
	if IsServer() then
		local parent = self:GetParent()
		if not parent:HasModifier("modifier_item_extended_spell_fencer") then
			parent:RemoveModifierByName("modifier_item_extended_spell_fencer_unique")
		end
	end
end
...
function modifier_item_extended_spell_fencer:GetCustomCooldownReduction()
...
if modifier_item_extended_spell_fencer_unique == nil then modifier_item_extended_spell_fencer_unique = class({}) end
function modifier_item_extended_spell_fencer_unique:IsHidden() return true end
function modifier_item_extended_spell_fencer_unique:IsDebuff() return false end
function modifier_item_extended_spell_fencer_unique:IsPurgable() return false end
function modifier_item_extended_spell_fencer_unique:IsPermanent() return true end

-- Declare modifier events/properties
function modifier_item_extended_spell_fencer_unique:DeclareFunctions()
	local funcs = {
...
function modifier_item_extended_spell_fencer_unique:OnAttackLanded( keys )
	if IsServer() then
