...
if item_extended_hellblade == nil then
	item_extended_hellblade = class({})
end
LinkLuaModifier("modifier_item_extended_hellblade", "items/item_hellblade", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_extended_hellblade_unique", "items/item_hellblade", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_extended_hellblade_debuff", "items/item_hellblade", LUA_MODIFIER_MOTION_NONE)


function item_extended_hellblade:GetIntrinsicModifierName()
	return "modifier_item_extended_hellblade"
end

function item_extended_hellblade:OnSpellStart()
	if IsServer() then
			-- Ability properties
		...
		local debuff = "modifier_item_extended_hellblade_debuff"	
...
if modifier_item_extended_hellblade == nil then
	modifier_item_extended_hellblade = class({})
end

function modifier_item_extended_hellblade:IsHidden() return true end
function modifier_item_extended_hellblade:IsPurgable() return false end
function modifier_item_extended_hellblade:IsDebuff() return false end
function modifier_item_extended_hellblade:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end

function modifier_item_extended_hellblade:OnCreated()	
	-- Ability properties
	...
	if IsServer() then
		...
		if not self.caster:HasModifier("modifier_item_extended_hellblade_unique") then
			self.caster:AddNewModifier(self.caster, self.ability, "modifier_item_extended_hellblade_unique", {})
		end
	end
end

-- Removes the unique modifier from the caster if this is the last skadi in its inventory
function modifier_item_extended_hellblade:OnDestroy()
	if IsServer() then		
		if not self.caster:HasModifier("modifier_item_extended_hellblade") then
			self.caster:RemoveModifierByName("modifier_item_extended_hellblade_unique")
		end
	end
end

function modifier_item_extended_hellblade:DeclareFunctions()
...
function modifier_item_extended_hellblade:GetModifierBonusStats_Agility()	
	return self.agility_bonus
end

function modifier_item_extended_hellblade:GetModifierBonusStats_Intellect()	
	return self.intelligence_bonus
end

function modifier_item_extended_hellblade:GetModifierBonusStats_Strength()
	return self.strength_bonus
end

function modifier_item_extended_hellblade:GetModifierBaseAttack_BonusDamage()	
	return self.damage
end
...
modifier_item_extended_hellblade_unique = class({})

function modifier_item_extended_hellblade_unique:IsHidden() return true end
function modifier_item_extended_hellblade_unique:IsPurgable() return false end
function modifier_item_extended_hellblade_unique:IsDebuff() return false end


function modifier_item_extended_hellblade_unique:OnCreated()
	-- Ability properties
	self.caster = self:GetCaster()
	self.ability = self:GetAbility()
	self.parent = self:GetParent()
	self.sound_cast = "Extended.Curseblade"
...
function modifier_item_extended_hellblade_unique:DeclareFunctions()
	local decFuncs = {MODIFIER_EVENT_ON_ATTACKED}

	return decFuncs
end

function modifier_item_extended_hellblade_unique:OnAttacked(keys)
	if IsServer() then
		-- Ability properties		
...
modifier_item_extended_hellblade_debuff = class({})

function modifier_item_extended_hellblade_debuff:IsDebuff()
	return true
end

function modifier_item_extended_hellblade_debuff:IsHidden()
	return false
end

function modifier_item_extended_hellblade_debuff:IsPurgable()
	return true
end

function modifier_item_extended_hellblade_debuff:OnCreated()
	-- Ability properties
	self.caster = self:GetCaster()
	self.parent = self:GetParent()
	self.ability = self:GetAbility()
...
function modifier_item_extended_hellblade_debuff:DeclareFunctions()
	local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
		
	return decFuncs	
end

function modifier_item_extended_hellblade_debuff:GetModifierMoveSpeedBonus_Percentage()
	return self.slow_amount
end

function modifier_item_extended_hellblade_debuff:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_item_extended_hellblade_debuff:GetEffectName()
	return "particles/item/curseblade/extended_hellblade_curse.vpcf"
end

function modifier_item_extended_hellblade_debuff:OnIntervalThink()
	if IsServer() then				
