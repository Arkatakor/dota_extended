...
extended_lion_earth_spike = class({})
LinkLuaModifier("modifier_extended_earthspike_stun", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)

function extended_lion_earth_spike:IsHiddenWhenStolen()
     return false
 end 

function extended_lion_earth_spike:OnSpellStart()
    -- Ability properties
...
function extended_lion_earth_spike:OnProjectileHit_ExtraData(target, location, extra_data)
    ...
    local sound_cast = "Hero_Lion.Impale"
    local particle_projectile = "particles/units/heroes/hero_lion/lion_spell_impale.vpcf"
    local modifier_stun = "modifier_extended_earthspike_stun"
    ...
    damage = damage + caster:FindTalentValue("special_bonus_extended_lion_2")
    ...
    if caster:HasTalent("special_bonus_extended_lion_1") then
...
modifier_extended_earthspike_stun = class({})

function modifier_extended_earthspike_stun:IsHidden() return false end
function modifier_extended_earthspike_stun:IsPurgeException() return true end
function modifier_extended_earthspike_stun:IsStunDebuff() return true end

function modifier_extended_earthspike_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_earthspike_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_earthspike_stun:GetEffectAttachType()
...
extended_lion_hex = class({})
LinkLuaModifier("modifier_extended_lion_hex", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)

function extended_lion_hex:IsHiddenWhenStolen()
    return false
end

function extended_lion_hex:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_hex = "modifier_extended_lion_hex"
...
-- Hex modifier
modifier_extended_lion_hex = class({})

function modifier_extended_lion_hex:OnCreated()    
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.sound_cast = "Hero_Lion.Voodoo"
    self.sound_meme_firetoad = "Extended.LionHexREEE"
    ...
    self.modifier_hex = "modifier_extended_lion_hex"
    ...
    self.hex_bounce_radius = self.hex_bounce_radius + self.caster:FindTalentValue("special_bonus_extended_lion_3")

    -- #6 Talent: Hex bounces to a second enemy
    self.maximum_hex_enemies = self.maximum_hex_enemies + self.caster:FindTalentValue("special_bonus_extended_lion_6")
...
function modifier_extended_lion_hex:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_lion_hex:IsHidden() return false end
function modifier_extended_lion_hex:IsPurgable() return true end
function modifier_extended_lion_hex:IsDebuff() return true end

function modifier_extended_lion_hex:CheckState()
...
function modifier_extended_lion_hex:DeclareFunctions()
...
function modifier_extended_lion_hex:GetModifierModelChange()
    return "models/props_gameplay/frog.vmdl"
end

function modifier_extended_lion_hex:GetModifierMoveSpeed_Absolute()
    return self.move_speed
end

function modifier_extended_lion_hex:OnDestroy()
    if IsServer() then
...
extended_lion_mana_drain = class({})
LinkLuaModifier("modifier_extended_manadrain_aura", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_manadrain_aura_debuff", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_manadrain_debuff", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_manadrain_buff", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)

function extended_lion_mana_drain:OnUnStolen()
    local caster = self:GetCaster()

    -- Remove Rubick's Mana Drain aura
    if caster:HasModifier("modifier_extended_manadrain_aura") then
        caster:RemoveModifierByName("modifier_extended_manadrain_aura")
    end
end

function extended_lion_mana_drain:IsHiddenWhenStolen()
    return false
end


function extended_lion_mana_drain:GetIntrinsicModifierName()
     return "modifier_extended_manadrain_aura"
end

function extended_lion_mana_drain:CastFilterResultTarget(target)
    if IsServer() then
        local caster = self:GetCaster()
...
function extended_lion_mana_drain:GetCustomCastErrorTarget(target)
    local caster = self:GetCaster()
...
function extended_lion_mana_drain:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget()        
    local sound_cast = "Hero_Lion.ManaDrain"    
    local modifier_drain_enemy = "modifier_extended_manadrain_debuff"
    local modifier_drain_ally = "modifier_extended_manadrain_buff"
    ...
    if enemy_target and caster:HasTalent("special_bonus_extended_lion_8") then
...
-- Mana Drain Aura
modifier_extended_manadrain_aura = class({})

function modifier_extended_manadrain_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_manadrain_aura:OnRefresh()
    self:OnCreated()
end

function modifier_extended_manadrain_aura:IsHidden() return true end
function modifier_extended_manadrain_aura:IsPurgable() return false end
function modifier_extended_manadrain_aura:IsDebuff() return false end

function modifier_extended_manadrain_aura:GetAuraDuration()
    return 0.1
end

function modifier_extended_manadrain_aura:GetAuraRadius()
    return self.aura_radius
end

function modifier_extended_manadrain_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NOT_ANCIENTS
end

function modifier_extended_manadrain_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_manadrain_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_manadrain_aura:GetEffectName()    
    return "particles/hero/lion/aura_manadrain.vpcf"
end

function modifier_extended_manadrain_aura:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_manadrain_aura:GetModifierAura()
    return "modifier_extended_manadrain_aura_debuff"
end

function modifier_extended_manadrain_aura:IsAura()
...
modifier_extended_manadrain_aura_debuff = class({})

function modifier_extended_manadrain_aura_debuff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_manadrain_aura_debuff:IsHidden() return false end
function modifier_extended_manadrain_aura_debuff:IsPurgable() return false end
function modifier_extended_manadrain_aura_debuff:IsDebuff() return true end

function modifier_extended_manadrain_aura_debuff:OnIntervalThink()
    if IsServer() then
...
modifier_extended_manadrain_debuff = class({})

function modifier_extended_manadrain_debuff:OnCreated()    
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()
...
function modifier_extended_manadrain_debuff:IsHidden() return false end
function modifier_extended_manadrain_debuff:IsPurgable() return false end
function modifier_extended_manadrain_debuff:IsDebuff() return true end

function modifier_extended_manadrain_debuff:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_manadrain_debuff:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_manadrain_debuff:GetModifierMoveSpeedBonus_Percentage()
    -- #4 Talent: Mana drain slows the target according to missing mana
    if self.caster:HasTalent("special_bonus_extended_lion_4") then
...
modifier_extended_manadrain_buff = class({})

function modifier_extended_manadrain_buff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_manadrain_buff:OnIntervalThink()
    if IsServer() then
        -- Get target's current mana
...
--          FINGER OF DEATH             --
------------------------------------------
extended_lion_finger_of_death = class({})
LinkLuaModifier("modifier_extended_trigger_finger_debuff", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_finger_of_death_hex", "hero/hero_lion", LUA_MODIFIER_MOTION_NONE)

function extended_lion_finger_of_death:IsHiddenWhenStolen()
    return false
end

function extended_lion_finger_of_death:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
    local enemies_frog_radius = ability:GetSpecialValueFor("enemies_frog_radius")

    -- #7 Talent: Finger of Death Hex radius increase
    enemies_frog_radius = enemies_frog_radius + caster:FindTalentValue("special_bonus_extended_lion_7")

    return enemies_frog_radius
end

function extended_lion_finger_of_death:GetManaCost(level)
    -- Ability properties
    local caster = self:GetCaster()    
    local ability = self
    local modifier_finger = "modifier_extended_trigger_finger_debuff"
...
function extended_lion_finger_of_death:GetCooldown(level)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function extended_lion_finger_of_death:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget()
    ...
    local modifier_finger = "modifier_extended_trigger_finger_debuff"    
    local modifier_hex = "modifier_extended_finger_of_death_hex"
    local scepter = caster:HasScepter()
    ...
    triggerfinger_duration = triggerfinger_duration - caster:FindTalentValue("special_bonus_extended_lion_5")    
...
modifier_extended_trigger_finger_debuff = class({})

function modifier_extended_trigger_finger_debuff:IsHidden() return false end
function modifier_extended_trigger_finger_debuff:IsPurgable() return false end
function modifier_extended_trigger_finger_debuff:IsDebuff() return true end


-- Finger's Hex modifier
modifier_extended_finger_of_death_hex = class({})

function modifier_extended_finger_of_death_hex:IsHidden() return false end
function modifier_extended_finger_of_death_hex:IsPurgable() return true end
function modifier_extended_finger_of_death_hex:IsDebuff() return true end

function modifier_extended_finger_of_death_hex:CheckState()
    local state = {[MODIFIER_STATE_HEXED] = true,
                   [MODIFIER_STATE_DISARMED] = true,
                   [MODIFIER_STATE_SILENCED] = true,
                   [MODIFIER_STATE_MUTED] = true}
...
function modifier_extended_finger_of_death_hex:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MODEL_CHANGE,
                      MODIFIER_PROPERTY_MOVESPEED_ABSOLUTE}
...
function modifier_extended_finger_of_death_hex:GetModifierModelChange()
    return "models/props_gameplay/frog.vmdl"
end

function modifier_extended_finger_of_death_hex:GetModifierMoveSpeed_Absolute()
