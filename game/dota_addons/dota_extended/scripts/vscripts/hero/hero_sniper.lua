...
--         SHRAPNEL           --
--------------------------------
extended_sniper_shrapnel = class({})
LinkLuaModifier("modifier_extended_shrapnel_attack", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_shrapnel_charges", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_shrapnel_aura", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_shrapnel_slow", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sniper_shrapnel:GetIntrinsicModifierName()
    return "modifier_extended_shrapnel_charges"
end

function extended_sniper_shrapnel:IsHiddenWhenStolen()
    return false
end

function extended_sniper_shrapnel:GetAOERadius()
    local ability = self
    local radius = ability:GetSpecialValueFor("radius")
    return radius
end

function extended_sniper_shrapnel:OnSpellStart()
    -- Ability properties
    ...
    local modifier_charge = "modifier_extended_shrapnel_charges"
    local modifier_slow_aura = "modifier_extended_shrapnel_aura"
    local modifier_range = "modifier_extended_shrapnel_attack"
...
modifier_extended_shrapnel_attack = class({})

function modifier_extended_shrapnel_attack:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()
        self.modifier_slow = "modifier_extended_shrapnel_slow"
        self.modifier_attack = "modifier_extended_shrapnel_attack"
...
function modifier_extended_shrapnel_attack:IsHidden() return false end
function modifier_extended_shrapnel_attack:IsPurgable() return false end
function modifier_extended_shrapnel_attack:IsDebuff() return false end

function modifier_extended_shrapnel_attack:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_shrapnel_attack:DeclareFunctions()
...
function modifier_extended_shrapnel_attack:OnOrder(keys)
    if IsServer() then
...
function modifier_extended_shrapnel_attack:GetModifierAttackRangeBonus()
...
function modifier_extended_shrapnel_attack:GetModifierDamageOutgoing_Percentage()
    if IsServer() then
        ...
        if self.distance_from_target <= (caster_attack_range - self.global_fire_distance) then
            ...
            local distance_damage_pct = distance_damage_pct + self.caster:FindTalentValue("special_bonus_extended_sniper_1")      
...
modifier_extended_shrapnel_charges = class({})

function modifier_extended_shrapnel_charges:OnCreated()    
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()
        self.modifier_charge = "modifier_extended_shrapnel_charges"
...
function modifier_extended_shrapnel_charges:OnIntervalThink()
    if IsServer() then
        ...
        if self.caster:HasTalent("special_bonus_extended_sniper_5") and not self.talent5 then

            -- Grant the stacks immediately, mark talent5 as applied
            self.talent5 = true

            local talent_stacks = self.caster:FindTalentValue("special_bonus_extended_sniper_5")
...
function modifier_extended_shrapnel_charges:OnStackCountChanged()
    if IsServer() then
        -- Current stacks
        local stacks = self:GetStackCount() 
...
function modifier_extended_shrapnel_charges:DestroyOnExpire()
    return false
end

function modifier_extended_shrapnel_charges:IsHidden() return false end
function modifier_extended_shrapnel_charges:IsPurgable() return false end
function modifier_extended_shrapnel_charges:IsDebuff() return false end

-- Shrapnel slow aura modifier
modifier_extended_shrapnel_aura = class({})

function modifier_extended_shrapnel_aura:IsHidden() return true end
function modifier_extended_shrapnel_aura:IsPurgable() return false end
function modifier_extended_shrapnel_aura:IsDebuff() return false end

function modifier_extended_shrapnel_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()    
...
function modifier_extended_shrapnel_aura:GetAuraRadius()
    return self.radius
end

function modifier_extended_shrapnel_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_shrapnel_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_shrapnel_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_shrapnel_aura:GetModifierAura()
    return "modifier_extended_shrapnel_slow"
end

function modifier_extended_shrapnel_aura:IsAura()
    return true
end


-- Shrapnel slow debuff modifier
modifier_extended_shrapnel_slow = class({})

function modifier_extended_shrapnel_slow:IsHidden() return false end
function modifier_extended_shrapnel_slow:IsPurgable() return false end 
function modifier_extended_shrapnel_slow:IsDebuff() return true end

function modifier_extended_shrapnel_slow:OnCreated()    
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_shrapnel_slow:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_shrapnel_slow:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_shrapnel_slow:GetModifierMoveSpeedBonus_Percentage()
...
--         HEADSHOT           --
--------------------------------
extended_sniper_headshot = class({})
LinkLuaModifier("modifier_extended_headshot_attacks", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_headshot_slow", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_perfectshot_stun", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sniper_headshot:GetIntrinsicModifierName()
    return "modifier_extended_headshot_attacks"
end

function extended_sniper_headshot:IsHiddenWhenStolen()
    return true
end

-- Attacks counter
modifier_extended_headshot_attacks = class({})

function modifier_extended_headshot_attacks:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()        
        ...
        self.modifier_headshot_counter = "modifier_extended_headshot_counter"
        self.modifier_extended_headshot_slow = "modifier_extended_headshot_slow"
        self.modifier_extended_perfectshot_stun = "modifier_extended_perfectshot_stun"
        self.ability_aim = "extended_sniper_take_aim"
...
function modifier_extended_headshot_attacks:OnRefresh()
    self:OnCreated()
end

function modifier_extended_headshot_attacks:IsHidden() return true end
function modifier_extended_headshot_attacks:IsPurgable() return false end
function modifier_extended_headshot_attacks:IsDebuff() return false end

function modifier_extended_headshot_attacks:DeclareFunctions()
...
function modifier_extended_headshot_attacks:OnAttackStart(keys)
    if IsServer() then        
...
function modifier_extended_headshot_attacks:OnAttack(keys)
    if IsServer() then        
        local attacker = keys.attacker
        local target = keys.target             
...
function modifier_extended_headshot_attacks:OnAttackLanded(keys)
    if IsServer() then        
        local attacker = keys.attacker
        local target = keys.target             
        ...
        if attacker == self.caster then  
            ...
            local headshot_duration = self.headshot_duration * (1 + (self.caster:FindTalentValue("special_bonus_extended_sniper_3") * 0.01))
            local perfectshot_stun_duration = self.perfectshot_stun_duration * (1+ (self.caster:FindTalentValue("special_bonus_extended_sniper_3") * 0.01))            

            -- If a Perfectshot is marked, headshot and stun the target
            if self.attack_perfectshot_stun then
                target:AddNewModifier(self.caster, self.ability, self.modifier_extended_headshot_slow, {duration = headshot_duration})
                target:AddNewModifier(self.caster, self.ability, self.modifier_extended_perfectshot_stun, {duration = perfectshot_stun_duration})
...
function modifier_extended_headshot_attacks:OnAttackFinished(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.target             
...
function modifier_extended_headshot_attacks:ApplyHeadshotMarks()
    -- Apply marks associated with headshots
    self.enable_headshot_bonus_damage = true
end

function modifier_extended_headshot_attacks:ApplyAllMarks()  
    -- Apply marks    
    self.enable_headshot_bonus_damage = true
...
function modifier_extended_headshot_attacks:ClearAllMarks()
    -- Clear all marks
...
function modifier_extended_headshot_attacks:GetModifierPreAttack_BonusDamage()
    if IsServer() then
...
function modifier_extended_headshot_attacks:GetModifierPreAttack_CriticalStrike()
    if IsServer() then
        -- Only apply if the next shot is going to be a perfect shot
        if self.enable_critical_damage then

            -- #6 Talent: Perfectshot critical damage increase
            local perfectshot_critical_dmg_pct = self.perfectshot_critical_dmg_pct + self.caster:FindTalentValue("special_bonus_extended_sniper_6")
            return perfectshot_critical_dmg_pct
        end        
    end
end

function modifier_extended_headshot_attacks:OnStackCountChanged(old_stack_count)
    if IsServer() then
...
modifier_extended_headshot_slow = class({})

function modifier_extended_headshot_slow:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()    
...
function modifier_extended_headshot_slow:DeclareFunctions()
...
function modifier_extended_headshot_slow:GetModifierAttackSpeedBonus_Constant()
    return self.headshot_as_slow * (-1)
end

function modifier_extended_headshot_slow:GetModifierMoveSpeedBonus_Percentage()
    return self.headshot_ms_slow_pct * (-1)
end

function modifier_extended_headshot_slow:IsHidden() return false end
function modifier_extended_headshot_slow:IsPurgable() return true end
function modifier_extended_headshot_slow:IsDebuff() return true end


-- Perfectshot stun modifier
modifier_extended_perfectshot_stun = class({})

function modifier_extended_perfectshot_stun:OnCreated()
    -- Ability properties
    self.ability = self:GetAbility()
...
function modifier_extended_perfectshot_stun:CheckState()
    local state    
...
function modifier_extended_perfectshot_stun:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,
                      MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
...
function modifier_extended_perfectshot_stun:IsHidden() return false end
function modifier_extended_perfectshot_stun:IsPurgeException() return true end
function modifier_extended_perfectshot_stun:IsStunDebuff() return true end


--------------------------------
--         TAKE AIM           --
--------------------------------
extended_sniper_take_aim = class({})
LinkLuaModifier("modifier_extended_take_aim_range", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sniper_take_aim:GetIntrinsicModifierName()
    return "modifier_extended_take_aim_range"
end

function extended_sniper_take_aim:IsStealable()
    return false
end

function extended_sniper_take_aim:GetCooldown(level)   
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #4 Talent: Take Aim cooldown decrease
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_sniper_4")

    return cooldown
end

function extended_sniper_take_aim:OnSpellStart() return end

-- Bonus range modifier
modifier_extended_take_aim_range = class({})

function modifier_extended_take_aim_range:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.modifier_headshot = "modifier_extended_headshot_attacks"
...
function modifier_extended_take_aim_range:IsHidden() return true end
function modifier_extended_take_aim_range:IsPurgable() return false end
function modifier_extended_take_aim_range:IsDebuff() return false end
    
function modifier_extended_take_aim_range:OnRefresh()
    self:OnCreated()
end

function modifier_extended_take_aim_range:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_take_aim_range:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_ATTACK_RANGE_BONUS,
                      MODIFIER_EVENT_ON_ATTACK}
...
function modifier_extended_take_aim_range:OnAttack(keys)
    if IsServer() then
        local attacker = keys.attacker
...
function modifier_extended_take_aim_range:GetModifierAttackRangeBonus()    
    ...
    local aim_bonus_range = self.aim_bonus_range + self.caster:FindTalentValue("special_bonus_extended_sniper_7")
...
--        ASSASSINATE         --
--------------------------------
extended_sniper_assassinate = class({})
LinkLuaModifier("modifier_extended_assassinate_cross", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_assassinate_ministun", "hero/hero_sniper.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sniper_assassinate:IsHiddenWhenStolen()
    return false
end

function extended_sniper_assassinate:GetBehavior()
    local caster = self:GetCaster()
...
function extended_sniper_assassinate:GetCastRange(location, target)
    local caster = self:GetCaster()
    local cast_range = self.BaseClass.GetCastRange(self, location, target)

    -- #8 Talent: Global Assassinate
    if caster:HasTalent("special_bonus_extended_sniper_8") then
        cast_range = caster:FindTalentValue("special_bonus_extended_sniper_8")
    end

    return cast_range
end

function extended_sniper_assassinate:GetAOERadius()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local scepter = caster:HasScepter()
...
function extended_sniper_assassinate:GetAssociatedSecondaryAbilities()
    return "extended_sniper_headshot"
end

function extended_sniper_assassinate:OnAbilityPhaseStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
    ...
    local modifier_cross = "modifier_extended_assassinate_cross"
    local scepter = caster:HasScepter()        
...
function extended_sniper_assassinate:OnAbilityPhaseInterrupted()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local modifier_cross = "modifier_extended_assassinate_cross"
...
function extended_sniper_assassinate:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
...
function extended_sniper_assassinate:OnProjectileHit(target, location)
    ...
    local modifier_cross = "modifier_extended_assassinate_cross"  
    local modifier_ministun = "modifier_extended_assassinate_ministun"
...
function extended_sniper_assassinate:AssassinateHit(target)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self      
    ...
    local modifier_ministun = "modifier_extended_assassinate_ministun"
    local modifier_perfectshot = "modifier_extended_perfectshot_stun"
    local modifier_headshot = "modifier_extended_headshot_slow"
    local head_shot_ability = "extended_sniper_headshot"        
    local scepter = caster:HasScepter()
    ...
    if not scepter then
        ...
        if caster:HasAbility(head_shot_ability) then
            ...
            if head_shot_ability_handler then
                ...
                if caster:HasModifier("modifier_item_extended_spell_fencer_unique") then
                    damage_type = DAMAGE_TYPE_MAGICAL
                end
    ...
    if caster:HasTalent("special_bonus_extended_sniper_2") then
        local push_distance = caster:FindTalentValue("special_bonus_extended_sniper_2")
...
function extended_sniper_assassinate:OnProjectileThink(location)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
-- Cross enemy debuff
modifier_extended_assassinate_cross = class({})

function modifier_extended_assassinate_cross:OnCreated()
    if IsServer() then
        self.caster = self:GetCaster()    
        self.parent = self:GetParent()
...
function modifier_extended_assassinate_cross:GetAttributes()
    return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE    
end

function modifier_extended_assassinate_cross:IsHidden() return false end
function modifier_extended_assassinate_cross:IsPurgable() return false end
function modifier_extended_assassinate_cross:IsDebuff() return true end
    
function modifier_extended_assassinate_cross:CheckState()    
    local state = nil
...
function modifier_extended_assassinate_cross:GetPriority()
    return MODIFIER_PRIORITY_HIGH
end

-- Ministun debuff
modifier_extended_assassinate_ministun = class({})

function modifier_extended_assassinate_ministun:IsHidden() return false end
function modifier_extended_assassinate_ministun:IsPurgeException() return true end
function modifier_extended_assassinate_ministun:IsStunDebuff() return true end

function modifier_extended_assassinate_ministun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_assassinate_ministun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_assassinate_ministun:GetEffectAttachType()
