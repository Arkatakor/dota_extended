...
--       ARCANE ORB      --
---------------------------
extended_obsidian_destroyer_arcane_orb = class({})
LinkLuaModifier("modifier_extended_arcane_orb_thinker", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_arcane_orb_buff", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_arcane_orb_debuff", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)

function extended_obsidian_destroyer_arcane_orb:IsStealable()
    return false
end

function extended_obsidian_destroyer_arcane_orb:IsNetherWardStealable()
    return false
end

function extended_obsidian_destroyer_arcane_orb:GetIntrinsicModifierName()
    return "modifier_extended_arcane_orb_thinker"
end

function extended_obsidian_destroyer_arcane_orb:OnSpellStart()
    if IsServer() then
        local caster = self:GetCaster()
        local ability = self
        local modifier = "modifier_extended_arcane_orb_thinker"
...
function extended_obsidian_destroyer_arcane_orb:GetCastRange(location, target)
...
modifier_extended_arcane_orb_thinker = class({})

function modifier_extended_arcane_orb_thinker:DeclareFunctions()
...
function modifier_extended_arcane_orb_thinker:OnCreated()
    -- Ability properties
    ...
    self.modifier_buff = "modifier_extended_arcane_orb_buff"  
    self.modifier_debuff = "modifier_extended_arcane_orb_debuff"    
    self.modifier_essence = "modifier_extended_essence_aura_buff"
...
function modifier_extended_arcane_orb_thinker:OnAttackStart(keys)
    if IsServer() then  
...
function modifier_extended_arcane_orb_thinker:OnAttack(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.target
...
function modifier_extended_arcane_orb_thinker:OnAttackLanded(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.target
...
function modifier_extended_arcane_orb_thinker:OnRefresh()
    self:OnCreated()
end

function modifier_extended_arcane_orb_thinker:ApplyArcaneOrbAttack(target)    

    -- Apply effect if the target didn't suddenly become magic immune              
    if not target:IsMagicImmune() then

        -- #1 Talent: Arcane Orb max mana as damage increase
        local mana_pool_damage_pct = self.mana_pool_damage_pct + self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_1")
        ...
        local int_steal_count = self.int_steal_count + self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_3")
...
function modifier_extended_arcane_orb_thinker:OnOrder(keys)
...
function SetArcaneOrbProjectile(caster, orb_attack)
    ...
    local satanic_modifier = "modifier_extended_satanic"
...
function modifier_extended_arcane_orb_thinker:IsHidden()
    return true
end

function modifier_extended_arcane_orb_thinker:IsPurgable()
    return false
end

function modifier_extended_arcane_orb_thinker:IsDebuff()
    return false
end
...
function ApplyIntelligenceSteal(caster, ability, target, stack_count, duration)    
    ...
    local modifier_buff = "modifier_extended_arcane_orb_buff"  
    local modifier_debuff = "modifier_extended_arcane_orb_debuff"           

    -- #4 Talent: Stolen intelligence duration increased
    duration = duration * (1 + (caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_4") * 0.01))    
...
modifier_extended_arcane_orb_buff = class({})

function modifier_extended_arcane_orb_buff:GetTexture()
    return "obsidian_destroyer_arcane_orb"
end

function modifier_extended_arcane_orb_buff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        -- #4 Talent: Stolen intelligence duration increased
        self.int_steal_duration = self.int_steal_duration * (1 + (self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_4") * 0.01))
...
function modifier_extended_arcane_orb_buff:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_arcane_orb_buff:OnRefresh()
    if IsServer() then
...
function modifier_extended_arcane_orb_buff:IsHidden() return false end
function modifier_extended_arcane_orb_buff:IsPurgable() return false end
function modifier_extended_arcane_orb_buff:IsDebuff() return false end

function modifier_extended_arcane_orb_buff:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_STATS_INTELLECT_BONUS}

    return decFunc
end

function modifier_extended_arcane_orb_buff:GetModifierBonusStats_Intellect()
...
modifier_extended_arcane_orb_debuff = class({})

function modifier_extended_arcane_orb_debuff:GetTexture()
    return "obsidian_destroyer_arcane_orb"
end

function modifier_extended_arcane_orb_debuff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_arcane_orb_debuff:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_arcane_orb_debuff:OnRefresh()
    if IsServer() then
        -- Insert new stack values
        table.insert(self.stacks_table, GameRules:GetGameTime())
    end
...
function modifier_extended_arcane_orb_debuff:IsHidden() return false end
function modifier_extended_arcane_orb_debuff:IsPurgable() return false end
function modifier_extended_arcane_orb_debuff:IsDebuff() return true end

function modifier_extended_arcane_orb_debuff:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_STATS_INTELLECT_BONUS}

    return decFunc
end

function modifier_extended_arcane_orb_debuff:GetModifierBonusStats_Intellect()
    local stacks = self:GetStackCount()
    return stacks * (-1)
end
...
--  ASTRAL IMPRISONMENT  --
---------------------------
extended_obsidian_destroyer_astral_imprisonment = class({})
LinkLuaModifier("modifier_extended_astral_imprisonment", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_astral_imprisonment_buff", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)

function extended_obsidian_destroyer_astral_imprisonment:GetBehavior()
    local caster = self:GetCaster()
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
...
function extended_obsidian_destroyer_astral_imprisonment:CastFilterResultTarget(target)   
    if IsServer() then
        local caster = self:GetCaster()
...
function extended_obsidian_destroyer_astral_imprisonment:GetCastPoint()
    local caster = self:GetCaster()
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
...
function extended_obsidian_destroyer_astral_imprisonment:GetCastRange(location, target)
    local caster = self:GetCaster()
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
    ...
    cast_range = cast_range + caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_2")
    return cast_range    
end

function extended_obsidian_destroyer_astral_imprisonment:GetManaCost(level)
    local caster = self:GetCaster()
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
...
function extended_obsidian_destroyer_astral_imprisonment:GetCooldown(level)
    local caster = self:GetCaster()
    local ability = self
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
...
function extended_obsidian_destroyer_astral_imprisonment:IsHiddenWhenStolen()
    local caster = self:GetCaster()
    if caster:HasAbility("extended_obsidian_destroyer_sanity_eclipse") then
        return true
    end
...
function extended_obsidian_destroyer_astral_imprisonment:OnSpellStart()    
    -- Ability properties    
    local caster = self:GetCaster()
    local ability = self    
    local modifier_self = "modifier_extended_astral_imprisonment_buff"
    ...
    if not caster:HasModifier(modifier_self) then
        ...
        local modifier_prison = "modifier_extended_astral_imprisonment"        
        local modifier_essence = "modifier_extended_essence_aura_buff"
...
-- Prison modifier 
modifier_extended_astral_imprisonment = class({})

function modifier_extended_astral_imprisonment:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_prison_self = "modifier_extended_astral_imprisonment_buff"
...
function modifier_extended_astral_imprisonment:IsHidden() return false end
function modifier_extended_astral_imprisonment:IsPurgable() return false end
function modifier_extended_astral_imprisonment:IsDebuff() return true end

function modifier_extended_astral_imprisonment:CheckState()
...
function modifier_extended_astral_imprisonment:OnDestroy()
    if IsServer() then
        -- Play end sound
...
-- Self prison buff 
modifier_extended_astral_imprisonment_buff = class({})

function modifier_extended_astral_imprisonment_buff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.prison_movespeed = self.prison_movespeed + self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_6")
...
function modifier_extended_astral_imprisonment_buff:OnIntervalThink()
...
function modifier_extended_astral_imprisonment_buff:OnDestroy()
    if IsServer() then
...
function modifier_extended_astral_imprisonment_buff:IsHidden() return true end
function modifier_extended_astral_imprisonment_buff:IsPurgable() return false end
function modifier_extended_astral_imprisonment_buff:IsDebuff() return false end
...
--     ESSENCE AURA      --
---------------------------
extended_obsidian_destroyer_essence_aura = class({})
LinkLuaModifier("modifier_extended_essence_aura", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_essence_aura_buff", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_essence_aura_proc", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_essence_aura_over_maximum", "hero/hero_obsidian_destroyer.lua", LUA_MODIFIER_MOTION_NONE)


function extended_obsidian_destroyer_essence_aura:GetIntrinsicModifierName()
    return "modifier_extended_essence_aura"
end

-- Aura modifier
modifier_extended_essence_aura = class({})

function modifier_extended_essence_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_essence_aura:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_essence_aura:OnRefresh()
    self:OnCreated()
end

function modifier_extended_essence_aura:GetAuraRadius()
    return self.radius
end

function modifier_extended_essence_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
end

function modifier_extended_essence_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_essence_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO
end

function modifier_extended_essence_aura:GetModifierAura()
    return "modifier_extended_essence_aura_buff"
end

function modifier_extended_essence_aura:IsAura()
    if IsServer() then
...
function modifier_extended_essence_aura:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_MANA_BONUS}

    return decFunc
end

function modifier_extended_essence_aura:GetModifierManaBonus()
    local int = self:GetStackCount()
    return self.base_bonus_mana + (int * self.int_bonus_mana_mult)
end

function modifier_extended_essence_aura:AllowIllusionDuplicate()
    return true
end

function modifier_extended_essence_aura:IsDebuff() return false end
function modifier_extended_essence_aura:IsHidden() return true end
function modifier_extended_essence_aura:IsPurgable() return false end    


modifier_extended_essence_aura_buff = class({})

function modifier_extended_essence_aura_buff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    ...
    self.modifier_proc = "modifier_extended_essence_aura_proc"
    self.modifier_overmana = "modifier_extended_essence_aura_over_maximum"
...
function modifier_extended_essence_aura_buff:DeclareFunctions()
    local decFunc = {MODIFIER_EVENT_ON_ABILITY_FULLY_CAST}

    return decFunc
end

function modifier_extended_essence_aura_buff:OnAbilityFullyCast(keys)
    if IsServer() then
...
function modifier_extended_essence_aura_buff:ProcEssenceAura()
    if IsServer() then
        ...
        if RollPseudoRandom(self.restore_chance_pct, self) then            
            ...
            if self.caster:HasTalent("special_bonus_extended_obsidian_destroyer_8") and self.caster == self.parent then
                -- Get current mana
                ...
                if excess_mana > 0 then
                    if not self.caster:HasModifier(self.modifier_overmana) then
                        local overmana_duration = self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_8")
            ...
            if self.caster:HasTalent("special_bonus_extended_obsidian_destroyer_5") then
...
function modifier_extended_essence_aura_buff:OnRefresh()
    self:OnCreated()
end

-- Caster's aura proc modifier
modifier_extended_essence_aura_proc = class({})

function modifier_extended_essence_aura_proc:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_essence_aura_proc:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_essence_aura_proc:OnRefresh()
    if IsServer() then
        -- Insert new stack values
        table.insert(self.stacks_table, GameRules:GetGameTime())
    end
...
function modifier_extended_essence_aura_proc:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_STATS_INTELLECT_BONUS}

    return decFunc
end 

function modifier_extended_essence_aura_proc:GetModifierBonusStats_Intellect()
    local stacks = self:GetStackCount()
    return stacks
end

function modifier_extended_essence_aura_proc:IsHidden() return false end    
function modifier_extended_essence_aura_proc:IsPurgable() return false end
function modifier_extended_essence_aura_proc:IsDebuff() return false end


-- #8 Talent: Essence Aura can go beyond maximum mana temporarily
modifier_extended_essence_aura_over_maximum = class({})

function modifier_extended_essence_aura_over_maximum:IsHidden() return true end    
function modifier_extended_essence_aura_over_maximum:IsPurgable() return false end
function modifier_extended_essence_aura_over_maximum:IsDebuff() return false end

function modifier_extended_essence_aura_over_maximum:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        -- Overmana duration
        self.overmana_duration = self.caster:FindTalentValue("special_bonus_extended_obsidian_destroyer_8")
...
function modifier_extended_essence_aura_over_maximum:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_essence_aura_over_maximum:OnRefresh()
    if IsServer() then
        -- Insert new stack values
        table.insert(self.stacks_table, GameRules:GetGameTime())
    end
...
function modifier_extended_essence_aura_over_maximum:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_EXTRA_MANA_BONUS}

    return decFunc
end

function modifier_extended_essence_aura_over_maximum:GetModifierExtraManaBonus()
    local stacks = self:GetStackCount()
    return stacks
end
...
--    SANITY ECLIPSE     --
---------------------------
extended_obsidian_destroyer_sanity_eclipse = class({})

function extended_obsidian_destroyer_sanity_eclipse:IsHiddenWhenStolen()
    return false
end

function extended_obsidian_destroyer_sanity_eclipse:GetAOERadius()
    local ability = self
    local radius = ability:GetSpecialValueFor("radius")
    return radius
end

function extended_obsidian_destroyer_sanity_eclipse:GetAssociatedSecondaryAbilities()
    return "extended_obsidian_destroyer_astral_imprisonment"
end

function extended_obsidian_destroyer_sanity_eclipse:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
    ...
    local prison_ability = caster:FindAbilityByName("extended_obsidian_destroyer_astral_imprisonment")        
    ...
    for _,enemy in pairs(enemies) do
        ...
        if caster:HasTalent("special_bonus_extended_obsidian_destroyer_7") or not enemy:IsMagicImmune() then            
            ...
            if enemy:IsIllusion() then
                ...
                if damage > 0 then
                    ...
                    if caster:HasTalent("special_bonus_extended_obsidian_destroyer_7") then
                        damage_type = DAMAGE_TYPE_PURE
                    end
