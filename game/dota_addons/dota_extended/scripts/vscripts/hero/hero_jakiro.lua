...
function modifier_extended_fire_breath_debuff:_UpdateDebuffLevelValues()
	if IsServer() then
		...
		local damage = ability:GetSpecialValueFor("damage") + caster:FindSpecificTalentValue("special_bonus_extended_jakiro_2", "fire_damage_increase")
		self.tick_damage = damage * self.damage_interval
	end
end
...
function modifier_extended_ice_breath_debuff:_SubClassOnCreated()
...
LinkLuaModifier("modifier_extended_liquid_fire_animate", "hero/hero_jakiro", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_liquid_fire_debuff", "hero/hero_jakiro", LUA_MODIFIER_MOTION_NONE)

function extended_jakiro_liquid_fire:IsNetherWardStealable() return false end
...
function extended_jakiro_liquid_fire:OnAbilityPhaseStart()
	local caster = self:GetCaster()
	caster:StartGesture(ACT_DOTA_ATTACK)

	-- Special animation for jakiro
	if caster:GetUnitName() == "npc_dota_hero_jakiro" then
		caster:AddNewModifier(caster, self.ability, "modifier_extended_liquid_fire_animate", {})
	end
...
function extended_jakiro_liquid_fire:OnAbilityPhaseInterrupted()
	local caster = self:GetCaster()
	caster:RemoveModifierByNameAndCaster("modifier_extended_liquid_fire_animate", caster)
end
...
function modifier_extended_liquid_fire_caster:_IsLiquidFireProjectile()
	local caster = self.caster
	return caster:GetRangedProjectileName() == "particles/units/heroes/hero_jakiro/jakiro_base_attack_fire.vpcf"
end

function modifier_extended_liquid_fire_caster:OnAttackStart(keys)
	if IsServer() then
		local caster = self.caster
		local ability = self.ability
		...
		if caster == attacker then
			...
			elseif self:_IsLiquidFireProjectile() then
				-- Revert projectile
				ChangeAttackProjectileExtended(caster)
			end
		end
	end
end
...
modifier_extended_liquid_fire_animate = class({
	IsHidden					    = function(self) return true end,
	IsPurgable						= function(self) return false end,
	IsDebuff						= function(self) return false end,
	RemoveOnDeath					= function(self) return true end,
...
function modifier_extended_liquid_fire_animate:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_TRANSLATE_ACTIVITY_MODIFIERS,
		MODIFIER_EVENT_ON_ATTACK
	}
...
function modifier_extended_liquid_fire_animate:OnAttack(keys)
	if IsServer() then
		local attacker = keys.attacker
...
function modifier_extended_liquid_fire_debuff:GetModifierAttackSpeedBonus_Constant() return self.attack_slow * (-1) end
function modifier_extended_liquid_fire_debuff:GetModifierTurnRate_Percentage() return self.turn_slow * (-1) end
...
function modifier_extended_macropyre_debuff:_SubClassOnCreated()
