...
--      FATAL BONDS        --
-----------------------------
extended_warlock_fatal_bonds = class({})
LinkLuaModifier("modifier_extended_fatal_bonds", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_fatal_bonds:IsHiddenWhenStolen()
    return false
end

function extended_warlock_fatal_bonds:OnSpellStart()
    -- Ability properties
    ...
    local modifier_bonds = "modifier_extended_fatal_bonds"    
...
modifier_extended_fatal_bonds = class({})

function modifier_extended_fatal_bonds:OnCreated()
    -- Ability properties
    ...
    self.modifier_bonds = "modifier_extended_fatal_bonds"
    self.modifier_word = "modifier_extended_shadow_word"
    self.ability_word = "extended_warlock_shadow_word"
    ...
    self.link_damage_share_pct = self.link_damage_share_pct + self.caster:FindTalentValue("special_bonus_extended_warlock_3")

    -- #7 Talent: Golems share damage they take to Fatal Bonded units, no range limit
    self.golem_link_radius = self.golem_link_radius + self.caster:FindTalentValue("special_bonus_extended_warlock_7")
...
function modifier_extended_fatal_bonds:IsHidden() return false end
function modifier_extended_fatal_bonds:IsPurgable() return true end
function modifier_extended_fatal_bonds:IsDebuff() return true end

function modifier_extended_fatal_bonds:OnIntervalThink()
...
function modifier_extended_fatal_bonds:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_TAKEDAMAGE}

    return decFuncs
end

function modifier_extended_fatal_bonds:OnTakeDamage(keys)
    if IsServer() then
        ...
        if string.find(unit:GetUnitName(), "npc_extended_warlock_golem") and unit:GetTeamNumber() ~= self.parent:GetTeamNumber() then            
...
--      SHADOW WORD        --
-----------------------------
extended_warlock_shadow_word = class({})
LinkLuaModifier("modifier_extended_shadow_word", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_shadow_word:IsHiddenWhenStolen()
    return false
end

function extended_warlock_shadow_word:GetAOERadius()
    local ability = self
...
function extended_warlock_shadow_word:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
    local sound_target = "Hero_Warlock.ShadowWord"    
    local sound_explosion = "Extended.WarlockShadowWordExplosion"
    local particle_aoe = "particles/hero/warlock/warlock_shadow_word_aoe.vpcf"
    local modifier_word = "modifier_extended_shadow_word"
    ...
    duration = duration + caster:FindTalentValue("special_bonus_extended_warlock_5")
...
modifier_extended_shadow_word = class({})

function modifier_extended_shadow_word:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()    
    self.parent = self:GetParent()
    ...
    self.golem_bonus_as = self.golem_bonus_as + self.caster:FindTalentValue("special_bonus_extended_warlock_1")
    ...
    if self.good_guy and string.find(self.parent:GetUnitName(), "npc_extended_warlock_golem") then
        self.is_golem = true
    end
...
function modifier_extended_shadow_word:IsHidden() return false end
function modifier_extended_shadow_word:IsPurgable() return true end

function modifier_extended_shadow_word:IsDebuff() 
    if self.good_guy then
        return false
    end
...
function modifier_extended_shadow_word:OnIntervalThink()
...
function modifier_extended_shadow_word:OnDestroy()
...
function modifier_extended_shadow_word:DeclareFunctions()
...
function modifier_extended_shadow_word:GetModifierMoveSpeedBonus_Percentage()
...
function modifier_extended_shadow_word:GetModifierAttackSpeedBonus_Constant()
    if not self.is_golem then
        return nil
    end
...
extended_warlock_upheaval = class({})
LinkLuaModifier("modifier_extended_upheaval", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_upheaval_debuff", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_upheaval_buff", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_upheaval:IsHiddenWhenStolen()
    return false
end

function extended_warlock_upheaval:GetChannelTime()
    local caster = self:GetCaster()
    if caster:HasTalent("special_bonus_extended_warlock_6") then
        return 0
    end
...
function extended_warlock_upheaval:GetAOERadius()
    local ability = self
    local radius = ability:GetSpecialValueFor("radius")
    return radius
end

function extended_warlock_upheaval:OnSpellStart()    
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()    
    ...
    local modifier_upheaval = "modifier_extended_upheaval"    

    if not caster:HasTalent("special_bonus_extended_warlock_6") then
        -- Play cast response
        ...
        local demon = CreateUnitByName("npc_extended_warlock_upheaval_demon", target_point, true, caster, caster, caster:GetTeamNumber())
        ...
        Timers:CreateTimer(FrameTime(), function()
            ...
            local ability_demon = demon:FindAbilityByName("extended_warlock_upheaval")
...
function extended_warlock_upheaval:OnChannelFinish()
    local caster = self:GetCaster()
    local sound_loop = "Hero_Warlock.Upheaval"    
    ...
    if string.find(caster:GetUnitName(), "npc_extended_warlock_upheaval_demon") then
...
-- Upheaval modifier
modifier_extended_upheaval = class({})

function modifier_extended_upheaval:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.particle_upheaval = "particles/units/heroes/hero_warlock/warlock_upheaval.vpcf"    
    self.modifier_debuff = "modifier_extended_upheaval_debuff"
    self.modifier_golem_buff = "modifier_extended_upheaval_buff"
...
function modifier_extended_upheaval:OnIntervalThink()
    ...
    for _,unit in pairs(units) do
        if string.find(unit:GetUnitName(), "npc_extended_warlock_golem") then
...
modifier_extended_upheaval_debuff = class({})

function modifier_extended_upheaval_debuff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self    
    self.parent = self:GetParent()
...
function modifier_extended_upheaval_debuff:IsHidden() return false end
function modifier_extended_upheaval_debuff:IsPurgable() return true end
function modifier_extended_upheaval_debuff:IsDebuff() return true end

function modifier_extended_upheaval_debuff:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_upheaval_debuff:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
                      MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT}
...
function modifier_extended_upheaval_debuff:GetModifierMoveSpeedBonus_Percentage()
    local stacks = self:GetStackCount()
    return stacks * (-1)
end

function modifier_extended_upheaval_debuff:GetModifierAttackSpeedBonus_Constant()
    -- #2 Talent: Upheaval also reduces attack speed
    if self.caster:HasTalent("special_bonus_extended_warlock_2") then
        local stacks = self:GetStackCount()
        return stacks * (-1)        
    end
...
-- Golem Upheaval buff
modifier_extended_upheaval_buff = class({})

function modifier_extended_upheaval_buff:IsHidden() return false end
function modifier_extended_upheaval_buff:IsPurgable() return false end
function modifier_extended_upheaval_buff:IsDebuff() return false end
...
--    CHAOTIC OFFERING     --
-----------------------------
extended_warlock_rain_of_chaos = class({})
LinkLuaModifier("modifier_extended_rain_of_chaos_stun", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_rain_of_chaos_golem_as", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_rain_of_chaos_golem_ms", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_rain_of_chaos_demon_link", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_rain_of_chaos:IsHiddenWhenStolen()
    return false
end

function extended_warlock_rain_of_chaos:IsNetherWardStealable()
    return false
end

function extended_warlock_rain_of_chaos:GetAOERadius()
    local ability = self
    local radius = ability:GetSpecialValueFor("radius")
    return radius
end

function extended_warlock_rain_of_chaos:OnAbilityPhaseStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function extended_warlock_rain_of_chaos:OnAbilityPhaseInterrupted()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local sound_precast = "Hero_Warlock.RainOfChaos_buildup"
...
function extended_warlock_rain_of_chaos:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
    ...
    local modifier_stun = "modifier_extended_rain_of_chaos_stun"
    local modifier_attack_speed = "modifier_extended_rain_of_chaos_golem_as"
    local modifier_move_speed = "modifier_extended_rain_of_chaos_golem_ms"
    local modifier_demon_link = "modifier_extended_rain_of_chaos_demon_link"
    local ability_fists = "extended_warlock_flaming_fists"
    local ability_immolate = "extended_warlock_permanent_immolation"
    ...
    Timers:CreateTimer(effect_delay, function()
        ...
        local golem = CreateUnitByName("npc_extended_warlock_golem_"..golem_level, target_point, true, caster, caster, caster:GetTeamNumber())
        ...
        if caster:HasTalent("special_bonus_extended_warlock_4") then
            golem:SetPhysicalArmorBaseValue(golem:GetPhysicalArmorValue() + caster:FindTalentValue("special_bonus_extended_warlock_4"))
        end
        ...
        if caster:HasTalent("special_bonus_extended_warlock_8") then
            ability_spell_immunity = golem:AddAbility("extended_warlock_golem_spell_immunity")
        ...
        if scepter then
            ...
            for i = 0, (scepter_demon_count -1) do
                ...
                -- Create demon
                local demon = CreateUnitByName("npc_extended_warlock_demonic_ascension", demon_spawn_point, true, caster, caster, caster:GetTeamNumber())
                demon:SetControllableByPlayer(playerID, true)
                demon:StartGesture(ACT_DOTA_IDLE)       
...
-- Stun modifier
modifier_extended_rain_of_chaos_stun = class({})

function modifier_extended_rain_of_chaos_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_rain_of_chaos_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_rain_of_chaos_stun:GetEffectAttachType()
    return PATTACH_OVERHEAD_FOLLOW
end

function modifier_extended_rain_of_chaos_stun:IsHidden() return false end
function modifier_extended_rain_of_chaos_stun:IsPurgeException() return true end
function modifier_extended_rain_of_chaos_stun:IsStunDebuff() return true end

-- Golem attack speed bonus modifier
modifier_extended_rain_of_chaos_golem_as = class({})

function modifier_extended_rain_of_chaos_golem_as:IsHidden() return true end
function modifier_extended_rain_of_chaos_golem_as:IsPurgable() return false end
function modifier_extended_rain_of_chaos_golem_as:IsDebuff() return false end

function modifier_extended_rain_of_chaos_golem_as:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT}

    return decFuncs
end 

function modifier_extended_rain_of_chaos_golem_as:GetModifierAttackSpeedBonus_Constant()
    local stacks = self:GetStackCount()
    return stacks
end

-- Golem move speed bonus modifier
modifier_extended_rain_of_chaos_golem_ms = class({})

function modifier_extended_rain_of_chaos_golem_ms:IsHidden() return true end
function modifier_extended_rain_of_chaos_golem_ms:IsPurgable() return false end
function modifier_extended_rain_of_chaos_golem_ms:IsDebuff() return false end

function modifier_extended_rain_of_chaos_golem_ms:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT}

    return decFuncs
end 

function modifier_extended_rain_of_chaos_golem_ms:GetModifierMoveSpeedBonus_Constant()
    local stacks = self:GetStackCount()
    return stacks
end
...
modifier_extended_rain_of_chaos_demon_link = class({})

function modifier_extended_rain_of_chaos_demon_link:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_rain_of_chaos_demon_link:OnIntervalThink()
...
function modifier_extended_rain_of_chaos_demon_link:DeclareFunctions()
...
function modifier_extended_rain_of_chaos_demon_link:GetModifierIncomingDamage_Percentage()
    return -100
end

function modifier_extended_rain_of_chaos_demon_link:GetModifierBaseDamageOutgoing_Percentage()
    local stacks = self:GetStackCount()
    return stacks * self.scepter_damage_per_demon_pct
end

function modifier_extended_rain_of_chaos_demon_link:OnTakeDamage(keys)
    if IsServer() then        
        local unit = keys.unit
...
function modifier_extended_rain_of_chaos_demon_link:OnDeath(keys)
    if IsServer() then
        local unit = keys.unit
...
function modifier_extended_rain_of_chaos_demon_link:OnRemoved()
...
--      FLAMING FISTS      --
-----------------------------
extended_warlock_flaming_fists = class({})
LinkLuaModifier("modifier_extended_flaming_fists", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_flaming_fists:GetIntrinsicModifierName()
    return "modifier_extended_flaming_fists"
end

-- Flaming fists modifier
modifier_extended_flaming_fists = class({})

function modifier_extended_flaming_fists:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.modifier_upheaval = "modifier_extended_upheaval_buff"
...
function modifier_extended_flaming_fists:OnRefresh()
    self:OnCreated()
end

function modifier_extended_flaming_fists:DeclareFunctions()
...
function modifier_extended_flaming_fists:GetModifierProcAttack_BonusDamage_Pure()
    return self.damage
end

function modifier_extended_flaming_fists:OnAttackLanded(keys)
    if IsServer() then
...
function modifier_extended_flaming_fists:IsHidden() return true end
function modifier_extended_flaming_fists:IsPurgable() return false end
function modifier_extended_flaming_fists:IsDebuff() return false end
...
-----------------------------
extended_warlock_permanent_immolation = class({})
LinkLuaModifier("modifier_extended_permanent_immolation_aura", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_permanent_immolation_debuff", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_permanent_immolation:GetIntrinsicModifierName()
    return "modifier_extended_permanent_immolation_aura"
end

-- Permanent immolation aura modifier
modifier_extended_permanent_immolation_aura = class({})

function modifier_extended_permanent_immolation_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.modifier_burn = "modifier_extended_permanent_immolation_debuff"
    self.modifier_upheaval = "modifier_extended_upheaval_buff"
    self.modifier_upheaval_debuff = "modifier_extended_upheaval_debuff"
    self.particle_burn = "particles/hero/warlock/warlock_upheaval_golem_burn.vpcf"
...
function modifier_extended_permanent_immolation_aura:OnIntervalThink()
    if IsServer() then
        if self.caster:HasModifier(self.modifier_upheaval) then
            local modifier_upheaval_handler = self.caster:FindModifierByName(self.modifier_upheaval)
            if modifier_upheaval_handler and modifier_upheaval_handler.radius and modifier_upheaval_handler.center then
...
function modifier_extended_permanent_immolation_aura:GetAuraRadius()    
    return self.actual_radius
end

function modifier_extended_permanent_immolation_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_permanent_immolation_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_permanent_immolation_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_permanent_immolation_aura:GetAuraEntityReject(target)
...
function modifier_extended_permanent_immolation_aura:GetModifierAura()
    return "modifier_extended_permanent_immolation_debuff"
end

function modifier_extended_permanent_immolation_aura:IsAura()
...
function modifier_extended_permanent_immolation_aura:IsHidden() return true end
function modifier_extended_permanent_immolation_aura:IsPurgable() return false end
function modifier_extended_permanent_immolation_aura:IsDebuff() return false end

-- Permanent immolation debuff modifier
modifier_extended_permanent_immolation_debuff = class({})

function modifier_extended_permanent_immolation_debuff:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_extended_permanent_immolation_debuff:IsHidden() return false end
function modifier_extended_permanent_immolation_debuff:IsPurgable() return false end
function modifier_extended_permanent_immolation_debuff:IsDebuff() return true end
...
--     SPELL IMMUNITY      --
-----------------------------
extended_warlock_golem_spell_immunity = class({})
LinkLuaModifier("modifier_extended_golem_spell_immunity", "hero/hero_warlock.lua", LUA_MODIFIER_MOTION_NONE)

function extended_warlock_golem_spell_immunity:GetIntrinsicModifierName()
    return "modifier_extended_golem_spell_immunity"
end

-- Spell immunity modifier
modifier_extended_golem_spell_immunity = class({})

function modifier_extended_golem_spell_immunity:CheckState()
    local state = {[MODIFIER_STATE_MAGIC_IMMUNE] = true}
    return state
end

function modifier_extended_golem_spell_immunity:IsHidden() return false end
function modifier_extended_golem_spell_immunity:IsPurgable() return false end
function modifier_extended_golem_spell_immunity:IsDebuff() return false end
