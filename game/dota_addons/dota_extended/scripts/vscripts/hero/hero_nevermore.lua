...
--     SHADOW RAZE (CLOSE)        --
------------------------------------
extended_nevermore_shadowraze_close = extended_nevermore_shadowraze_close or class({})
...
function extended_nevermore_shadowraze_close:IsHiddenWhenStolen()
    return false
end

function extended_nevermore_shadowraze_close:GetManaCost(level)
    local caster = self:GetCaster()
    local manacost = self.BaseClass.GetManaCost(self, level)

    -- #1 Talent: Shadowraze mana cost reduction
    manacost = manacost - caster:FindTalentValue("special_bonus_extended_nevermore_1")

    return manacost
end

function extended_nevermore_shadowraze_close:GetCooldown(level)
    local caster = self:GetCaster()
    local modifier_harvest = "modifier_extended_reqiuem_harvest"
    ...
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_nevermore_3")

    return cooldown
end

function extended_nevermore_shadowraze_close:OnUpgrade()
    local caster = self:GetCaster()
    UpgradeShadowRazes(caster, self)
end

function extended_nevermore_shadowraze_close:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
--     SHADOW RAZE (MEDIUM)       --
------------------------------------
extended_nevermore_shadowraze_medium = extended_nevermore_shadowraze_medium or class({})

function extended_nevermore_shadowraze_medium:IsHiddenWhenStolen()
    return false
end

function extended_nevermore_shadowraze_medium:GetManaCost(level)
    local caster = self:GetCaster()
    local manacost = self.BaseClass.GetManaCost(self, level)
...
function extended_nevermore_shadowraze_medium:GetCooldown(level)
    local caster = self:GetCaster()
    local modifier_harvest = "modifier_extended_reqiuem_harvest"
...
function extended_nevermore_shadowraze_medium:OnUpgrade()
    local caster = self:GetCaster()
    UpgradeShadowRazes(caster, self)
end

function extended_nevermore_shadowraze_medium:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
--       SHADOW RAZE (FAR)        --
------------------------------------
extended_nevermore_shadowraze_far = extended_nevermore_shadowraze_far or class({})

function extended_nevermore_shadowraze_far:IsHiddenWhenStolen()
    return false
end

function extended_nevermore_shadowraze_far:GetManaCost(level)
    local caster = self:GetCaster()
    local manacost = self.BaseClass.GetManaCost(self, level)
...
function extended_nevermore_shadowraze_far:GetCooldown(level)
    local caster = self:GetCaster()
    local modifier_harvest = "modifier_extended_reqiuem_harvest"
...
function extended_nevermore_shadowraze_far:OnUpgrade()
    local caster = self:GetCaster()
    UpgradeShadowRazes(caster, self)
end

function extended_nevermore_shadowraze_far:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function ApplyShadowRazeDamage(caster, ability, enemy)
    ...
    local modifier_combo = "modifier_shadow_raze_combo"
    local modifier_souls = "modifier_extended_necromastery_souls"
    local modifier_dark_lord = "modifier_extended_dark_lord_debuff"
    ...
    if caster:HasModifier(modifier_souls) then
        ...
        damage_per_soul = damage_per_soul + caster:FindTalentValue("special_bonus_extended_nevermore_8")
...
function UpgradeShadowRazes(caster, ability)
    local raze_close = "extended_nevermore_shadowraze_close"
    local raze_medium = "extended_nevermore_shadowraze_medium"
    local raze_far = "extended_nevermore_shadowraze_far"
...
function modifier_shadow_raze_combo:OnCreated()
    if IsServer() then
        ...
        self.razes = {}
        self.razes[1] = "extended_nevermore_shadowraze_close"
        self.razes[2] = "extended_nevermore_shadowraze_medium"
        self.razes[3] = "extended_nevermore_shadowraze_far"
...
--           NECROMASTERY         --
------------------------------------
extended_nevermore_necromastery = extended_nevermore_necromastery or class({})
LinkLuaModifier("modifier_extended_necromastery_souls", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)

function extended_nevermore_necromastery:GetIntrinsicModifierName()
    return "modifier_extended_necromastery_souls"
end

function extended_nevermore_necromastery:IsHiddenWhenStolen()
    return true
end

function extended_nevermore_necromastery:OnUnStolen()
    local caster = self:GetCaster()
    local modifier_souls = "modifier_extended_necromastery_souls"
...
function extended_nevermore_necromastery:OnUpgrade()
    local caster = self:GetCaster()
...
modifier_extended_necromastery_souls = modifier_extended_necromastery_souls or class({})

function modifier_extended_necromastery_souls:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    ...
    self.requiem_ability = "extended_nevermore_requiem"
...
function modifier_extended_necromastery_souls:GetHeroEffectName()
    return "particles/units/heroes/hero_nevermore/nevermore_souls_hero_effect.vpcf"
end

function modifier_extended_necromastery_souls:OnIntervalThink()
    if IsServer() then
        -- #6 Talent: Maximum Necromastery soul increase
        if self.caster:HasTalent("special_bonus_extended_nevermore_6") and not self.talent6 then
            self.talent6 = true
            self.max_souls = self.max_souls + self.caster:FindTalentValue("special_bonus_extended_nevermore_6")
...
function modifier_extended_necromastery_souls:OnRefresh()
    -- Refresh the talent variable
    self.talent6 = false
    self:OnCreated()
end

function modifier_extended_necromastery_souls:OnStackCountChanged(old_stack_count)
    if IsServer() then
...
function modifier_extended_necromastery_souls:DestroyOnExpire()
    return false
end

function modifier_extended_necromastery_souls:DeclareFunctions()
...
function modifier_extended_necromastery_souls:GetModifierPreAttack_BonusDamage()
    local stacks = self:GetStackCount()

    -- #8 Talent: Necromastery soul grant additional damage
    local damage_per_soul = self.damage_per_soul + self.caster:FindTalentValue("special_bonus_extended_nevermore_8")

    return damage_per_soul * stacks
end

function modifier_extended_necromastery_souls:OnAttackLanded(keys)
    if IsServer() then
        ...
        if self.caster == attacker then
            ...
            if self.caster:HasTalent("special_bonus_extended_nevermore_4") then
                if target:GetTeamNumber() ~= self.caster:GetTeamNumber() and target:IsCreep() or target:IsHero() then
                    ...
                    if self.caster:IsRealHero() then
                        ...
                        local stacks = self:GetStackCount()
                        local lifesteal_per_soul = self.caster:FindTalentValue("special_bonus_extended_nevermore_4")
...
function modifier_extended_necromastery_souls:OnDeath(keys)
    if IsServer() then
        ...
        if string.find(self.caster:GetUnitName(), "npc_extended_pugna_nether_ward") then
            return nil
        end
        ...
        if self.caster == target then
            ...
            if not self.caster:HasTalent("special_bonus_extended_nevermore_7") then
...
function modifier_extended_necromastery_souls:IsPermanent() return true end
function modifier_extended_necromastery_souls:IsHidden() return false end
function modifier_extended_necromastery_souls:IsPurgable() return false end
function modifier_extended_necromastery_souls:IsDebuff() return false end
function modifier_extended_necromastery_souls:AllowIllusionDuplicate() return true end
...
------------------------------------
extended_nevermore_dark_lord = extended_nevermore_dark_lord or class({})
LinkLuaModifier("modifier_extended_dark_lord_aura", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_dark_lord_debuff", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)

function extended_nevermore_dark_lord:GetIntrinsicModifierName()
    return "modifier_extended_dark_lord_aura"
end

function extended_nevermore_dark_lord:OnUpgrade()
    local caster = self:GetCaster()
...
modifier_extended_dark_lord_aura = modifier_extended_dark_lord_aura or class({})

function modifier_extended_dark_lord_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.particle_soul_hero = "particles/hero/nevermore/nevermore_soul_projectile.vpcf"
    self.modifier_souls = "modifier_extended_necromastery_souls"
...
function modifier_extended_dark_lord_aura:OnIntervalThink()
    if IsServer() then
        -- #5 Talent: Presence of the Lord gain a soul from nearby enemies
        if self.caster:HasTalent("special_bonus_extended_nevermore_5") and self.caster:HasModifier(self.modifier_souls) then
            local soul_drain_count = self.caster:FindTalentValue("special_bonus_extended_nevermore_5")
...
function modifier_extended_dark_lord_aura:IsHidden() return true end
function modifier_extended_dark_lord_aura:IsPurgable() return false end
function modifier_extended_dark_lord_aura:IsDebuff() return false end

function modifier_extended_dark_lord_aura:OnRefresh()
    self:OnCreated()
end

function modifier_extended_dark_lord_aura:AllowIllusionDuplicate()
    return true
end

function modifier_extended_dark_lord_aura:GetAuraEntityReject(target)
...
function modifier_extended_dark_lord_aura:GetAuraRadius()
    return self.aura_radius
end

function modifier_extended_dark_lord_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
end

function modifier_extended_dark_lord_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_dark_lord_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_dark_lord_aura:GetModifierAura()
    return "modifier_extended_dark_lord_debuff"
end

function modifier_extended_dark_lord_aura:IsAura()
...
modifier_extended_dark_lord_debuff = modifier_extended_dark_lord_debuff or class({})

function modifier_extended_dark_lord_debuff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_dark_lord_debuff:OnIntervalThink()
    if IsServer() then
        local stacks = self:GetStackCount()
...
function modifier_extended_dark_lord_debuff:OnStackCountChanged()
    if IsServer() then
...
function modifier_extended_dark_lord_debuff:IsHidden() return false end
function modifier_extended_dark_lord_debuff:IsPurgable() return false end
function modifier_extended_dark_lord_debuff:IsDebuff() return true end

function modifier_extended_dark_lord_debuff:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS}

    return decFuncs
end

function modifier_extended_dark_lord_debuff:GetModifierPhysicalArmorBonus()
    local stacks = self:GetStackCount()

    -- #2 Talent: Presence of the Dark Lord Armor reduction increase
    local armor_reduction = self.armor_reduction + self.caster:FindSpecificTalentValue("special_bonus_extended_nevermore_2", "armor_reduction")
    local raze_armor_reduction = self.raze_armor_reduction + self.caster:FindSpecificTalentValue("special_bonus_extended_nevermore_2", "raze_armor_reduction")
...
--       REQUIEM OF SOULS         --
------------------------------------
extended_nevermore_requiem = extended_nevermore_requiem or class({})
LinkLuaModifier("modifier_extended_reqiuem_phase_buff", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_reqiuem_debuff", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_reqiuem_harvest", "hero/hero_nevermore.lua", LUA_MODIFIER_MOTION_NONE)

function extended_nevermore_requiem:IsHiddenWhenStolen()
    return false
end

function extended_nevermore_requiem:GetAssociatedSecondaryAbilities()
    return "extended_nevermore_necromastery"
end

function extended_nevermore_requiem:OnAbilityPhaseStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local sound_pre_cast = "Hero_Nevermore.RequiemOfSoulsCast"
    local cast_animation = ACT_DOTA_CAST_ABILITY_6
    local modifier_phase = "modifier_extended_reqiuem_phase_buff"
...
function extended_nevermore_requiem:OnAbilityPhaseInterrupted()
    local caster = self:GetCaster()
    local cast_animation = ACT_DOTA_CAST_ABILITY_6
    local modifier_phase = "modifier_extended_reqiuem_phase_buff"
...
function extended_nevermore_requiem:OnSpellStart(death_cast)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    -- #6 Talent: Maximum Necromastery soul increase
    max_souls = max_souls + caster:FindTalentValue("special_bonus_extended_nevermore_6")

    local line_count
    -- Determind if it was a death cast (clears and ignores temporary souls)
    if death_cast and not caster:HasTalent("special_bonus_extended_nevermore_7") then
...
function extended_nevermore_requiem:OnProjectileHit_ExtraData(target, location, extra_data)
    ...
    local caster = self:GetCaster()
    local ability = self
    local modifier_debuff = "modifier_extended_reqiuem_debuff"
    local modifier_harvest = "modifier_extended_reqiuem_harvest"
...
modifier_extended_reqiuem_phase_buff = modifier_extended_reqiuem_phase_buff or class({})

function modifier_extended_reqiuem_phase_buff:CheckState()
    local state = {[MODIFIER_STATE_NO_UNIT_COLLISION] = true}
    return state
end

function modifier_extended_reqiuem_phase_buff:IsHidden() return true end
function modifier_extended_reqiuem_phase_buff:IsPurgable() return false end
function modifier_extended_reqiuem_phase_buff:IsDebuff() return false end



-- Requiem of Souls slow debuff
modifier_extended_reqiuem_debuff = modifier_extended_reqiuem_debuff or class({})

function modifier_extended_reqiuem_debuff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_reqiuem_debuff:IsHidden() return false end
function modifier_extended_reqiuem_debuff:IsPurgable() return true end
function modifier_extended_reqiuem_debuff:IsDebuff() return true end

function modifier_extended_reqiuem_debuff:DeclareFunctions()
...
function modifier_extended_reqiuem_debuff:GetModifierBaseDamageOutgoing_Percentage()
    return self.damage_reduction_pct * (-1)
end

function modifier_extended_reqiuem_debuff:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end


-- Requiem of Souls Soul Harvest modifier
modifier_extended_reqiuem_harvest = modifier_extended_reqiuem_harvest or class({})

function modifier_extended_reqiuem_harvest:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_reqiuem_harvest:IsHidden() return false end
function modifier_extended_reqiuem_harvest:IsPurgable() return false end
function modifier_extended_reqiuem_harvest:IsDebuff() return false end