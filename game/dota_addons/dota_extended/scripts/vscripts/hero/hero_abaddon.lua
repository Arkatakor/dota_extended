...
function getCurseOfAvernusDummyReference()
	if IsServer() and _curse_of_avernus_reference == nil then
		...
		_curse_of_avernus_reference = abaddon_dummy:AddAbility("extended_abaddon_curse_of_avernus")
...
function getOverChannelIncrease(caster)
	if caster:HasModifier("modifer_over_channel_handler") then
		local over_channel = caster:FindAbilityByName("extended_abaddon_over_channel")
		if over_channel then
			local ability_level = over_channel:GetLevel() - 1
			-- #6 Talent: +50% Overchannel power
			return over_channel:GetLevelSpecialValueFor( "extra_dmg" , ability_level ) * (1 + caster:FindTalentValue("special_bonus_extended_abaddon_6"))
		end
	end
...
extended_abaddon_mist_coil = extended_abaddon_mist_coil or class({})

function extended_abaddon_mist_coil:OnSpellStart()
	if IsServer() then
...
function extended_abaddon_mist_coil:GetCooldown()
	-- #3 Talent: -1.5sec cooldown on Mist Coil
	return self:GetSpecialValueFor("cooldown") + self:GetCaster():FindTalentValue("special_bonus_extended_abaddon_3")
end

function extended_abaddon_mist_coil:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		local caster = self:GetCaster()
        ...
        if target:GetTeam() ~= caster:GetTeam() then
			...
			local curse_of_avernus = caster:FindAbilityByName("extended_abaddon_curse_of_avernus")
			...
			if debuff_duration > 0 then
				target:AddNewModifier(caster, curse_of_avernus, "modifier_extended_curse_of_avernus_debuff_slow", { duration = debuff_duration })
			end
...
MergeTables(LinkedModifiers,{
	["modifier_extended_aphotic_shield_buff_block"] = LUA_MODIFIER_MOTION_NONE,
})
extended_abaddon_aphotic_shield = extended_abaddon_aphotic_shield or class({})

function extended_abaddon_aphotic_shield:GetCastRange(Location, Target)
	-- #1 Talent: +150 Aphotic Shield cast range
	return self:GetSpecialValueFor("cast_range") + self:GetCaster():FindTalentValue("special_bonus_extended_abaddon_1")
end

function extended_abaddon_aphotic_shield:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local target = self:GetCursorTarget()
		...
		local modifier_name_aphotic_shield = "modifier_extended_aphotic_shield_buff_block"
		...
		local duration = self:GetSpecialValueFor("duration") + caster:FindTalentValue("special_bonus_extended_abaddon_2")
		-- Add new modifier
		target:AddNewModifier(caster, self, modifier_name_aphotic_shield, { duration = duration })
	end
end

modifier_extended_aphotic_shield_buff_block = modifier_extended_aphotic_shield_buff_block or class({
	IsHidden				= function(self) return false end,
	IsPurgable	  			= function(self) return true end,
	IsDebuff	  			= function(self) return false end
})

function modifier_extended_aphotic_shield_buff_block:DeclareFunctions()
	local funcs = {
...
function modifier_extended_aphotic_shield_buff_block:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
		...
		self.shield_init_value = ability:GetLevelSpecialValueFor( "shield", ability_level ) + caster:FindTalentValue("special_bonus_extended_abaddon_5") + getOverChannelIncrease(caster)
...
function modifier_extended_aphotic_shield_buff_block:OnRemoved()
	if IsServer() then
		local target 				= self:GetParent()
		local caster 				= self:GetCaster()
		local ability 				= self:GetAbility()
		...
		if debuff_duration > 0 then
			for _,enemy in pairs(enemies) do
				enemy:AddNewModifier(caster, curse_of_avernus, "modifier_extended_curse_of_avernus_debuff_slow", { duration = debuff_duration })
...
function modifier_extended_aphotic_shield_buff_block:OnTakeDamage(kv)
	if IsServer() then
		local target = self:GetParent()
		...
		if target == kv.unit then
			...
			if target:HasModifier("modifier_extended_borrowed_time_buff_hot_caster") == false then
				...
				if damage > 0 then
					...
					if self.shield_remaining <= 0 then
						target:RemoveModifierByName("modifier_extended_aphotic_shield_buff_block")
					end
				end
			end
		end
...
function modifier_extended_aphotic_shield_buff_block:OnIntervalThink()
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_curse_of_avernus_debuff_slow"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_curse_of_avernus_buff_haste"] = LUA_MODIFIER_MOTION_NONE
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_curse_of_avernus_passive"] = LUA_MODIFIER_MOTION_NONE,
})
extended_abaddon_curse_of_avernus = extended_abaddon_curse_of_avernus or class({
	GetIntrinsicModifierName = function(self) return "modifier_extended_curse_of_avernus_passive" end
})

modifier_extended_curse_of_avernus_passive = modifier_extended_curse_of_avernus_passive or class({
...
function modifier_extended_curse_of_avernus_passive:OnCreated()
	if IsServer() then
		local target = self:GetParent()
...
function modifier_extended_curse_of_avernus_passive:DeclareFunctions()
	local funcs = {
...
function modifier_extended_curse_of_avernus_passive:OnAttack(kv)
	if IsServer() then
		local caster = self:GetCaster()
		...
		if not caster:PassivesDisabled() then
			...
			if attacker == caster then
				...
				if target:GetTeamNumber() ~= caster:GetTeamNumber() then
					...
					target:AddNewModifier(caster, ability, "modifier_extended_curse_of_avernus_debuff_slow", { duration = debuff_duration })
				end
			end
		end
	end
...
modifier_extended_curse_of_avernus_debuff_slow = modifier_extended_curse_of_avernus_debuff_slow or class({
	IsHidden				= function(self) return false end,
	IsPurgable	  			= function(self) return true end,
...
function modifier_extended_curse_of_avernus_debuff_slow:_UpdateSlowValues()
	local ability = self:GetAbility()
...
function modifier_extended_curse_of_avernus_debuff_slow:OnCreated()
	...
	if IsServer() then
		...
		local buff_name = "modifier_extended_curse_of_avernus_buff_haste"
...
function modifier_extended_curse_of_avernus_debuff_slow:OnRefresh()
	self:_UpdateSlowValues()
end

function modifier_extended_curse_of_avernus_debuff_slow:DeclareFunctions()
	local funcs = {
...
function modifier_extended_curse_of_avernus_debuff_slow:OnAttack(kv)
	if IsServer() then
		...
		if target == self:GetParent() then
			...
			if caster:GetTeamNumber() == attacker:GetTeamNumber() then
				...
				attacker:AddNewModifier(caster, ability, "modifier_extended_curse_of_avernus_buff_haste", { duration = buff_duration })
			end
		end
	end
end

function modifier_extended_curse_of_avernus_debuff_slow:OnTakeDamage(kv)
	...
	if IsServer() then
		...
		if heal_convert > 0 then
			...
			if target == kv.unit then
				...
				if caster:HasModifier("modifier_extended_borrowed_time_buff_hot_caster") then
...
function modifier_extended_curse_of_avernus_debuff_slow:GetModifierMoveSpeedBonus_Percentage() return self.move_slow end
function modifier_extended_curse_of_avernus_debuff_slow:GetModifierAttackSpeedBonus_Constant() return self.attack_slow end

modifier_extended_curse_of_avernus_buff_haste = modifier_extended_curse_of_avernus_buff_haste or class({
	IsHidden				= function(self) return false end,
	IsPurgable	  			= function(self) return true end,
	IsDebuff	  			= function(self) return false end,
...
function modifier_extended_curse_of_avernus_buff_haste:_UpdateIncreaseValues()
	local ability = self:GetAbility()
	local caster = self:GetCaster()

	-- #4 Talent: +5% move speed/+30 attack speed on Curse of Avernus
	local talent_name = "special_bonus_extended_abaddon_4"
...
function modifier_extended_curse_of_avernus_buff_haste:OnCreated()
	self:_UpdateIncreaseValues()
end

function modifier_extended_curse_of_avernus_buff_haste:OnRefresh()
	self:_UpdateIncreaseValues()
end

function modifier_extended_curse_of_avernus_buff_haste:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
		MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT
	}
...
function modifier_extended_curse_of_avernus_buff_haste:GetModifierMoveSpeedBonus_Percentage() return self.move_increase end
function modifier_extended_curse_of_avernus_buff_haste:GetModifierAttackSpeedBonus_Constant() return self.attack_increase end
...
extended_abaddon_over_channel = extended_abaddon_over_channel or class({
	IsStealable 			= function(self) return false end,
	IsInnateAbility			= function(self) return true end,

})

function extended_abaddon_over_channel:OnToggle()
	if IsServer() then
		local caster 		= self:GetCaster()
...
function extended_abaddon_over_channel:OnOwnerSpawned()
...
function extended_abaddon_over_channel:OnOwnerDied()
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_borrowed_time_buff_hot_caster"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_borrowed_time_buff_hot_ally"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifer_extended_borrowed_time_handler"] = LUA_MODIFIER_MOTION_NONE,
})
extended_abaddon_borrowed_time = extended_abaddon_borrowed_time or class({
	GetIntrinsicModifierName = function(self) if self:GetCaster():IsRealHero() then return "modifer_extended_borrowed_time_handler" end end
})
function extended_abaddon_borrowed_time:IsNetherWardStealable() return false end
function extended_abaddon_borrowed_time:OnUpgrade()
	local over_channel = self:GetCaster():FindAbilityByName("extended_abaddon_over_channel")
	if over_channel then
		over_channel:SetLevel(1 + self:GetLevel())
	end
end

function extended_abaddon_borrowed_time:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local ability_level = self:GetLevel()
		-- #7 Talent: +1sec duration on Borrowed Time
		local buff_duration = self:GetLevelSpecialValueFor("duration", ability_level) + caster:FindTalentValue("special_bonus_extended_abaddon_7")
		caster:AddNewModifier(caster, self, "modifier_extended_borrowed_time_buff_hot_caster", { duration = buff_duration })
	end
end

modifer_extended_borrowed_time_handler = modifer_extended_borrowed_time_handler or class({
	IsHidden				= function(self) return true end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,
	AllowIllusionDuplicate	= function(self) return false end,
...
function modifer_extended_borrowed_time_handler:_CheckHealth(damage)
	local target = self:GetParent()
	local ability = self:GetAbility()
...
function modifer_extended_borrowed_time_handler:OnCreated()
	if IsServer() then
		local target = self:GetParent()
		if target:IsIllusion() then
			self:Destroy()
...
function modifer_extended_borrowed_time_handler:DeclareFunctions()
	local funcs = {
		MODIFIER_EVENT_ON_TAKEDAMAGE,
...
function modifer_extended_borrowed_time_handler:OnTakeDamage(kv)
...
function modifer_extended_borrowed_time_handler:OnStateChanged(kv)
...
modifier_extended_borrowed_time_buff_hot_caster = modifier_extended_borrowed_time_buff_hot_caster or class({
	IsHidden				= function(self) return false end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,
	IsAura					= function(self) return true end,
	IsAuraActiveOnDeath		= function(self) return false end,
	GetModifierAura			= function(self) return "modifier_extended_borrowed_time_buff_hot_ally" end,
...
function modifier_extended_borrowed_time_buff_hot_caster:DeclareFunctions()
	local funcs = {
		MODIFIER_EVENT_ON_TAKEDAMAGE
	}
...
function modifier_extended_borrowed_time_buff_hot_caster:GetAuraRadius()
	-- #8 Talent: +300 aura range on Borrowed Time
	return self:GetAbility():GetSpecialValueFor("redirect_range") + self:GetCaster():FindTalentValue("special_bonus_extended_abaddon_8")
end

function modifier_extended_borrowed_time_buff_hot_caster:GetAuraEntityReject(hEntity)
	-- Do not apply aura to target
	return hEntity == self:GetParent()
end

function modifier_extended_borrowed_time_buff_hot_caster:OnCreated()
	if IsServer() then
		local target = self:GetParent()
...
function modifier_extended_borrowed_time_buff_hot_caster:OnIntervalThink()
	-- Get current health per frame
	self.target_current_health = self:GetParent():GetHealth()
end

function modifier_extended_borrowed_time_buff_hot_caster:OnTakeDamage(kv)
	if IsServer() then
...
modifier_extended_borrowed_time_buff_hot_ally = modifier_extended_borrowed_time_buff_hot_ally or class({
	IsHidden				= function(self) return false end,
	IsPurgable	  			= function(self) return false end,
	IsDebuff	  			= function(self) return false end,
})

function modifier_extended_borrowed_time_buff_hot_ally:DeclareFunctions()
	local funcs = {
		MODIFIER_EVENT_ON_TAKEDAMAGE
	}
...
function modifier_extended_borrowed_time_buff_hot_ally:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
		local target = self:GetParent()
...
function modifier_extended_borrowed_time_buff_hot_ally:OnRemoved()
	if IsServer() then
		local caster = self:GetCaster()
		local buff_list = caster._borrowed_time_buffed_allies
		if buff_list then
...
function modifier_extended_borrowed_time_buff_hot_ally:OnTakeDamage(kv)
	if IsServer() then
		local target = self:GetParent()
