...
--       BURROWSTRIKE        --
-------------------------------
extended_sandking_burrowstrike = class({})
LinkLuaModifier("modifier_extended_burrowstrike_stun", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_burrowstrike_burrow", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sandking_burrowstrike:IsHiddenWhenStolen()
    return false
end

function extended_sandking_burrowstrike:IsNetherWardStealable()
    return false
end

function extended_sandking_burrowstrike:GetCastRange(location, target)
    local caster = self:GetCaster()
    local cast_range = self.BaseClass.GetCastRange(self, location, target)

    -- #3 Talent: Burrowstrike cast range increase
    cast_range = cast_range + caster:FindTalentValue("special_bonus_extended_sand_king_3")
    return cast_range
end

function extended_sandking_burrowstrike:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()    
    local ability = self
    ...
    local modifier_burrow = "modifier_extended_burrowstrike_burrow"    
    ...
    burrow_radius = burrow_radius + caster:FindTalentValue("special_bonus_extended_sand_king_1")
...
function extended_sandking_burrowstrike:OnProjectileHit(target, location)
    ...
    local modifier_stun = "modifier_extended_burrowstrike_stun"    
    local modifier_poison = "modifier_extended_caustic_finale_poison"
    ...
    -- Caustic Finale
    local caustic_ability_name = "extended_sandking_caustic_finale"
...
modifier_extended_burrowstrike_stun = class({})

function modifier_extended_burrowstrike_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_burrowstrike_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_burrowstrike_stun:GetEffectAttachType()
    return PATTACH_OVERHEAD_FOLLOW
end

function modifier_extended_burrowstrike_stun:IsHidden() return false end
function modifier_extended_burrowstrike_stun:IsPurgeException() return true end
function modifier_extended_burrowstrike_stun:IsStunDebuff() return true end


-- Burrowstrike burrow modifier
modifier_extended_burrowstrike_burrow = class({})

function modifier_extended_burrowstrike_burrow:OnCreated()
    if IsServer() then
...
function modifier_extended_burrowstrike_burrow:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true,
...
function modifier_extended_burrowstrike_burrow:OnDestroy()
    if IsServer() then
...
function modifier_extended_burrowstrike_burrow:IsHidden() return true end
function modifier_extended_burrowstrike_burrow:IsPurgable() return false end
function modifier_extended_burrowstrike_burrow:IsDebuff() return false end


-------------------------------
--        SAND STORM         --
-------------------------------
extended_sandking_sand_storm = class({})
LinkLuaModifier("modifier_extended_sandstorm", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_sandstorm_invis", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_sandstorm_aura", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sandking_sand_storm:IsHiddenWhenStolen()
    return false
end

function extended_sandking_sand_storm:GetIntrinsicModifierName()
    return "modifier_extended_sandstorm_aura"
end

function extended_sandking_sand_storm:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local sound_cast = "Ability.SandKing_SandStorm.start"
    local sound_loop = "Ability.SandKing_SandStorm.loop"
    local sound_darude = "Extended.SandKingSandStorm"
    local particle_sandstorm = "particles/units/heroes/hero_sandking/sandking_sandstorm.vpcf"
    local modifier_sandstorm = "modifier_extended_sandstorm"    
    local modifier_invis = "modifier_extended_sandstorm_invis"
    ...
    if string.find(caster:GetUnitName(), "npc_extended_pugna_nether_ward") then        
...
function extended_sandking_sand_storm:OnChannelFinish()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local sound_loop = "Ability.SandKing_SandStorm.loop"
...
modifier_extended_sandstorm = class({})

function modifier_extended_sandstorm:OnCreated()
    if IsServer() then
        -- Ability properties    
        self.caster = self:GetCaster()
        ...
        self.damage = self.damage + self.caster:FindTalentValue("special_bonus_extended_sand_king_2")
...
function modifier_extended_sandstorm:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_sandstorm:IsHidden() return true end
function modifier_extended_sandstorm:IsPurgable() return false end
function modifier_extended_sandstorm:IsDebuff() return false end

function modifier_extended_sandstorm:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_OVERRIDE_ANIMATION}

    return decFuncs    
end

function modifier_extended_sandstorm:GetOverrideAnimation()
...
modifier_extended_sandstorm_invis = class({})

function modifier_extended_sandstorm_invis:IsHidden() return false end
function modifier_extended_sandstorm_invis:IsPurgable() return false end
function modifier_extended_sandstorm_invis:IsDebuff() return false end

function modifier_extended_sandstorm_invis:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_INVISIBILITY_LEVEL}

    return decFuncs
end

function modifier_extended_sandstorm_invis:GetModifierInvisibilityLevel()
    return 1
end

function modifier_extended_sandstorm_invis:CheckState()
    local state = {[MODIFIER_STATE_NO_UNIT_COLLISION] = true,
                   [MODIFIER_STATE_INVISIBLE] = true}
    return state
end

function modifier_extended_sandstorm_invis:GetPriority()
    return MODIFIER_PRIORITY_NORMAL
end


-- Sandstorm aura modifier
modifier_extended_sandstorm_aura = class({})

function modifier_extended_sandstorm_aura:OnCreated() 
    if IsServer() then
        Timers:CreateTimer(function()
            self.caster = self:GetCaster()
            self.aura_talent = self.caster:HasTalent("special_bonus_extended_sand_king_7")
            ...
            if self.aura_talent then
                ...
                -- Aura values    
                self.damage = self.damage * (1 - self.caster:FindTalentValue("special_bonus_extended_sand_king_7") * 0.01)
                self.radius = self.radius * (1 - self.caster:FindTalentValue("special_bonus_extended_sand_king_7") * 0.01)
                self.pull_speed = self.pull_speed * (1 - self.caster:FindTalentValue("special_bonus_extended_sand_king_7") * 0.01)
...
function modifier_extended_sandstorm_aura:IsHidden() 
    if self.aura_talent then
        return false
    end
...
function modifier_extended_sandstorm_aura:IsPurgable() return false end
function modifier_extended_sandstorm_aura:IsDebuff() return false end

function modifier_extended_sandstorm_aura:OnIntervalThink()
    if IsServer() then
        -- Resolve NPC positions
        ResolveNPCPositions(self.caster:GetAbsOrigin(), self.radius)
...
function modifier_extended_sandstorm_aura:OnRefresh()
    self:OnCreated()
end
...
extended_sandking_caustic_finale = class({})
LinkLuaModifier("modifier_extended_caustic_finale_trigger", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_caustic_finale_poison", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_caustic_finale_debuff", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)


function extended_sandking_caustic_finale:GetIntrinsicModifierName()
    return "modifier_extended_caustic_finale_trigger"
end

-- Caustic Finale caster's trigger
modifier_extended_caustic_finale_trigger = class({})

function modifier_extended_caustic_finale_trigger:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()    
    self.modifier_poison = "modifier_extended_caustic_finale_poison"

    -- Ability specials
    self.poison_duration = self.ability:GetSpecialValueFor("poison_duration")
end

function modifier_extended_caustic_finale_trigger:OnRefresh()
    self:OnCreated()
end

function modifier_extended_caustic_finale_trigger:IsHidden() return true end   
function modifier_extended_caustic_finale_trigger:IsPurgable() return false end
function modifier_extended_caustic_finale_trigger:IsDebuff() return false end

function modifier_extended_caustic_finale_trigger:DeclareFunctions()
...
function modifier_extended_caustic_finale_trigger:OnAttackLanded(keys)
    if IsServer() then
...
function modifier_extended_caustic_finale_trigger:OnTakeDamage(keys)
    if IsServer() then
        ...
        if self.caster:HasTalent("special_bonus_extended_sand_king_8") then
...
modifier_extended_caustic_finale_poison = class({})

function modifier_extended_caustic_finale_poison:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    ...
    self.modifier_poison = "modifier_extended_caustic_finale_poison"
    self.modifier_slow = "modifier_extended_caustic_finale_debuff"
...
function modifier_extended_caustic_finale_poison:IsHidden() return false end   
function modifier_extended_caustic_finale_poison:IsPurgable() return true end
function modifier_extended_caustic_finale_poison:IsDebuff() return true end

function modifier_extended_caustic_finale_poison:OnDestroy()
    if IsServer() then       
...
modifier_extended_caustic_finale_debuff = class({})

function modifier_extended_caustic_finale_debuff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    ...
    self.ms_slow_pct = self.ms_slow_pct + self.caster:FindTalentValue("special_bonus_extended_sand_king_5")
end

function modifier_extended_caustic_finale_debuff:IsHidden() return false end   
function modifier_extended_caustic_finale_debuff:IsPurgable() return true end
function modifier_extended_caustic_finale_debuff:IsDebuff() return true end

function modifier_extended_caustic_finale_debuff:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_caustic_finale_debuff:GetModifierMoveSpeedBonus_Percentage()
...
--       BURROWSTRIKE        --
-------------------------------
extended_sandking_epicenter = class({})
LinkLuaModifier("modifier_extended_epicenter_pulse", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_epicenter_slow", "hero/hero_sand_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_sandking_epicenter:IsHiddenWhenStolen()
    return false
end

function extended_sandking_epicenter:GetChannelTime()
    local caster = self:GetCaster()
    local ability = self
...
function extended_sandking_epicenter:OnSpellStart()    
    -- Ability properties
    local caster = self:GetCaster()    
    local ability = self
    ...
    local modifier_pulse = "modifier_extended_epicenter_pulse"
...
function extended_sandking_epicenter:OnChannelFinish(interrupted)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local modifier_pulse = "modifier_extended_epicenter_pulse"
...
-- Epicenter modifier
modifier_extended_epicenter_pulse = class({})

function modifier_extended_epicenter_pulse:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_slow = "modifier_extended_epicenter_slow"
        ...
        self.pulse_count = self.pulse_count + self.caster:FindTalentValue("special_bonus_extended_sand_king_6")
...
function modifier_extended_epicenter_pulse:OnIntervalThink()
    if IsServer() then
        -- Assign radiuses
        self.pull_radius = self.radius

        -- #4 Talent: Epicenter increase pull radius
        self.pull_radius = self.pull_radius + self.caster:FindTalentValue("special_bonus_extended_sand_king_4")
...
function modifier_extended_epicenter_pulse:IsHidden() return true end
function modifier_extended_epicenter_pulse:IsPurgable() return false end
function modifier_extended_epicenter_pulse:IsDebuff() return false end

-- Epicenter Slow modifier
modifier_extended_epicenter_slow = class({})

function modifier_extended_epicenter_slow:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_epicenter_slow:IsHidden() return false end
function modifier_extended_epicenter_slow:IsPurgable() return true end
function modifier_extended_epicenter_slow:IsDebuff() return true end

function modifier_extended_epicenter_slow:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
...
function modifier_extended_epicenter_slow:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end

function modifier_extended_epicenter_slow:GetModifierAttackSpeedBonus_Constant()
