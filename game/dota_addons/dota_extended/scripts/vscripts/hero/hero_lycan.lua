...
function modifier_extended_lycan_wolf_charge:RemoveOnDeath()
	return false
end
...
function modifier_extended_feral_impulse_aura:GetEffectName()
	return "particles/auras/aura_feral_impulse.vpcf"
end

function modifier_extended_feral_impulse_aura:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end
...
function modifier_extended_feral_impulse:OnCreated()	
	...
	self.aura_buff = "modifier_extended_feral_impulse_aura" 
...
function modifier_extended_feral_impulse:GetModifierBaseDamageOutgoing_Percentage()	 		
	...
	damage_inc_per_unit = damage_inc_per_unit + self.caster:FindTalentValue("special_bonus_extended_lycan_2")	
...
function modifier_extended_feral_impulse:GetModifierConstantHealthRegen()
	...
	regen_inc_per_unit = regen_inc_per_unit + self.caster:FindTalentValue("special_bonus_extended_lycan_2")		
...
function modifier_extended_shapeshift_transform:IsPurgable()
	return false
end

function modifier_extended_shapeshift_transform:IsDebuff()
	return false
end
...
function modifier_extended_shapeshift_certain_crit:IsHidden()
	return false
end

function modifier_extended_shapeshift_certain_crit:IsPurgable()
	return false
end

function modifier_extended_shapeshift_certain_crit:IsDebuff()
	return false
end
...
function extended_lycan_wolfsbane:IsInnateAbility()
	return true
end
...
function modifier_extended_summoned_wolf_wicked_crunch_debuff:OnCreated()	
		-- Ability properties
		self.caster = self:GetCaster()		
		self.ability = self:GetAbility()
	...
	if IsServer() then
		...
		self.max_stacks = self.max_stacks + self.owner:FindTalentValue("special_bonus_extended_lycan_6")
	end
end
...
function modifier_extended_summoned_wolf_wicked_crunch_debuff:OnStackCountChanged()
	if IsServer() then
		local stacks = self:GetStackCount()
