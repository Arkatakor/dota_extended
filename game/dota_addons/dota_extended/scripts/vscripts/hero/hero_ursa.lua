...
function modifier_extended_overpower_buff:OnAttack( keys )
	...
	if keys.attacker == caster then		
		...
		if caster:HasModifier("modifier_extended_starfury_dmg_penalty") then
			return nil
		end
...
function modifier_extended_enrage_buff:OnIntervalThink()
	...
	if IsServer() then
		local caster = self:GetCaster()
		local ability = self:GetAbility()	
		local ability_earthshock = caster:FindAbilityByName("extended_ursa_earthshock")
		local ability_overpower = caster:FindAbilityByName("extended_ursa_overpower")	
...
function modifier_extended_enrage_buff:GetModifierModelScale()
	return 40
end
...
function modifier_extended_scepter_enrage_damage:RemoveOnDeath()
	return false
end

function modifier_extended_scepter_enrage_damage:IsPurgable()
	return false
end

function modifier_extended_scepter_enrage_damage:IsDebuff()
	return false
end
...
function extended_ursa_territorial_hunter:IsInnateAbility()
	return true
end
