...
MergeTables(LinkedModifiers,{
	["modifier_extended_shuriken_toss_debuff_pull"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_shuriken_toss_stunned"] = LUA_MODIFIER_MOTION_NONE,
})
extended_bounty_hunter_shuriken_toss = extended_bounty_hunter_shuriken_toss or class({})
...
function extended_bounty_hunter_shuriken_toss:OnProjectileHit_ExtraData(target, location, extradata)
	if IsServer() then
		...
		damage = damage + caster:FindTalentValue("special_bonus_extended_bounty_hunter_3")

		-- #5 Talent - Shuriken bounce radius becomes global
		bounce_radius = bounce_radius + caster:FindTalentValue("special_bonus_extended_bounty_hunter_5")
		...
		target:AddNewModifier(caster, ability, "modifier_extended_shuriken_toss_stunned", {duration = stun_duration})
		...
		-- Apply pull modifier
		target:AddNewModifier(caster, ability, "modifier_extended_shuriken_toss_debuff_pull", {duration = pull_duration})
		...
		for _,enemy in pairs(enemies) do
			...
			if enemy:HasModifier("modifier_extended_track_debuff_mark") and not enemy_found then
...
-- Stun modifier
modifier_extended_shuriken_toss_stunned = modifier_extended_shuriken_toss_stunned or class({})

function modifier_extended_shuriken_toss_stunned:CheckState()
	local state = {[MODIFIER_STATE_STUNNED] = true}
	return state
end

function modifier_extended_shuriken_toss_stunned:IsStunDebuff()
	return true
end

function modifier_extended_shuriken_toss_stunned:IsHidden()
	return false
end

function modifier_extended_shuriken_toss_stunned:GetEffectName()
	return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_shuriken_toss_stunned:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end


-- Pull modifier
modifier_extended_shuriken_toss_debuff_pull = modifier_extended_shuriken_toss_debuff_pull or class({})

function modifier_extended_shuriken_toss_debuff_pull:OnCreated()
	if IsServer() then
		-- Ability properties
...
function modifier_extended_shuriken_toss_debuff_pull:OnIntervalThink()
	if IsServer() then
...
function modifier_extended_shuriken_toss_debuff_pull:OnRemoved()
	if IsServer() then
		-- Clear particles
...
function modifier_extended_shuriken_toss_debuff_pull:IsDebuff()
	return true
end

function modifier_extended_shuriken_toss_debuff_pull:IsHidden()
	return false
end

function modifier_extended_shuriken_toss_debuff_pull:IsPurgable()
	return true
end
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_jinada_debuff_slow"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_jinada_buff_crit"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_jinada_passive"] = LUA_MODIFIER_MOTION_NONE,
})

extended_bounty_hunter_jinada = extended_bounty_hunter_jinada or class({})

function extended_bounty_hunter_jinada:IsNetherWardStealable() return false end
...
function extended_bounty_hunter_jinada:GetCooldown(level)
	...
	cd = cd - caster:FindTalentValue("special_bonus_extended_bounty_hunter_7")
	return cd
end

function extended_bounty_hunter_jinada:GetIntrinsicModifierName()
	return "modifier_extended_jinada_passive"
end
...
function extended_bounty_hunter_jinada:CastFilterResultTarget(target)
	if IsServer() then
		...
		if not target:HasModifier("modifier_extended_track_debuff_mark") or target:GetTeamNumber() == caster:GetTeamNumber() then
...
function extended_bounty_hunter_jinada:OnSpellStart()
	if IsServer() then
		...
		Timers:CreateTimer(1, function()
			if caster:HasModifier("modifier_extended_jinada_buff_crit") then
				caster:RemoveModifierByName("modifier_extended_jinada_buff_crit")
			end
		end)
...
modifier_extended_jinada_passive = modifier_extended_jinada_passive or class({})

function modifier_extended_jinada_passive:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT
end

function modifier_extended_jinada_passive:OnCreated()
	self:StartIntervalThink(0.2)
end

function modifier_extended_jinada_passive:OnIntervalThink()
	if IsServer() then
		local caster = self:GetCaster()
		local ability = self:GetAbility()
		local crit_modifier = "modifier_extended_jinada_buff_crit"
...
function modifier_extended_jinada_passive:IsHidden()
	return true
end

function modifier_extended_jinada_passive:IsPurgable()
	return false
end

function modifier_extended_jinada_passive:IsDebuff()
	return false
end

-- Slow debuff modifier
modifier_extended_jinada_debuff_slow = modifier_extended_jinada_debuff_slow or class({})

function modifier_extended_jinada_debuff_slow:OnCreated()
		-- Prepare variables
		...
		-- #2 Talent: Jinada's slow increase
		 self.ms_slow_pct = self.ms_slow_pct + self.caster:FindTalentValue("special_bonus_extended_bounty_hunter_2")
		 self.as_slow = self.as_slow + self.caster:FindTalentValue("special_bonus_extended_bounty_hunter_2")
end

function modifier_extended_jinada_debuff_slow:DeclareFunctions()
...
function modifier_extended_jinada_debuff_slow:GetModifierMoveSpeedBonus_Percentage()
	return self.ms_slow_pct * (-1)
end

function modifier_extended_jinada_debuff_slow:GetModifierAttackSpeedBonus_Constant()
	return self.as_slow * (-1)
end

function modifier_extended_jinada_debuff_slow:IsDebuff()
	return true
end

function modifier_extended_jinada_debuff_slow:IsPurgable()
	return true
end

function modifier_extended_jinada_debuff_slow:IsHidden()
	return false
end


-- Jinada crit modifier
modifier_extended_jinada_buff_crit = modifier_extended_jinada_buff_crit or class({})

function modifier_extended_jinada_buff_crit:OnCreated()
	if IsServer() then
		self.parent = self:GetCaster()
		self.ability = self:GetAbility()
		self.parent = self:GetParent()
		...
		self.modifier_slow = "modifier_extended_jinada_debuff_slow"
...
function modifier_extended_jinada_buff_crit:DeclareFunctions()
...
function modifier_extended_jinada_buff_crit:OnAttackLanded(keys)
	if IsServer() then
...
function modifier_extended_jinada_buff_crit:GetModifierPreAttack_CriticalStrike(keys)
	local attacker = keys.attacker
	local target = keys.target
...
function modifier_extended_jinada_buff_crit:IsHidden()
	return true
end

function modifier_extended_jinada_buff_crit:IsPurgable()
	return false
end

function modifier_extended_jinada_buff_crit:IsDebuff()
	return false
end
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_shadow_walk_buff_invis"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_shadow_walk_vision"] = LUA_MODIFIER_MOTION_NONE,
})

extended_bounty_hunter_shadow_walk = extended_bounty_hunter_shadow_walk or class({})
LinkLuaModifier("modifier_extended_shadow_walk_buff_invis", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_shadow_walk_vision", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)

function extended_bounty_hunter_shadow_walk:IsNetherWardStealable() return false end
function extended_bounty_hunter_shadow_walk:GetCastRange(location, target)
	local caster = self:GetCaster()
	local ability = self
	local true_sight_radius = ability:GetSpecialValueFor("true_sight_radius")

	-- #1 Talent: Shadow Walk true sight radius increase
	true_sight_radius = true_sight_radius + caster:FindTalentValue("special_bonus_extended_bounty_hunter_1")
...
function extended_bounty_hunter_shadow_walk:OnSpellStart()
	if IsServer() then
		...
		local modifier_invis = "modifier_extended_shadow_walk_buff_invis"
...
modifier_extended_shadow_walk_buff_invis = modifier_extended_shadow_walk_buff_invis or class({})

function modifier_extended_shadow_walk_buff_invis:OnCreated()
	self.caster = self:GetCaster()
	self.ability = self:GetAbility()
	...
	-- #1 Talent: Shadow Walk true sight radius increase
	self.true_sight_radius = self.true_sight_radius + self.caster:FindTalentValue("special_bonus_extended_bounty_hunter_1")

end

function modifier_extended_shadow_walk_buff_invis:CheckState()
	local state = {[MODIFIER_STATE_INVISIBLE] = true,
				   [MODIFIER_STATE_NO_UNIT_COLLISION] = true}
	return state
end

function modifier_extended_shadow_walk_buff_invis:GetPriority()
	return MODIFIER_PRIORITY_NORMAL
end

function modifier_extended_shadow_walk_buff_invis:DeclareFunctions()
	local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
...
function modifier_extended_shadow_walk_buff_invis:GetModifierInvisibilityLevel()
	return 1
end

function modifier_extended_shadow_walk_buff_invis:GetModifierMoveSpeedBonus_Percentage()
	return self.invis_ms_bonus
end

function modifier_extended_shadow_walk_buff_invis:OnAbilityExecuted(keys)
	if IsServer() then
...
function modifier_extended_shadow_walk_buff_invis:OnAttackLanded(keys)
	if IsServer() then
		-- key properties
		local attacker = keys.attacker
		local target = keys.target
...
function modifier_extended_shadow_walk_buff_invis:GetAuraRadius()
	return self.true_sight_radius
end

function modifier_extended_shadow_walk_buff_invis:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
end

function modifier_extended_shadow_walk_buff_invis:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_shadow_walk_buff_invis:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_shadow_walk_buff_invis:GetModifierAura()
	return "modifier_extended_shadow_walk_vision"
end

function modifier_extended_shadow_walk_buff_invis:IsAura()
	return true
end

function modifier_extended_shadow_walk_buff_invis:IsDebuff()
	return false
end

function modifier_extended_shadow_walk_buff_invis:IsHidden()
	return false
end

function modifier_extended_shadow_walk_buff_invis:IsPurgable()
	return false
end

-- True sight debuff
modifier_extended_shadow_walk_vision = modifier_extended_shadow_walk_vision or class({})

function modifier_extended_shadow_walk_vision:CheckState()
	local state = {[MODIFIER_STATE_INVISIBLE] = false}
	return state
end

function modifier_extended_shadow_walk_vision:GetPriority()
	return MODIFIER_PRIORITY_HIGH
end

function modifier_extended_shadow_walk_vision:IsHidden()
	return true
end

function modifier_extended_shadow_walk_vision:IsPurgable()
	return false
end

function modifier_extended_shadow_walk_vision:IsDebuff()
	return true
end
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_track_buff_ms"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_track_debuff_mark"] = LUA_MODIFIER_MOTION_NONE,
})

extended_bounty_hunter_track = extended_bounty_hunter_track or class({})

function extended_bounty_hunter_track:GetCooldown(level)
	local caster = self:GetCaster()

	-- #4 Talent: Track cooldown decrease
	return self.BaseClass.GetCooldown(self, level) - caster:FindTalentValue("special_bonus_extended_bounty_hunter_4")
end
...
function extended_bounty_hunter_track:OnSpellStart()
	if IsServer() then
		...
		local modifier_track = "modifier_extended_track_debuff_mark"
		...
		duration = duration + caster:FindTalentValue("special_bonus_extended_bounty_hunter_6")
...
-- Track modifier (aura)
modifier_extended_track_debuff_mark = modifier_extended_track_debuff_mark or class({})

function modifier_extended_track_debuff_mark:OnCreated()
		-- Ability properties
		self.caster = self:GetCaster()
		self.ability = self:GetAbility()
		self.parent = self:GetParent()
...
function modifier_extended_track_debuff_mark:CheckState()
	local state = {[MODIFIER_STATE_INVISIBLE] = false}
	return state
end

function modifier_extended_track_debuff_mark:GetAuraDuration()
	return self.haste_linger
end

function modifier_extended_track_debuff_mark:GetAuraRadius()
	return self.haste_radius
end

function modifier_extended_track_debuff_mark:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
end

function modifier_extended_track_debuff_mark:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_track_debuff_mark:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_track_debuff_mark:GetModifierAura()
	return "modifier_extended_track_buff_ms"
end

function modifier_extended_track_debuff_mark:IsAura()
	return true
end

function modifier_extended_track_debuff_mark:IsDebuff()
	return true
end

function modifier_extended_track_debuff_mark:IsPurgable()
	-- #8 Talent - unpurgable track
	local purge_value = self.caster:FindTalentValue("special_bonus_extended_bounty_hunter_8")
...
function modifier_extended_track_debuff_mark:IsPermanent()
	return false
end

function modifier_extended_track_debuff_mark:IsHidden()
	-- #8 Talent - unpurgable track
	local hidden_value = self.caster:FindTalentValue("special_bonus_extended_bounty_hunter_8")
...
function modifier_extended_track_debuff_mark:DeclareFunctions()
...
function modifier_extended_track_debuff_mark:OnHeroKilled(keys)
	if IsServer() then
		local target = keys.target
...
function modifier_extended_track_debuff_mark:GetModifierProvidesFOWVision()
	return 1
end

function modifier_extended_track_debuff_mark:GetPriority()
	return MODIFIER_PRIORITY_HIGH
end


-- Allied haste modifier
modifier_extended_track_buff_ms = modifier_extended_track_buff_ms or class({})

function modifier_extended_track_buff_ms:OnCreated()
		-- Ability properties
		self.caster = self:GetCaster()
		self.ability = self:GetAbility()
		self.parent = self:GetParent()
...
function modifier_extended_track_buff_ms:DeclareFunctions()
	local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

	return decFuncs
end

function modifier_extended_track_buff_ms:GetModifierMoveSpeedBonus_Percentage()
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_headhunter_buff_handler"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_headhunter_passive"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_headhunter_debuff_handler"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_headhunter_debuff_illu"] = LUA_MODIFIER_MOTION_NONE,
})
extended_bounty_hunter_headhunter = extended_bounty_hunter_headhunter or class({})
LinkLuaModifier("modifier_extended_headhunter_passive", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_headhunter_debuff_handler", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_headhunter_buff_handler", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_headhunter_debuff_illu", "hero/hero_bounty_hunter", LUA_MODIFIER_MOTION_NONE)

function extended_bounty_hunter_headhunter:GetIntrinsicModifierName()
	return "modifier_extended_headhunter_passive"
end

function extended_bounty_hunter_headhunter:IsInnateAbility()
	return true
end
...
function extended_bounty_hunter_headhunter:OnProjectileHit(target, location)
	...
	local caster = self:GetCaster()
	local ability = self
	local modifier_contract_buff = "modifier_extended_headhunter_buff_handler"
	local modifier_contract_debuff = "modifier_extended_headhunter_debuff_handler"
...
--Contract buff (self)
modifier_extended_headhunter_passive = modifier_extended_headhunter_passive or class({})

function modifier_extended_headhunter_passive:OnCreated()
	if IsServer() then
		-- Ability properties
		self.caster = self:GetCaster()
		self.ability = self:GetAbility()
		self.modifier_contract = "modifier_extended_headhunter_debuff_handler"
...
function modifier_extended_headhunter_passive:OnIntervalThink()
	if IsServer() then
...
function modifier_extended_headhunter_passive:IsDebuff()
	return false
end

function modifier_extended_headhunter_passive:IsPurgable()
	return false
end

function modifier_extended_headhunter_passive:IsHidden()
	return true
end

-- Contract self buff
modifier_extended_headhunter_buff_handler = class({})

function modifier_extended_headhunter_buff_handler:IsDebuff()
	return false
end

function modifier_extended_headhunter_buff_handler:IsPurgable()
	return false
end

function modifier_extended_headhunter_buff_handler:IsHidden()
	return false
end


-- Contract debuff
modifier_extended_headhunter_debuff_handler = modifier_extended_headhunter_debuff_handler or class({})

function modifier_extended_headhunter_debuff_handler:OnCreated()
	if IsServer() then
		self.caster = self:GetCaster()
		self.ability = self:GetAbility()
		self.parent = self:GetParent()
		self.particle_contract = "particles/hero/bounty_hunter/bounty_hunter_headhunter_scroll.vpcf"
		self.modifier_contract_buff = "modifier_extended_headhunter_buff_handler"
		self.track_debuff = "modifier_extended_track_debuff_mark"
		self.track_ability_name = "extended_bounty_hunter_track"
		self.gold_minimum = self.ability:GetSpecialValueFor("gold_minimum")
		self.modifier_dummy = "modifier_extended_headhunter_debuff_illu"
...
function modifier_extended_headhunter_debuff_handler:OnIntervalThink()
...
function modifier_extended_headhunter_debuff_handler:DeclareFunctions()
	local decFuncs = {MODIFIER_EVENT_ON_HERO_KILLED}

	return decFuncs
end

function modifier_extended_headhunter_debuff_handler:OnHeroKilled(keys)
	if IsServer() then
		local attacker = keys.attacker
		local target = keys.target
...
function modifier_extended_headhunter_debuff_handler:IsDebuff()
	return true
end

function modifier_extended_headhunter_debuff_handler:IsPurgable()
	return false
end

function modifier_extended_headhunter_debuff_handler:IsHidden()
	return true
end

modifier_extended_headhunter_debuff_illu = modifier_extended_headhunter_debuff_illu or class({})

function modifier_extended_headhunter_debuff_illu:OnCreated()
	if IsServer() then
		self.caster = self:GetCaster()
		self.parent = self:GetParent()
		self.particle_contract = "particles/hero/bounty_hunter/bounty_hunter_headhunter_scroll.vpcf"
...
function modifier_extended_headhunter_debuff_illu:IsDebuff()
	return true
end

function modifier_extended_headhunter_debuff_illu:IsPurgable()
	return false
end

function modifier_extended_headhunter_debuff_illu:IsHidden()
	return true
end
-------------------------------------------
