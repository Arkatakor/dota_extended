...
extended_omniknight_purification = class({})
LinkLuaModifier("modifier_extended_purification_buff", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_purification_omniguard_ready", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_purification_omniguard_recharging", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)


function extended_omniknight_purification:IsHiddenWhenStolen()
    return false
end

function extended_omniknight_purification:GetIntrinsicModifierName()
    return "modifier_extended_purification_omniguard_ready"
end

function extended_omniknight_purification:GetAOERadius()
    local ability = self
...
function extended_omniknight_purification:OnSpellStart()
    -- Ability properties
    ...
    if caster:HasTalent("special_bonus_extended_omniknight_4") then
        local bounce_radius = caster:FindTalentValue("special_bonus_extended_omniknight_4")
...
function Purification(caster, ability, target)
    ...
    local modifier_purifiception = "modifier_extended_purification_buff"    
    ...
    heal_amount = heal_amount + caster:FindTalentValue("special_bonus_extended_omniknight_8")
...
modifier_extended_purification_buff = class({})

function modifier_extended_purification_buff:OnCreated()
    -- Ability properties
...
function modifier_extended_purification_buff:IsHidden() return false end
function modifier_extended_purification_buff:IsPurgable() return true end
function modifier_extended_purification_buff:IsDebuff() return false end

function modifier_extended_purification_buff:DeclareFunctions()
...
function modifier_extended_purification_buff:GetEffectName()
    return "particles/hero/omniknight/purification_buff.vpcf"    
end

function modifier_extended_purification_buff:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_purification_buff:GetModifierHealAmplify_Percentage(keys)            
...
function modifier_extended_purification_buff:OnHealthGained(keys)    
    if IsServer() then               
...
function modifier_extended_purification_buff:OnStackCountChanged()
    if IsServer() then
        local stacks = self:GetStackCount()
...
modifier_extended_purification_omniguard_ready = class({})

function modifier_extended_purification_omniguard_ready:OnCreated()            
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.modifier_recharge = "modifier_extended_purification_omniguard_recharging"                
...
function modifier_extended_purification_omniguard_ready:IsHidden()
    if self.caster:HasTalent("special_bonus_extended_omniknight_2") then
        return false
    end
...
function modifier_extended_purification_omniguard_ready:IsPurgable() return false end
function modifier_extended_purification_omniguard_ready:IsDebuff() return false end



function modifier_extended_purification_omniguard_ready:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_TAKEDAMAGE}

    return decFuncs
end

function modifier_extended_purification_omniguard_ready:OnTakeDamage(keys)
    if IsServer() then
        ...
        if self.caster == unit then
            ...
            if not self.caster:HasTalent("special_bonus_extended_omniknight_2") then
                return nil
            end
            ...
            self.cooldown = self.caster:FindSpecificTalentValue("special_bonus_extended_omniknight_2", "cooldown")
            self.trigger_hp_pct = self.caster:FindSpecificTalentValue("special_bonus_extended_omniknight_2", "trigger_hp_pct")
...
modifier_extended_purification_omniguard_recharging = class({})

function modifier_extended_purification_omniguard_recharging:OnCreated()
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.modifier_omniguard = "modifier_extended_purification_omniguard_ready"
end

function modifier_extended_purification_omniguard_recharging:IsHidden() return false end
function modifier_extended_purification_omniguard_recharging:IsPurgable() return false end
function modifier_extended_purification_omniguard_recharging:IsDebuff() return false end

function modifier_extended_purification_omniguard_recharging:GetTexture()
    return "custom/omnikinight_purification_cooldown"
end

function modifier_extended_purification_omniguard_recharging:OnDestroy()
    if IsServer() then
...
extended_omniknight_repel = class({})
LinkLuaModifier("modifier_extended_repel", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_degen_aura", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_degen_debuff", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)

function extended_omniknight_repel:IsHiddenWhenStolen()
    return false
end

function extended_omniknight_repel:GetIntrinsicModifierName()
    return "modifier_extended_degen_aura"
end

function extended_omniknight_repel:OnUnStolen()
    local caster = self:GetCaster()
    local modifier_degen = "modifier_extended_degen_aura"
...
function extended_omniknight_repel:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget() 
    ...
    if caster:HasTalent("special_bonus_extended_omniknight_6") then
        -- Talent values
        local radius = caster:FindSpecificTalentValue("special_bonus_extended_omniknight_6", "radius")
        local talent_duration = caster:FindSpecificTalentValue("special_bonus_extended_omniknight_6", "duration")
...
function Repel(caster, ability, target, duration)
    ...
    local modifier_repel = "modifier_extended_repel"
    local modifier_degen_aura = "modifier_extended_degen_aura"    
...
-- Repel modifier
modifier_extended_repel = class({})

function modifier_extended_repel:IsHidden() return false end
function modifier_extended_repel:IsPurgable() return false end
function modifier_extended_repel:IsDebuff() return false end

function modifier_extended_repel:CheckState()
    local state = {[MODIFIER_STATE_MAGIC_IMMUNE] = true}
    return state
end

function modifier_extended_repel:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}

    return decFuncs
end

function modifier_extended_repel:GetModifierMagicalResistanceBonus()
    return 100
end

function modifier_extended_repel:GetEffectName()
    return "particles/units/heroes/hero_omniknight/omniknight_repel_buff.vpcf"
end

function modifier_extended_repel:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end


-- Degen Aura
modifier_extended_degen_aura = class({})

function modifier_extended_degen_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_degen_aura:GetEffectName()
    return "particles/auras/aura_degen.vpcf"
end

function modifier_extended_degen_aura:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_degen_aura:GetAuraDuration()
    return self.linger_duration
end

function modifier_extended_degen_aura:GetAuraRadius()
    return self.aura_radius
end

function modifier_extended_degen_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_degen_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_degen_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_degen_aura:GetModifierAura()
    return "modifier_extended_degen_debuff"
end

function modifier_extended_degen_aura:IsAura()
    if self.caster:PassivesDisabled() then
        return false
    end
...
function modifier_extended_degen_aura:IsHidden() return true end
function modifier_extended_degen_aura:IsPurgable() return false end
function modifier_extended_degen_aura:IsDebuff() return false end

function modifier_extended_degen_aura:OnRefresh()
    self:OnCreated()
end

-- Degen slow debuff 
modifier_extended_degen_debuff = class({})

function modifier_extended_degen_debuff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    ...
    self.ms_slow_pct = self.ms_slow_pct + self.caster:FindTalentValue("special_bonus_extended_omniknight_3")
    self.as_slow = self.as_slow + self.caster:FindTalentValue("special_bonus_extended_omniknight_3")
end

function modifier_extended_degen_debuff:GetEffectName()
    return "particles/units/heroes/hero_omniknight/omniknight_degen_aura_debuff.vpcf"    
end

function modifier_extended_degen_debuff:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW 
end

function modifier_extended_degen_debuff:DeclareFunctions()
...
function modifier_extended_degen_debuff:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end

function modifier_extended_degen_debuff:GetModifierAttackSpeedBonus_Constant()
    return self.as_slow * (-1)
end

function modifier_extended_degen_debuff:GetTexture()
...
extended_omniknight_hammer_of_virtue = class({})
LinkLuaModifier("modifier_extended_hammer_of_virtue", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_hammer_of_virtue_nodamage", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)

function extended_omniknight_hammer_of_virtue:GetIntrinsicModifierName()
    return "modifier_extended_hammer_of_virtue"
end

-- Hammer of virtue passive modifier
modifier_extended_hammer_of_virtue = class({})

function modifier_extended_hammer_of_virtue:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()    
        ...
        self.modifier_nodmg = "modifier_extended_hammer_of_virtue_nodamage"
...
function modifier_extended_hammer_of_virtue:IsHidden() return true end
function modifier_extended_hammer_of_virtue:IsPurgable() return false end
function modifier_extended_hammer_of_virtue:IsDebuff() return false end

function modifier_extended_hammer_of_virtue:OnRefresh()
    self:OnCreated()
end

function modifier_extended_hammer_of_virtue:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_ATTACK_LANDED}

    return decFuncs
end

function modifier_extended_hammer_of_virtue:OnAttackLanded(keys)
    if IsServer() then
        ...
        if self.caster:HasTalent("special_bonus_extended_omniknight_1") and not self.talent_1_leveled then
            self.damage_as_heal_pct = self.damage_as_heal_pct + self.caster:FindTalentValue("special_bonus_extended_omniknight_1")        
...
modifier_extended_hammer_of_virtue_nodamage =class({})

function modifier_extended_hammer_of_virtue_nodamage:IsHidden() return true end
function modifier_extended_hammer_of_virtue_nodamage:IsDebuff() return false end
function modifier_extended_hammer_of_virtue_nodamage:IsPurgable() return false end

-- Declare modifier events/properties
function modifier_extended_hammer_of_virtue_nodamage:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_ABSOLUTE_NO_DAMAGE_PHYSICAL}

    return decFuncs
end

function modifier_extended_hammer_of_virtue_nodamage:GetAbsoluteNoDamagePhysical()
    return 1 
end
...
extended_omniknight_guardian_angel = class({})
LinkLuaModifier("modifier_extended_guardian_angel", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_guardian_angel_shield", "hero/hero_omniknight.lua", LUA_MODIFIER_MOTION_NONE)

function extended_omniknight_guardian_angel:IsHiddenWhenStolen()
    return false
end

function extended_omniknight_guardian_angel:GetCooldown(level)
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #7 Talent: Guardian Angel cooldown decrease
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_omniknight_7")

    return cooldown    
end

function extended_omniknight_guardian_angel:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_angel = "modifier_extended_guardian_angel"
...
modifier_extended_guardian_angel = class({})

function modifier_extended_guardian_angel:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    ...
    self.modifier_shield = "modifier_extended_guardian_angel_shield"
...
function modifier_extended_guardian_angel:GetStatusEffectName()
    return "particles/status_fx/status_effect_guardian_angel.vpcf"
end

function modifier_extended_guardian_angel:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_ABSOLUTE_NO_DAMAGE_PHYSICAL}

    return decFuncs
end

function modifier_extended_guardian_angel:GetAbsoluteNoDamagePhysical()
    return 1
end

function modifier_extended_guardian_angel:IsHidden() return false end
function modifier_extended_guardian_angel:IsPurgable() return true end
function modifier_extended_guardian_angel:IsDebuff() return false end

function modifier_extended_guardian_angel:OnDestroy()
    if IsServer() then
...
modifier_extended_guardian_angel_shield = class({})

function modifier_extended_guardian_angel_shield:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.max_hp_shield_health_pct = self.max_hp_shield_health_pct + self.caster:FindTalentValue("special_bonus_extended_omniknight_5")        
...
function modifier_extended_guardian_angel_shield:GetEffectName()
    return "particles/hero/omniknight/guardian_angel_shield.vpcf"
end

function modifier_extended_guardian_angel_shield:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_guardian_angel_shield:DeclareFunctions()
...
function modifier_extended_guardian_angel_shield:GetModifierAvoidDamage()
    return 1
end

function modifier_extended_guardian_angel_shield:OnTakeDamage(keys)
    if IsServer() then
...
function modifier_extended_guardian_angel_shield:IsHidden() return false end
function modifier_extended_guardian_angel_shield:IsPurgable() return false end
function modifier_extended_guardian_angel_shield:IsDebuff() return false end