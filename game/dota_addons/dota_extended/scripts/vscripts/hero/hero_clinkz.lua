...
function extended_clinkz_strafe:IsNetherWardStealable() return false end
...
function extended_clinkz_strafe:GetManaCost(level)
    local caster = self:GetCaster()
    -- If Clinkz is currently mounted, remove the mana cost
    if caster:HasModifier("modifier_extended_strafe_mount") then
        return 0    
    end
...
function SetArrowAttackProjectile(caster, searing_arrows)    
    -- modifiers
    local skadi_modifier = "modifier_item_extended_skadi"
    local deso_modifier = "modifier_item_extended_desolator" 
