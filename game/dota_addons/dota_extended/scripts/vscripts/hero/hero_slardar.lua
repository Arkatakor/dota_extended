...
function modifier_extended_guardian_sprint_buff:OnCreated()
	...
	self.modifier_talent_slow = "modifier_extended_guardian_sprint_aspd_slow"
	self.modifier_rain = "modifier_extended_rain_cloud_buff"	
...
function modifier_extended_guardian_sprint_buff:OnIntervalThink()
	if IsServer() then		
		...
		if self.caster:HasTalent("special_bonus_extended_slardar_5") then
		...
		if self.caster:HasTalent("special_bonus_extended_slardar_2") then
...
function modifier_extended_rip_current_movement:OnCreated()
	if IsServer() then
		...
		self.modifier_stun = "modifier_extended_rip_current_stun"
		self.modifier_slow = "modifier_extended_rip_current_slow"
...
function modifier_extended_rip_current_movement:CheckState()
...
function modifier_extended_rip_current_movement:RemoveOnDeath()
	return false
end

function modifier_extended_rip_current_movement:IgnoreTenacity()
	return true
end
...
function modifier_extended_rip_current_movement:OnRemoved()
	if IsServer() then		
		...
		radius = radius + self.caster:FindSpecificTalentValue("special_bonus_extended_slardar_1", "radius")
		damage = damage + self.caster:FindSpecificTalentValue("special_bonus_extended_slardar_1", "damage")
...
function modifier_extended_bash_of_the_deep_stun:GetEffectName()
	return "particles/generic_gameplay/generic_stunned.vpcf"	
end

function modifier_extended_bash_of_the_deep_stun:GetEffectAttachType()
...
function extended_slardar_corrosive_haze:OnInventoryContentsChanged()
	-- Checks if Slardar now has a scepter, or still has it.
	if IsServer() then
		local caster = self:GetCaster()		
		local rain_ability = "extended_slardar_rain_cloud"
...
function modifier_extended_rain_cloud_dummy:IgnoreTenacity()
	return true
end
