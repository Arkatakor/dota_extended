...
--			TRANSPOSITION
-------------------------------------------
LinkLuaModifier("modifier_extended_telekinesis", "hero/hero_rubick", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier("modifier_extended_telekinesis_stun", "hero/hero_rubick", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_telekinesis_caster", "hero/hero_rubick", LUA_MODIFIER_MOTION_NONE)

extended_rubick_telekinesis = class({})
function extended_rubick_telekinesis:IsHiddenWhenStolen() return false end
function extended_rubick_telekinesis:IsRefreshable() return true end
function extended_rubick_telekinesis:IsStealable() return true end
function extended_rubick_telekinesis:IsNetherWardStealable() return true end
-------------------------------------------

function extended_rubick_telekinesis:OnSpellStart( params )
	local caster = self:GetCaster()
	-- Handler on lifted targets
	if caster:HasModifier("modifier_extended_telekinesis_caster") then
	...
	-- Handler on regular 
		...
		if self.target:GetTeam() ~= caster:GetTeam() then
			...
			self.target:AddNewModifier(caster, self, "modifier_extended_telekinesis_stun", { duration = duration })
			is_ally = false
		else
		...
		self.target_modifier = self.target:AddNewModifier(caster, self, "modifier_extended_telekinesis", { duration = duration })
		...
		-- Add caster handler
		caster:AddNewModifier(caster, self, "modifier_extended_telekinesis_caster", { duration = duration + FrameTime()})
		
		self:EndCooldown()
	end
end

function extended_rubick_telekinesis:GetAbilityTextureName()
	if self:GetCaster():HasModifier("modifier_extended_telekinesis_caster") then
		return "rubick_telekinesis_land"
	end
	return "rubick_telekinesis"
end

function extended_rubick_telekinesis:GetBehavior()
	if self:GetCaster():HasModifier("modifier_extended_telekinesis_caster") then
...
function extended_rubick_telekinesis:GetManaCost( target )
	if self:GetCaster():HasModifier("modifier_extended_telekinesis_caster") then
		return 0
	else
...
function extended_rubick_telekinesis:GetCastRange( location , target)
	if self:GetCaster():HasModifier("modifier_extended_telekinesis_caster") then
		return 25000
	end
...
function extended_rubick_telekinesis:CastFilterResultTarget( target )
	if IsServer() then
		local caster = self:GetCaster()
...
-------------------------------------------
modifier_extended_telekinesis_caster = class({})
function modifier_extended_telekinesis_caster:IsDebuff() return false end
function modifier_extended_telekinesis_caster:IsHidden() return true end
function modifier_extended_telekinesis_caster:IsPurgable() return false end
function modifier_extended_telekinesis_caster:IsPurgeException() return false end
function modifier_extended_telekinesis_caster:IsStunDebuff() return false end
-------------------------------------------

function modifier_extended_telekinesis_caster:OnDestroy()
...
-------------------------------------------
modifier_extended_telekinesis = class({})
function modifier_extended_telekinesis:IsDebuff() 
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then return true end
	return false 
end
function modifier_extended_telekinesis:IsHidden() return false end
function modifier_extended_telekinesis:IsPurgable() return false end
function modifier_extended_telekinesis:IsPurgeException() return false end
function modifier_extended_telekinesis:IsStunDebuff() return false end
function modifier_extended_telekinesis:IgnoreTenacity() return true end
-------------------------------------------

function modifier_extended_telekinesis:OnCreated( params )
	if IsServer() then
		 -- Ability properties
		local caster = self:GetCaster()
		local ability = self:GetAbility()
...
function modifier_extended_telekinesis:OnDestroy( params )
	if IsServer() then
		local caster = self:GetCaster()
...
function modifier_extended_telekinesis:UpdateVerticalMotion(unit, dt)
    if IsServer() then
...
function modifier_extended_telekinesis:UpdateHorizontalMotion(unit, dt)
	if IsServer() then
		-- If the unit being lifted died, interrupt motion controllers and remove self
 		if not self.parent:IsAlive() then
 			self.parent:InterruptMotionControllers(true)
...
function modifier_extended_telekinesis:OnRemoved()
	if IsServer() then
		 -- Ability properties
		local ability = self:GetAbility()
		local impact_radius = ability:GetSpecialValueFor("impact_radius")
...
function modifier_extended_telekinesis:GetTexture()
	return "rubick_telekinesis"
end

-------------------------------------------
modifier_extended_telekinesis_stun = class({})
function modifier_extended_telekinesis_stun:IsDebuff() return true end
function modifier_extended_telekinesis_stun:IsHidden() return true end
function modifier_extended_telekinesis_stun:IsPurgable() return false end
function modifier_extended_telekinesis_stun:IsPurgeException() return false end
function modifier_extended_telekinesis_stun:IsStunDebuff() return true end
-------------------------------------------

function modifier_extended_telekinesis_stun:DeclareFunctions()
	local decFuns =
	{
...
function modifier_extended_telekinesis_stun:GetOverrideAnimation()
	return ACT_DOTA_FLAIL
end

function modifier_extended_telekinesis_stun:CheckState()
    local state = 
	{
