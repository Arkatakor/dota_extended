...
MergeTables(LinkedModifiers,{
	["modifier_extended_acid_spray_debuff_dot"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_acid_spray_thinker"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_acid_spray_handler"] = LUA_MODIFIER_MOTION_NONE,
})

extended_alchemist_acid_spray = extended_alchemist_acid_spray or class ({})

function extended_alchemist_acid_spray:GetCastRange(location, target)
    return self.BaseClass.GetCastRange(self, location, target)
end

function extended_alchemist_acid_spray:IsHiddenWhenStolen()
    local caster = self:GetCaster()
    if caster:HasAbility("extended_alchemist_chemical_rage") then
        return true
    end
    return false
end

function extended_alchemist_acid_spray:GetAOERadius()
    local caster = self:GetCaster()
    local radius = self:GetSpecialValueFor("radius")    

    -- #1 Talent: Acid Spray radius increase
    radius = radius + caster:FindTalentValue("special_bonus_extended_alchemist_1")
    
    return radius
end

function extended_alchemist_acid_spray:OnSpellStart()
    local caster = self:GetCaster()
    local ability = self
    ...
    local thinker = CreateModifierThinker(caster, self, "modifier_extended_acid_spray_thinker", {duration = duration}, point, team_id, false)
    return true
end

modifier_extended_acid_spray_thinker = modifier_extended_acid_spray_thinker or class({})

function modifier_extended_acid_spray_thinker:IsAura()
    return true
end

function modifier_extended_acid_spray_thinker:OnCreated(keys)
    if IsServer() then
        self.caster = self:GetCaster()
        self.thinker = self:GetParent()
        self.ability = self:GetAbility()        
        self.modifier_spray = "modifier_extended_acid_spray_handler"
        ...
        -- #1 Talent: Acid Spray radius increase
        self.radius = self.radius + self.caster:FindTalentValue("special_bonus_extended_alchemist_1")
...
function modifier_extended_acid_spray_thinker:OnIntervalThink()
...
function modifier_extended_acid_spray_thinker:GetAuraRadius()
    return self.radius
end

function modifier_extended_acid_spray_thinker:GetAuraSearchTeam()    
    return self.ability:GetAbilityTargetTeam()
end

function modifier_extended_acid_spray_thinker:GetAuraSearchType()    
    return self.ability:GetAbilityTargetType()
end

function modifier_extended_acid_spray_thinker:GetAuraSearchFlags()    
    return self.ability:GetAbilityTargetFlags()
end

function modifier_extended_acid_spray_thinker:GetModifierAura()
    return "modifier_extended_acid_spray_handler"
end


function modifier_extended_acid_spray_thinker:OnDestroy(keys)
    if IsServer() then
...
modifier_extended_acid_spray_handler = modifier_extended_acid_spray_handler or class({})

function modifier_extended_acid_spray_handler:IsHidden()
    return true
end

function modifier_extended_acid_spray_handler:OnCreated()
    if IsServer() then
        local caster = self:GetCaster()
        local ability = self:GetAbility()
        local unit = self:GetParent()

        self.modifier = unit:AddNewModifier(caster, ability, "modifier_extended_acid_spray_debuff_dot", {})
...
function modifier_extended_acid_spray_handler:OnIntervalThink()
    if IsServer() then
...
modifier_extended_acid_spray_debuff_dot = modifier_extended_acid_spray_debuff_dot or class({})

function modifier_extended_acid_spray_debuff_dot:IsDebuff()
    return true
end

function modifier_extended_acid_spray_debuff_dot:IsPurgable()
    return true
end

function modifier_extended_acid_spray_debuff_dot:OnCreated()
    self.caster = self:GetCaster()
    local ability = self:GetAbility()
    ...
    self.armor_reduction = self.armor_reduction + self.caster:FindTalentValue("special_bonus_extended_alchemist_2")   
end

function modifier_extended_acid_spray_debuff_dot:OnIntervalThink(aura_tick, consume_stacks)
    if IsServer() then
        if aura_tick then
        ...
        if self.caster:IsIllusion() then --prevent ability from becoming nil if the illusion is dead for to long
            if not self.caster:IsAlive() then
                ...
                self.ability = self.caster:FindAbilityByName("extended_alchemist_acid_spray")
            end
        end
        ...
        if aura_tick
        or consume_stacks
        or (not unit:HasModifier("modifier_extended_acid_spray_handler")
        and not unit:HasModifier("modifier_extended_chemical_rage_aura")) then
            ...
            if self:GetStackCount() == 0 then 
                ...
                unit:RemoveModifierByName("modifier_extended_acid_spray_debuff_dot")
                return
            end
            if consume_stacks then
...
function modifier_extended_acid_spray_debuff_dot:OnStackCountChanged(old_stack_count)
    self.caster = self.caster or self:GetCaster()
    self.ability = self.ability or self:GetAbility()
    if self.caster:IsIllusion() then --prevent ability from becoming nil if the illusion is dead for to long
        if not self.caster:IsAlive() then
    ...
    if self.caster:HasModifier("modifier_extended_chemical_rage_buff_haste") then
...
function modifier_extended_acid_spray_debuff_dot:GetTexture()
    return "alchemist_acid_spray" --prevent texture from dissapearing if the illusion is dead for to long
end

function modifier_extended_acid_spray_debuff_dot:DeclareFunctions()
    return {MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS,}
end

function modifier_extended_acid_spray_debuff_dot:GetModifierPhysicalArmorBonus()
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_unstable_concoction_stunned"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_unstable_concoction_handler"] = LUA_MODIFIER_MOTION_NONE,
})
extended_alchemist_unstable_concoction = extended_alchemist_unstable_concoction or class({})

function extended_alchemist_unstable_concoction:GetCastRange(location, target)
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
        return self.BaseClass.GetCastRange(self, location, target)
    end
    return 0
end

function extended_alchemist_unstable_concoction:IsHiddenWhenStolen()
    return false
end

function extended_alchemist_unstable_concoction:OnUnStolen()
    local caster = self:GetCaster()
    caster:RemoveModifierByName("modifier_extended_unstable_concoction_handler")
end

function extended_alchemist_unstable_concoction:OnSpellStart()
    local caster = self:GetCaster()
    ...
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
        ...
        local modifier_unstable_handler = caster:FindModifierByName("modifier_extended_unstable_concoction_handler")
    ...
    local greed_modifier = caster:FindModifierByName("modifier_extended_goblins_greed_passive")
    ...
    duration = duration + caster:FindTalentValue("special_bonus_extended_alchemist_3")
    
    caster:AddNewModifier(caster, self, "modifier_extended_unstable_concoction_handler", {duration = duration,})
...
function extended_alchemist_unstable_concoction:OnProjectileHit(target, location)
    local caster = self:GetCaster()
            ...
            if not target:IsMagicImmune() then
                if not target:IsInvulnerable() then
                    if not target:IsOutOfGame() then
                        ...
                        target:AddNewModifier(caster, self, "modifier_extended_unstable_concoction_stunned", {duration = stun_duration,})
                    end
                end
            end
        else
    ...
    for _,unit in pairs(units) do
        ...
        unit:AddNewModifier(caster, self, "modifier_extended_unstable_concoction_stunned", {duration = stun_duration,})
        ...
        if unit:HasModifier("modifier_extended_acid_spray_handler") then
            local acid_spray_modifier = unit:FindModifierByName("modifier_extended_acid_spray_handler")
            ...
            damage_multiplier = damage_multiplier + caster:FindTalentValue("special_bonus_extended_alchemist_4")
            ...
            for _,acid_spray_unit in pairs(acid_spray_units) do
                ...
                local modifier = acid_spray_unit:FindModifierByName("modifier_extended_acid_spray_debuff_dot")
                if modifier then
                    modifier:OnIntervalThink(false, true)
                end
            end
            local modifier = unit:FindModifierByName("modifier_extended_acid_spray_debuff_dot")
            if modifier then
                modifier:OnIntervalThink(false, true)
            end
        end        
...
function extended_alchemist_unstable_concoction:GetAbilityTextureName()
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
...
function extended_alchemist_unstable_concoction:GetCooldown(level)
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
        if IsServer() then
...
function extended_alchemist_unstable_concoction:GetManaCost(level)
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
        return 0
    end
...
function extended_alchemist_unstable_concoction:GetCastTime()
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
...
function extended_alchemist_unstable_concoction:GetBehavior()
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
...
function extended_alchemist_unstable_concoction:ProcsMagicStick()
    local caster = self:GetCaster()
    if caster:HasModifier("modifier_extended_unstable_concoction_handler") then
        return false
    end
...
function extended_alchemist_unstable_concoction:GetAOERadius()
    local caster = self:GetCaster()
    local brew_start = 0
...
modifier_extended_unstable_concoction_handler = modifier_extended_unstable_concoction_handler or class({})

function modifier_extended_unstable_concoction_handler:IsPurgable()
    return false
end


function modifier_extended_unstable_concoction_handler:IsHidden()
    return true
end

function modifier_extended_unstable_concoction_handler:OnDestroy()
    local caster = self:GetCaster()
    local ability = self:GetAbility()
    if IsServer() then
...
function modifier_extended_unstable_concoction_handler:OnCreated()
    self:StartIntervalThink(FrameTime())
end

function modifier_extended_unstable_concoction_handler:OnIntervalThink()
    if IsServer() then
...
modifier_extended_unstable_concoction_stunned = modifier_extended_unstable_concoction_stunned or class({})
function modifier_extended_unstable_concoction_stunned:CheckState()
	local state =
		{[MODIFIER_STATE_STUNNED] = true}
	return state	
end

function modifier_extended_unstable_concoction_stunned:IsPurgable() return false end
function modifier_extended_unstable_concoction_stunned:IsPurgeException() return false end
function modifier_extended_unstable_concoction_stunned:IsStunDebuff() return true end
function modifier_extended_unstable_concoction_stunned:IsHidden() return false end
function modifier_extended_unstable_concoction_stunned:GetEffectName() return "particles/generic_gameplay/generic_stunned.vpcf" end
function modifier_extended_unstable_concoction_stunned:GetEffectAttachType() return PATTACH_OVERHEAD_FOLLOW end
-------------------------------------------
--		GOBLINS GREED & GREEVILS GREED
-------------------------------------------
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_goblins_greed_passive"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_goblins_greed_mark"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_goblins_greed_vision"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_greevils_greed_handler"] = LUA_MODIFIER_MOTION_NONE,
})
extended_alchemist_goblins_greed = extended_alchemist_goblins_greed or class ({})

function extended_alchemist_goblins_greed:IsStealable()
    return false
end

function extended_alchemist_goblins_greed:OnInventoryContentsChanged()
    -- Checks if Alchemist now has a scepter, or still has it.
    if IsServer() then
        local caster = self:GetCaster()     
        local mammonite_ability = "extended_alchemist_mammonite"        
...
function extended_alchemist_goblins_greed:GetCastRange(location, target)
    return self.BaseClass.GetCastRange(self, location, target)
end

function extended_alchemist_goblins_greed:GetCooldown(level)
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #8 Talent: Halves Greevil's Greed's cooldown
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_alchemist_8")    
    return cooldown
end

function extended_alchemist_goblins_greed:GetIntrinsicModifierName()
    return "modifier_extended_goblins_greed_passive"
end

function extended_alchemist_goblins_greed:OnUpgrade()
    local caster = self:GetCaster()
...
function extended_alchemist_goblins_greed:OnSpellStart()
    local caster = self:GetCaster()
    if caster:IsIllusion() then --skip greevil movement if hybrid casts it cus only alch gets a pet greevil
        target:AddNewModifier(caster, self, "modifier_extended_goblins_greed_mark", {})
        return
    end
    local target = self:GetCursorTarget()
    self.greevil:CastAbilityOnTarget(target, self.greevil_ability, caster:GetPlayerID())
    self.greevil.target = target
    target:AddNewModifier(caster, self, "modifier_extended_goblins_greed_vision", {})
end

modifier_extended_goblins_greed_passive = modifier_extended_goblins_greed_passive or class ({})

function modifier_extended_goblins_greed_passive:RemoveOnDeath()
    return false
end

function modifier_extended_goblins_greed_passive:IsPermanent()
    return true
end

function modifier_extended_goblins_greed_passive:GetAttributes()
    return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end

function modifier_extended_goblins_greed_passive:OnCreated()
    if IsServer() then
        local caster = self:GetCaster()        
        if not caster:IsIllusion() then
            local ability = self:GetAbility()
            ability.greevil = CreateUnitByName("npc_extended_alchemist_greevil", caster:GetAbsOrigin(), true, caster, caster, caster:GetTeam())
            ability.greevil:SetOwner(caster)
            ability.greevil_ability = ability.greevil:FindAbilityByName("extended_alchemist_greevils_greed")
...
function modifier_extended_goblins_greed_passive:DeclareFunctions()
    return {
        MODIFIER_EVENT_ON_DEATH,
    }
end

function modifier_extended_goblins_greed_passive:OnDeath(keys)
    local caster = self:GetCaster()
    local ability = self:GetAbility()
    ...
    if caster == attacker and caster:GetTeamNumber() ~= unit:GetTeamNumber() or unit:HasModifier("modifier_extended_goblins_greed_mark") then
...
modifier_extended_goblins_greed_vision = modifier_extended_goblins_greed_vision or class({})

function modifier_extended_goblins_greed_vision:IsHidden()
    return true
end

function modifier_extended_goblins_greed_vision:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_PROVIDES_FOW_POSITION,
    }
end

function modifier_extended_goblins_greed_vision:GetModifierProvidesFOWVision()
    return 1
end

modifier_extended_goblins_greed = modifier_extended_goblins_greed or class({})

extended_alchemist_greevils_greed = extended_alchemist_greevils_greed or class ({})

function extended_alchemist_greevils_greed:GetCastRange()
    return 1
end

function extended_alchemist_greevils_greed:OnSpellStart()
    local caster = self:GetCaster()
    local target = self:GetCursorTarget()
    local owner = caster:GetOwner()

    local greed_ability = owner:FindAbilityByName("extended_alchemist_goblins_greed")
    ...
    if not target:TriggerSpellAbsorb(self) then
        target:RemoveModifierByName("modifier_extended_goblins_greed_vision")        
        target:AddNewModifier(caster, self, "modifier_extended_goblins_greed_mark", {duration = greed_duration})
...
function extended_alchemist_greevils_greed:GetIntrinsicModifierName()
    return "modifier_extended_greevils_greed_handler"
end

modifier_extended_greevils_greed_handler = modifier_extended_greevils_greed_handler or class ({})

function modifier_extended_greevils_greed_handler:OnCreated()
    if IsServer() then
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_greevils_greed_handler:OnIntervalThink()
    if IsServer() then        
...
function modifier_extended_greevils_greed_handler:CheckState()
    return {
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_chemical_rage_buff_haste"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
    ["modifier_extended_chemical_rage_handler"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_chemical_rage_aura"] = LUA_MODIFIER_MOTION_NONE,
})
extended_alchemist_chemical_rage = extended_alchemist_chemical_rage or class ({})

function extended_alchemist_chemical_rage:IsHiddenWhenStolen()
    return false
end

function extended_alchemist_chemical_rage:GetAssociatedSecondaryAbilities()
    return "extended_alchemist_acid_spray"
end

function extended_alchemist_chemical_rage:OnSpellStart()
    local caster = self:GetCaster()
    ...
    caster:AddNewModifier(caster, self, "modifier_extended_chemical_rage_handler", {})
    caster:EmitSound("Hero_Alchemist.ChemicalRage.Cast")
end

modifier_extended_chemical_rage_handler = modifier_extended_chemical_rage_handler or class ({})

function modifier_extended_chemical_rage_handler:IsHidden()
    return true
end

function modifier_extended_chemical_rage_handler:OnCreated()
    if IsServer() then
        local caster = self:GetCaster()
...
function modifier_extended_chemical_rage_handler:CheckState()
    return {
        [MODIFIER_STATE_INVULNERABLE] = true,
    }
end

function modifier_extended_chemical_rage_handler:OnDestroy()
    if IsServer() then
        local caster = self:GetCaster()
        local ability = self:GetAbility()
        if caster:HasModifier("modifier_extended_chemical_rage_buff_haste") then
            caster:RemoveModifierByName("modifier_extended_chemical_rage_buff_haste")
        end
        caster:AddNewModifier(caster, ability, "modifier_extended_chemical_rage_buff_haste", {})
    end
end

modifier_extended_chemical_rage_buff_haste = modifier_extended_chemical_rage_buff_haste or class ({})

function modifier_extended_chemical_rage_buff_haste:AllowIllusionDuplicate()
    return true
end

function modifier_extended_chemical_rage_buff_haste:OnCreated()
    if IsServer() then
        local caster = self:GetCaster()
        local ability = self:GetAbility()
        local particle_acid_aura = "particles/hero/alchemist/chemical_rage_acid_aura.vpcf"

        self.ability = caster:FindAbilityByName("extended_alchemist_acid_spray")
        self.bat_change = self:GetAbility():GetSpecialValueFor("base_attack_time")
        self.radius = self.ability:GetSpecialValueFor("radius")

        -- #1 Talent: Acid Spray radius increase
        self.radius = self.radius + caster:FindTalentValue("special_bonus_extended_alchemist_1")        
        ...
        duration = duration + caster:FindTalentValue("special_bonus_extended_alchemist_5")
        
        self:SetDuration(duration, false)
    end
end

function modifier_extended_chemical_rage_buff_haste:OnDestroy()
    if IsServer() then
        local caster = self:GetCaster()
        if caster:GetUnitName() == "npc_dota_hero_alchemist" then
...
function modifier_extended_chemical_rage_buff_haste:IsAura()
    return true
end

function modifier_extended_chemical_rage_buff_haste:GetAuraRadius()
    return self.radius
end

function modifier_extended_chemical_rage_buff_haste:GetAuraSearchTeam()
    return self.ability:GetAbilityTargetTeam()
end

function modifier_extended_chemical_rage_buff_haste:GetAuraSearchType()
    return self.ability:GetAbilityTargetType()
end

function modifier_extended_chemical_rage_buff_haste:GetAuraSearchFlags()
    return self.ability:GetAbilityTargetFlags()
end

function modifier_extended_chemical_rage_buff_haste:GetModifierAura()
    return "modifier_extended_chemical_rage_aura"
end

function modifier_extended_chemical_rage_buff_haste:DeclareFunctions()
    local table = {
...
function modifier_extended_chemical_rage_buff_haste:GetActivityTranslationModifiers()
    return "chemical_rage"
end

function modifier_extended_chemical_rage_buff_haste:GetAttackSound()
	return "Hero_Alchemist.ChemicalRage.Attack"
end

function modifier_extended_chemical_rage_buff_haste:GetModifierBaseManaRegen()
    local ability = self:GetAbility()
    return ability:GetSpecialValueFor("bonus_mana_regen")
end

function modifier_extended_chemical_rage_buff_haste:GetModifierConstantHealthRegen()
    local caster = self:GetCaster()
    local ability = self:GetAbility()
    local regen = ability:GetSpecialValueFor("bonus_health_regen")

    -- #6 Talent: Chemical Rage health regeneration increase
    regen = regen + caster:FindTalentValue("special_bonus_extended_alchemist_6")    
    return regen
end

function modifier_extended_chemical_rage_buff_haste:GetModifierMoveSpeedBonus_Constant()
    local ability = self:GetAbility()
    return ability:GetSpecialValueFor("bonus_movespeed")
end

function modifier_extended_chemical_rage_buff_haste:GetModifierBaseAttackTimeConstant()
	return self.bat_change
end

function modifier_extended_chemical_rage_buff_haste:GetEffectName()
    return "particles/units/heroes/hero_alchemist/alchemist_chemical_rage.vpcf"
end

function modifier_extended_chemical_rage_buff_haste:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_chemical_rage_buff_haste:GetStatusEffectName()
    return "particles/status_fx/status_effect_chemical_rage.vpcf"
end

function modifier_extended_chemical_rage_buff_haste:StatusEffectPriority()
    return 10
end

function modifier_extended_chemical_rage_buff_haste:GetHeroEffectName()
    return "particles/units/heroes/hero_alchemist/alchemist_chemical_rage_hero_effect.vpcf"
end

function modifier_extended_chemical_rage_buff_haste:HeroEffectPriority()
    return 10
end

-- Chemical Rage Acid aura
modifier_extended_chemical_rage_aura = modifier_extended_chemical_rage_aura or class({})

function modifier_extended_chemical_rage_aura:IsDebuff()
    return true
end

function modifier_extended_chemical_rage_aura:IsHidden()
    return true
end

function modifier_extended_chemical_rage_aura:OnCreated()
    if IsServer() then
        local caster = self:GetCaster()        
        local unit = self:GetParent()
        self.ability = caster:FindAbilityByName("extended_alchemist_acid_spray")
        self.modifier = unit:FindModifierByName("modifier_extended_acid_spray_debuff_dot")
...
function modifier_extended_chemical_rage_aura:OnRefresh()
    self:OnCreated()
end

function modifier_extended_chemical_rage_aura:OnIntervalThink()
    if IsServer() then
        if self.modifier:IsNull() then
            return --volvo pls
        end
...
extended_alchemist_mammonite = extended_alchemist_mammonite or class({})

function extended_alchemist_mammonite:OnToggle() return end    

function extended_alchemist_mammonite:GetIntrinsicModifierName()
