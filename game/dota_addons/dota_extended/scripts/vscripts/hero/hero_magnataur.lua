...
LinkLuaModifier("modifier_extended_polarize_debuff", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
modifier_extended_polarize_debuff = modifier_extended_polarize_debuff or class({})

function modifier_extended_polarize_debuff:IsDebuff()
	return true
end

function modifier_extended_polarize_debuff:IsPurgable()
	return true
end

function modifier_extended_polarize_debuff:IsHidden()
	return false
end

function modifier_extended_polarize_debuff:GetTexture()
	return "custom/magnus_polarize"
end

function modifier_extended_polarize_debuff:GetEffectName()
	return "particles/econ/items/magnataur/seismic_berserker/seismic_berserker_weapon_horn_b.vpcf"
end

function modifier_extended_polarize_debuff:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

LinkLuaModifier("modifier_extended_polarize_debuff_stack", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
modifier_extended_polarize_debuff_stack = modifier_extended_polarize_debuff_stack or class({})

function modifier_extended_polarize_debuff_stack:IsDebuff()
	return true
end

function modifier_extended_polarize_debuff_stack:IsHidden()
	return true
end

function modifier_extended_polarize_debuff_stack:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_extended_polarize_debuff_stack:OnCreated( params )
	if IsServer() then
		local parent = self:GetParent()
		local caster = self:GetCaster()

		if parent:HasModifier("modifier_extended_polarize_debuff") then
			local modifier = parent:FindModifierByName("modifier_extended_polarize_debuff")
			modifier:IncrementStackCount()
			local max_duration = params.duration
			local modifier_counter = parent:FindAllModifiersByName("modifier_extended_polarize_debuff_stack")
			...
			parent:AddNewModifier(caster, nil, "modifier_extended_polarize_debuff", {duration = params.duration})
			parent:FindModifierByName("modifier_extended_polarize_debuff"):SetStackCount(1)
		end
	end
end

function modifier_extended_polarize_debuff_stack:OnDestroy( )
	if IsServer() then
		local parent = self:GetParent()
		if parent:HasModifier("modifier_extended_polarize_debuff") then
			parent:FindModifierByName("modifier_extended_polarize_debuff"):DecrementStackCount()
		end
	end
end
...
extended_magnataur_shockwave = extended_magnataur_shockwave or class({})

function extended_magnataur_shockwave:OnAbilityPhaseStart()
	if IsServer() then
		local caster = self:GetCaster()
...
function extended_magnataur_shockwave:OnAbilityPhaseInterrupted()
	if IsServer() then
...
function extended_magnataur_shockwave:IsStealable()
	return true
end

function extended_magnataur_shockwave:IsHiddenWhenStolen()
	return false
end

function extended_magnataur_shockwave:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
...
function extended_magnataur_shockwave:OnProjectileHit_ExtraData(target, location, ExtraData)
	if target then
		local caster = self:GetCaster()
		local was_hit = false
		...
		if was_hit then
			...
			target:AddNewModifier(caster, nil, "modifier_extended_polarize_debuff_stack", {duration = ExtraData.polarize_duration})
		else
			-- Apply full damage
			...
			if (self[ExtraData.index .. "_counter"] > 0) and target:HasModifier("modifier_extended_polarize_debuff") and target:IsRealHero() then
...
function extended_magnataur_shockwave:GetCooldown( nLevel )
	return self.BaseClass.GetCooldown( self, nLevel ) - self:GetCaster():FindTalentValue("special_bonus_extended_magnataur_5")
end
...
extended_magnataur_empower = extended_magnataur_empower or class({})
LinkLuaModifier("modifier_extended_empower_aura", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_empower", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_empower_particle", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_supercharged", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_empower_linger", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)

function extended_magnataur_empower:OnAbilityPhaseStart()
	self:GetCaster():EmitSound("Hero_Magnataur.Empower.Cast")
	return true
end

function extended_magnataur_empower:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		...
		if target:HasModifier("modifier_extended_empower") then
			target:AddNewModifier(caster, self, "modifier_extended_supercharged", { duration = supercharge_duration})
		end
		target:AddNewModifier(caster, self, "modifier_extended_empower", { duration = empower_duration})
...
function extended_magnataur_empower:GetIntrinsicModifierName()
    return "modifier_extended_empower_aura"
end

function extended_magnataur_empower:IsHiddenWhenStolen()
	return false
end

function extended_magnataur_empower:IsStealable()
	return true
end

-- Scepter Aura & self-sustain
modifier_extended_empower_aura = modifier_extended_empower_aura or class({})

function modifier_extended_empower_aura:GetAuraEntityReject(target)
    if IsServer() then
		local parent = self:GetParent()
...
function modifier_extended_empower_aura:GetAuraRadius()
...
function modifier_extended_empower_aura:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD + DOTA_UNIT_TARGET_FLAG_INVULNERABLE
end

function modifier_extended_empower_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_empower_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO
end

function modifier_extended_empower_aura:GetModifierAura()
	return "modifier_extended_empower"
end

function modifier_extended_empower_aura:IsAura()
	return true
end

function modifier_extended_empower_aura:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT
end

function modifier_extended_empower_aura:IsHidden()
	return true
end

modifier_extended_empower = modifier_extended_empower or class({})

function modifier_extended_empower:DeclareFunctions()
	local decFuncs =
	{
...
function modifier_extended_empower:GetModifierBaseDamageOutgoing_Percentage()
	if self:GetCaster():IsRealHero() then
		return self:GetAbility():GetSpecialValueFor("bonus_damage_pct") + self:GetCaster():FindSpecificTalentValue("special_bonus_extended_magnataur_3", "bonus_damage_pct")
	end
	return 0
end

function modifier_extended_empower:OnAttackLanded( params )
	...
	if IsServer() then
		...
		if params.attacker == parent and ( not parent:IsIllusion() ) then
			...
			if parent:HasModifier("modifier_extended_supercharged") then
			...
			if params.attacker:IsRangedAttacker() then
				-- Ranged-Attacker
				local cleave_damage_ranged = (ability:GetSpecialValueFor("cleave_damage_ranged") + caster:FindSpecificTalentValue("special_bonus_extended_magnataur_3", "cleave_damage_ranged")) / 100
				...
				-- Melee-Attacker
				local cleave_damage_pct = (ability:GetSpecialValueFor("cleave_damage_pct") + caster:FindSpecificTalentValue("special_bonus_extended_magnataur_3", "cleave_damage_pct")) / 100
...
function modifier_extended_empower:IsDebuff()
	return false
end

function modifier_extended_empower:IsPurgable()
	return true
end

function modifier_extended_empower:IsHidden()
	return false
end

function modifier_extended_empower:OnCreated( params )
	if IsServer() then
		local parent = self:GetParent()
		local caster = self:GetCaster()
		parent:AddNewModifier(caster, self:GetAbility(), "modifier_extended_empower_particle", {})
...
function modifier_extended_empower:OnRefresh( params )
	if IsServer() then
		local parent = self:GetParent()
		local caster = self:GetCaster()
		parent:AddNewModifier(caster, self:GetAbility(), "modifier_extended_empower_particle", {})
		if caster ~= parent then
			self.remaining_duration = params.duration
			parent:RemoveModifierByName("modifier_extended_empower_linger")
		end
		parent:EmitSound("Hero_Magnataur.Empower.Target")
	end
end

function modifier_extended_empower:OnDestroy( )
	if IsServer() then
		if self:GetCaster() ~= self:GetParent() then
			if self:GetDuration() == 0.5 then
				...
				if parent:HasModifier("modifier_extended_empower_linger") then
					local linger_remaining = parent:FindModifierByName("modifier_extended_empower_linger"):GetRemainingTime()
				...
				parent:AddNewModifier(self:GetCaster(), ability, "modifier_extended_empower", { duration = linger_duration, was_refreshed = true})
			end
		end
	end
end

function modifier_extended_empower:OnIntervalThink( )
	if IsServer() then
		...
		if self:GetDuration() == 0.5 then
			if not parent:HasModifier("modifier_extended_empower_linger") then
				parent:AddNewModifier(caster, ability, "modifier_extended_empower_linger", { duration = self.remaining_duration})
			end
		else
			self.remaining_duration = self.remaining_duration - self.interval
		end
		if not (parent:HasModifier("modifier_extended_empower_particle") or parent:HasModifier("modifier_extended_supercharged")) then
			parent:AddNewModifier(caster, ability, "modifier_extended_empower_particle", {})
		end
	end
end

modifier_extended_empower_particle = modifier_extended_empower_particle or class({})

function modifier_extended_empower_particle:IsHidden()
	return true
end

function modifier_extended_empower_particle:OnCreated( )
	local parent = self:GetParent()
	if parent:HasModifier("modifier_extended_supercharged") or not parent:HasModifier("modifier_extended_empower") then
		self:Destroy()
		return
	end
	if not self.particle then
...
function modifier_extended_empower_particle:OnIntervalThink( )
	self:OnCreated()
end

function modifier_extended_empower_particle:OnRefresh()
	self:OnCreated()
end

function modifier_extended_empower_particle:IsPurgable()
	return true
end

modifier_extended_empower_linger = modifier_extended_empower_linger or class({})

function modifier_extended_empower_linger:IsDebuff()
	return false
end

function modifier_extended_empower_linger:IsHidden()
	return true
end

function modifier_extended_empower_linger:IsPurgable()
	return true
end

modifier_extended_supercharged = modifier_extended_supercharged or class({})

function modifier_extended_supercharged:OnCreated( params )
	if IsServer() then
...
function modifier_extended_supercharged:OnDestroy()
	if IsServer() then
		local parent = self:GetParent()
		if parent:HasModifier("modifier_extended_empower") then
			parent:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_empower_particle", {})
		end
	end
end

function modifier_extended_supercharged:IsDebuff()
	return false
end

function modifier_extended_supercharged:IsHidden()
	return false
end

function modifier_extended_supercharged:GetEffectName()
	return "particles/hero/magnataur/magnataur_supercharge.vpcf"
end

function modifier_extended_supercharged:GetEffectAttachType()
	return PATTACH_POINT_FOLLOW
end

function modifier_extended_supercharged:GetTexture()
	return "custom/magnus_supercharge"
end

function modifier_extended_supercharged:DeclareFunctions()
	local decFuncs =
	{
...
function modifier_extended_supercharged:GetModifierAttackSpeedBonus_Constant()
	return self:GetAbility():GetSpecialValueFor("supercharge_as") + self:GetCaster():FindSpecificTalentValue("special_bonus_extended_magnataur_6", "bonus_as")
end

function modifier_extended_supercharged:GetModifierMoveSpeedBonus_Percentage()
	return self:GetAbility():GetSpecialValueFor("supercharge_ms") + self:GetCaster():FindSpecificTalentValue("special_bonus_extended_magnataur_6", "bonus_ms")
end
...
extended_magnataur_skewer = extended_magnataur_skewer or class({})
LinkLuaModifier("modifier_extended_skewer_motion_controller", "hero/hero_magnataur", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_extended_skewer_motion_controller_linger", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_skewer_motion_controller_target", "hero/hero_magnataur", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_extended_skewer_slow", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_skewer_entangle", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)

function extended_magnataur_skewer:OnAbilityPhaseStart()
	local caster = self:GetCaster()
	if caster:HasModifier("modifier_extended_skewer_motion_controller_linger") then return false end
	return true
end

function extended_magnataur_skewer:GetBehavior()
	if self:GetCaster():HasModifier("modifier_extended_skewer_motion_controller") then
...
function extended_magnataur_skewer:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local target_loc = self:GetCursorPosition()
		local caster_loc = caster:GetAbsOrigin()
		...
		caster:AddNewModifier(caster, self, "modifier_extended_skewer_motion_controller", {distance = distance, direction_x = direction.x, direction_y = direction.y, direction_z = direction.z, cooldown = current_cooldown, cast_sound = cast_sound})
	end
end

function extended_magnataur_skewer:GetCastAnimation()
    return ACT_DOTA_MAGNUS_SKEWER_START
end

function extended_magnataur_skewer:IsHiddenWhenStolen()
    return false
end

function extended_magnataur_skewer:IsStealable()
	return true
end

function extended_magnataur_skewer:GetAbilityTextureName()
	if self:GetCaster():HasModifier("modifier_extended_skewer_motion_controller") then
...
function extended_magnataur_skewer:GetCooldown( nLevel )
	return self.BaseClass.GetCooldown( self, nLevel ) - self:GetCaster():FindTalentValue("special_bonus_extended_magnataur_4")
end

modifier_extended_skewer_motion_controller = modifier_extended_skewer_motion_controller or class({})

function modifier_extended_skewer_motion_controller:OnCreated( params )
	if IsServer() then
		local caster = self:GetCaster()
		local ability = self:GetAbility()
		local caster_loc = caster:GetAbsOrigin()
...
function modifier_extended_skewer_motion_controller:OnDestroy()
	if IsServer() then
		local caster = self:GetCaster()
		caster:AddNewModifier(caster, self:GetAbility(), "modifier_extended_skewer_motion_controller_linger", {duration = 0.1})
		self:GetAbility():StartCooldown(self.cooldown)
	end
end

function modifier_extended_skewer_motion_controller:IsHidden()
	return true
end

function modifier_extended_skewer_motion_controller:RemoveOnDeath()
	return false
end

function modifier_extended_skewer_motion_controller:IgnoreTenacity()
	return true
end

function modifier_extended_skewer_motion_controller:CheckState()
	local state =
	{
...
function modifier_extended_skewer_motion_controller:UpdateHorizontalMotion( unit, time )
	if IsServer() then
		local caster = self:GetCaster()
		local caster_loc = caster:GetAbsOrigin()
		local ability = self:GetAbility()
		...
		for _,enemy in ipairs(enemies) do
			if not enemy:HasModifier("modifier_extended_skewer_motion_controller_target") then
				enemy:SetAbsOrigin( caster:GetAbsOrigin() + self.direction * self.horned_distance )
				enemy:AddNewModifier(caster, ability, "modifier_extended_skewer_motion_controller_target", {direction_x = self.direction.x, direction_y = self.direction.y, direction_z = self.direction.z, speed = self.speed})
			end
		end
		...
		if (self.traveled < self.distance) and caster:IsAlive() and not self.begged_for_pardon then
			...
			for _,enemy in ipairs(piked_enemies) do
				if enemy:HasModifier("modifier_extended_polarize_debuff") then
			...
			for _,enemy in ipairs(piked_enemies) do
				...
				local modifier = enemy:FindModifierByNameAndCaster("modifier_extended_skewer_motion_controller_target",caster)
				if modifier then
					if self.begged_for_pardon and not enemy:HasModifier("modifier_extended_polarize_debuff") then
						...
						Timers:CreateTimer(knockup_duration, function()
							ApplyDamage({victim = enemy, attacker = caster, ability = ability, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL})
							if enemy:HasModifier("modifier_extended_polarize_debuff") and (polarize_counter == 2) then
								enemy:AddNewModifier(caster, ability, "modifier_extended_skewer_entangle", {duration = self.entangle_dur})
							end
						end)
					end
					enemy:AddNewModifier(caster, ability, "modifier_extended_skewer_slow", {duration = self.slow_duration, pardoned = self.begged_for_pardon})
				end
			end
		end
	end
...
function modifier_extended_skewer_motion_controller:OnHorizontalMotionInterrupted()
	if IsServer() then
...
modifier_extended_skewer_motion_controller_linger = modifier_extended_skewer_motion_controller_linger or class({})

function modifier_extended_skewer_motion_controller_linger:IsHidden()
	return true
end

function modifier_extended_skewer_motion_controller_linger:IsPurgable()
	return false
end

modifier_extended_skewer_motion_controller_target = modifier_extended_skewer_motion_controller_target or class({})

function modifier_extended_skewer_motion_controller_target:OnCreated( params )
	if IsServer() then
		self.direction = Vector(params.direction_x, params.direction_y, params.direction_z)
...
function modifier_extended_skewer_motion_controller_target:OnDestroy()
	return false
end

function modifier_extended_skewer_motion_controller_target:RemoveOnDeath()
	return false
end

function modifier_extended_skewer_motion_controller_target:IgnoreTenacity()
	return true
end

function modifier_extended_skewer_motion_controller_target:CheckState()
	local state =
	{
		[MODIFIER_STATE_STUNNED] = true
	}
...
function modifier_extended_skewer_motion_controller_target:UpdateHorizontalMotion( unit, time )
	if IsServer() then
		local caster = self:GetCaster()

		-- Move the target while caster has motion-controller
		if caster:HasModifier("modifier_extended_skewer_motion_controller") and caster:IsAlive() then
...
function modifier_extended_skewer_motion_controller_target:OnHorizontalMotionInterrupted()
	if IsServer() then
		self:Destroy()
	end
end

function modifier_extended_skewer_motion_controller_target:GetOverrideAnimation()
	return ACT_DOTA_FLAIL
end

function modifier_extended_skewer_motion_controller_target:DeclareFunctions()
	local decFuncs =
	{
...
modifier_extended_skewer_slow = modifier_extended_skewer_slow or class({})

function modifier_extended_skewer_slow:DeclareFunctions()
	local decFuncs =
	{
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
		MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,
...
function modifier_extended_skewer_slow:GetModifierMoveSpeedBonus_Percentage()
...
function modifier_extended_skewer_slow:GetModifierAttackSpeedBonus_Constant()
	if self.pardoned then return self:GetAbility():GetSpecialValueFor("pardon_slow") * (-1) end
	return self:GetAbility():GetSpecialValueFor("slow") * (-1)
end

function modifier_extended_skewer_slow:GetEffectName()
	return "particles/units/heroes/hero_magnataur/magnataur_skewer_debuff.vpcf"
end

function modifier_extended_skewer_slow:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_skewer_slow:OnCreated( params )
	self.pardoned = params.pardoned
end

modifier_extended_skewer_entangle = modifier_extended_skewer_entangle or class({})

function modifier_extended_skewer_entangle:DeclareFunctions()
	local decFuncs =
	{
...
function modifier_extended_skewer_entangle:GetModifierMoveSpeed_Limit()
	return 0.1
end

function modifier_extended_skewer_entangle:CheckState()
	local state =
	{
...
function modifier_extended_skewer_entangle:GetEffectName()
	return "particles/hero/magnataur/skewer_entangle_debuff.vpcf"
end

function modifier_extended_skewer_entangle:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

function modifier_extended_skewer_entangle:OnCreated()
	if IsServer() then
...
function modifier_extended_skewer_entangle:OnIntervalThink()
	if IsServer() then
...
extended_magnataur_reverse_polarity = extended_magnataur_reverse_polarity or class({})
LinkLuaModifier("modifier_extended_reverse_polarity_slow", "hero/hero_magnataur", LUA_MODIFIER_MOTION_NONE)

function extended_magnataur_reverse_polarity:OnAbilityPhaseStart()
	if IsServer() then
		local caster = self:GetCaster()
...
function extended_magnataur_reverse_polarity:IsHiddenWhenStolen()
	return false
end

function extended_magnataur_reverse_polarity:IsStealable()
	return true
end

function extended_magnataur_reverse_polarity:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local caster_loc = caster:GetAbsOrigin()
		...
		for _,enemy in ipairs(enemies) do
			...
			if enemy:HasModifier("modifier_extended_polarize_debuff") then
				pull_distance = pull_distance + (enemy:FindModifierByName("modifier_extended_polarize_debuff"):GetStackCount() * pull_per_stack)
			...
			if pull_distance >= current_distance then
				...
				enemy:AddNewModifier(caster, nil, "modifier_extended_polarize_debuff_stack", {duration = polarize_duration})
			...
			elseif pull_distance > radius then
				...
				enemy:AddNewModifier(caster, self, "modifier_extended_reverse_polarity_slow", {duration = hero_stun_duration})
...
function extended_magnataur_reverse_polarity:GetCastRange( location , target)
	return self.BaseClass.GetCastRange(self,location,target) + self:GetCaster():FindTalentValue("special_bonus_extended_magnataur_7")
end

modifier_extended_reverse_polarity_slow = modifier_extended_reverse_polarity_slow or class({})

function modifier_extended_reverse_polarity_slow:DeclareFunctions()
	local decFuncs =
	{
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE
	}
...
function modifier_extended_reverse_polarity_slow:GetModifierMoveSpeedBonus_Percentage( )
	return self:GetAbility():GetSpecialValueFor("polarize_slow") * (-1)
end

function modifier_extended_reverse_polarity_slow:IsHidden()
	return false
end

function modifier_extended_reverse_polarity_slow:IsDebuff()
	return true
end

function modifier_extended_reverse_polarity_slow:IsPurgable()
	return true
end