...
function modifier_special_bonus_extended_tinker_7:DeclareFunctions()
	local decFuncs =
	{
...
function modifier_special_bonus_extended_tinker_7:GetModifierPercentageCasttime()
	return self:GetParent():FindTalentValue("special_bonus_extended_tinker_7")
end

-------------------------------------------
--			   REARM
-------------------------------------------
LinkLuaModifier("modifier_extended_rearm_animation", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_rearm_overdrive", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_rearm_shield", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)

extended_tinker_rearm = class({})

function extended_tinker_rearm:IsNetherWardStealable()
	return false
end

function extended_tinker_rearm:IsInnateAbility()
	return true
end

function extended_tinker_rearm:GetAssociatedSecondaryAbilities()
	return "extended_tinker_technomancy"
end

function extended_tinker_rearm:IsStealable()
	return true
end

function extended_tinker_rearm:IsHiddenWhenStolen()
	return false
end

function extended_tinker_rearm:GetIntrinsicModifierName()
    return "modifier_extended_rearm_overdrive"
end

function extended_tinker_rearm:GetChannelAnimation()
	return nil
end

function extended_tinker_rearm:GetChannelTime()
	if self:GetCaster():HasTalent("special_bonus_extended_tinker_8") then
		return 0
	end
	return self.BaseClass.GetChannelTime(self)
end

function extended_tinker_rearm:GetCooldown( nLevel )
	if self:GetCaster():HasTalent("special_bonus_extended_tinker_8") then
		return self.BaseClass.GetChannelTime(self)
	end
	return 0
...
function extended_tinker_rearm:GetManaCost()
	local extra_cost = (self:GetLevel() - 1) * self:GetSpecialValueFor("rearm_mana_per_lvl")
	return (self:GetSpecialValueFor("base_manacost") + extra_cost - self:GetCaster():FindTalentValue("special_bonus_extended_tinker_1"))
end

function extended_tinker_rearm:GetCastAnimation()
	return ACT_DOTA_TINKER_REARM1
end

function extended_tinker_rearm:OnSpellStart()
	if IsServer() then
		...
		if caster:HasTalent("special_bonus_extended_tinker_8") then
			caster:AddNewModifier(caster, self, "modifier_extended_rearm_animation", { duration = 0.1})
		else
			caster:AddNewModifier(caster, self, "modifier_extended_rearm_animation", { duration = self:GetChannelTime()})
		end
		if caster:HasTalent("special_bonus_extended_tinker_8") then
			self:OnChannelFinish( false )
		end
		-- #6 Talent - Shield
		if caster:HasTalent("special_bonus_extended_tinker_6") then
			caster:AddNewModifier(caster, self, "modifier_extended_rearm_shield", { duration = 3.5 })
		end
	end
end

function extended_tinker_rearm:OnChannelFinish( bInterrupted )
	if IsServer() then
		local caster = self:GetCaster()
		...
		caster:RemoveModifierByName("modifier_extended_rearm_animation")
	end
end

modifier_extended_rearm_animation = class({})
function modifier_extended_rearm_animation:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
		...
		if not caster:HasTalent("special_bonus_extended_tinker_8") then
		...
		Timers:CreateTimer(FrameTime(), function()
			if caster:HasModifier("modifier_extended_rearm_animation") and not caster:HasTalent("special_bonus_extended_tinker_8") then
...
function modifier_extended_rearm_animation:GetEffectName()
	return "particles/units/heroes/hero_tinker/tinker_rearm.vpcf"
end

function modifier_extended_rearm_animation:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

modifier_extended_rearm_overdrive = class({})
function modifier_extended_rearm_overdrive:IsHidden()
	if self:GetParent():HasScepter() then return false end
	return true
end

function modifier_extended_rearm_overdrive:IsDebuff()
	return false
end

function modifier_extended_rearm_overdrive:IsPurgable()
	return false
end

function modifier_extended_rearm_overdrive:OnCreated()
...
function modifier_extended_rearm_overdrive:OnIntervalThink()
...
function modifier_extended_rearm_overdrive:DeclareFunctions()
	local funcs = 
	{
...
function modifier_extended_rearm_overdrive:GetModifierSpellAmplify_Percentage()
	return (self.aghs_spellpower * self:GetStackCount())
end

modifier_extended_rearm_shield = class({})
function modifier_extended_rearm_shield:DeclareFunctions()
	local decFuncs = 
	{
...
function modifier_extended_rearm_shield:IsHidden()
	return false
end

function modifier_extended_rearm_shield:IsDebuff()
	return false
end

function modifier_extended_rearm_shield:IsPurgable()
	return true
end

function modifier_extended_rearm_shield:GetEffectName()
	return "particles/hero/tinker/rearmshield_shield.vpcf"
end

function modifier_extended_rearm_shield:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_rearm_shield:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
		self.current_health = self:GetParent():GetHealth()
		self.shield_hp = caster:GetMaxHealth() * (caster:FindTalentValue("special_bonus_extended_tinker_6") / 100)
		self:StartIntervalThink(FrameTime())
	end
end

function modifier_extended_rearm_shield:OnRefresh()
	if IsServer() then
		local caster = self:GetCaster()
		self.shield_hp = caster:GetMaxHealth() * (caster:FindTalentValue("special_bonus_extended_tinker_6") / 100)
	end
...
function modifier_extended_rearm_shield:OnIntervalThink()
	-- Get current health per frame
	self.current_health = self:GetParent():GetHealth()
end

function modifier_extended_rearm_shield:OnTakeDamage( params )
	if IsServer() then
		local parent = self:GetParent()
...
--			   LASER
-------------------------------------------
LinkLuaModifier("modifier_extended_laser_blind", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)

extended_tinker_laser = class({})
function extended_tinker_laser:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
...
function extended_tinker_laser:LaserRefraction(source, target)
	local caster = self:GetCaster()
	local caster_loc = caster:GetAbsOrigin()
	local target_loc = target:GetAbsOrigin()
	...
	if caster:HasAbility("extended_tinker_technomancy") then
		local technomancy = caster:FindAbilityByName("extended_tinker_technomancy")
	...
	target:AddNewModifier(caster, self, "modifier_extended_laser_blind", { duration = blind_duration })
end

function extended_tinker_laser:IsNetherWardStealable()
	return true
end

function extended_tinker_laser:GetAssociatedSecondaryAbilities()
	return "extended_tinker_technomancy"
end

function extended_tinker_laser:IsStealable()
	return true
end

function extended_tinker_laser:IsHiddenWhenStolen()
	return false
end

modifier_extended_laser_blind = class({})

function modifier_extended_laser_blind:IsHidden() return false end
function modifier_extended_laser_blind:IsPurgable() return true end
function modifier_extended_laser_blind:IsDebuff() return true end

function modifier_extended_laser_blind:GetModifierMiss_Percentage()
	return self:GetAbility():GetSpecialValueFor("miss_chance_pct")
end

function modifier_extended_laser_blind:DeclareFunctions()
	local decFuncs = 
	{	
...
-------------------------------------------
extended_tinker_heat_seeking_missile = class({})

function extended_tinker_heat_seeking_missile:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local caster_loc = caster:GetAbsOrigin()
...
function extended_tinker_heat_seeking_missile:OnProjectileHit_ExtraData(target, location, ExtraData)
	local caster = self:GetCaster()
	if target then
...
function extended_tinker_heat_seeking_missile:IsNetherWardStealable()
	return true
end

function extended_tinker_heat_seeking_missile:GetAssociatedSecondaryAbilities()
	return "extended_tinker_technomancy"
end

function extended_tinker_heat_seeking_missile:IsStealable()
	return true
end

function extended_tinker_heat_seeking_missile:IsHiddenWhenStolen()
	return false
end
...
-------------------------------------------
LinkLuaModifier("modifier_extended_march_flame_aura", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_march_flame_damage", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_march_tesla_stun", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_march_sticky_root", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_march_dismantle", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_march_drone", "hero/hero_tinker", LUA_MODIFIER_MOTION_NONE)

extended_tinker_march_of_the_machines = class({})
function extended_tinker_march_of_the_machines:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		...
		speed = speed * (1 + (caster:FindTalentValue("special_bonus_extended_tinker_3") / 100))
		travel_distance = travel_distance * (1 + (caster:FindTalentValue("special_bonus_extended_tinker_3") / 100))
...
function extended_tinker_march_of_the_machines:OnProjectileHit_ExtraData(target, location, ExtraData)
	local caster = self:GetCaster()
		...
		if ExtraData.raildamage then
			...
			for _,enemy in ipairs(enemies) do
				...
				if ExtraData.tesla_stun_duration then
					enemy:AddNewModifier(caster, self, "modifier_extended_march_tesla_stun", {duration = ExtraData.tesla_stun_duration})
				end
				-- Sticky-bot handling
				if ExtraData.sticky_duration then
					enemy:AddNewModifier(caster, self, "modifier_extended_march_sticky_root", {duration = ExtraData.sticky_duration})
				end
				-- Dismantle-bot handling
				if ExtraData.dismantle_duration then
					enemy:AddNewModifier(caster, self, "modifier_extended_march_dismantle", {duration = ExtraData.dismantle_duration})
				end
				-- Drone-bot handling
				if ExtraData.drone_duration then
					enemy:AddNewModifier(caster, self, "modifier_extended_march_drone", {duration = ExtraData.drone_duration})
				end
			end
		end
	end
...
function extended_tinker_march_of_the_machines:OnProjectileThink_ExtraData(location, ExtraData)
	if IsServer() then
		local caster = self:GetCaster()
		...
		if ExtraData.flame_duration then
			...
			if self[ExtraData.index].counter == 0 then
				...
				dummy:AddNewModifier(caster, self, "modifier_extended_march_flame_aura", {duration = ExtraData.flame_duration, flame_radius = ExtraData.flame_radius})
...
function extended_tinker_march_of_the_machines:IsNetherWardStealable()
	return true
end

function extended_tinker_march_of_the_machines:GetAssociatedSecondaryAbilities()
	return "extended_tinker_technomancy"
end

function extended_tinker_march_of_the_machines:IsStealable()
	return true
end

function extended_tinker_march_of_the_machines:IsHiddenWhenStolen()
	return false
end

modifier_extended_march_flame_aura = class({})
function modifier_extended_march_flame_aura:OnCreated(params)
	if IsServer() then
		local caster = self:GetCaster()
		local parent = self:GetParent()
...
function modifier_extended_march_flame_aura:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_march_flame_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_march_flame_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_march_flame_aura:IsAura()
	return true
end

function modifier_extended_march_flame_aura:GetAuraRadius()
	return self.radius
end

function modifier_extended_march_flame_aura:GetModifierAura()
	return "modifier_extended_march_flame_damage"
end

modifier_extended_march_flame_damage = class({})

function modifier_extended_march_flame_damage:IsDebuff()
	return true
end

function modifier_extended_march_flame_damage:OnCreated()
	if IsServer() then
...
function modifier_extended_march_flame_damage:OnIntervalThink()
	if IsServer() then
		local ability = self:GetAbility()
...
function modifier_extended_march_flame_damage:GetEffectName()
	return "particles/units/heroes/hero_batrider/batrider_firefly_debuff.vpcf"
end

function modifier_extended_march_flame_damage:GetEffectAttachType()
	return PATTACH_POINT_FOLLOW
end

function modifier_extended_march_flame_damage:IsHidden()
	return true
end

modifier_extended_march_tesla_stun = class({})
function modifier_extended_march_tesla_stun:IsStunDebuff()
	return true
end

function modifier_extended_march_tesla_stun:GetEffectName()
	return "particles/units/heroes/hero_stormspirit/stormspirit_electric_vortex_debuff_end.vpcf"
end

function modifier_extended_march_tesla_stun:GetEffectAttachType()
	return PATTACH_POINT_FOLLOW
end

function modifier_extended_march_tesla_stun:OnCreated()
	if IsServer() then
...
function modifier_extended_march_tesla_stun:CheckState()
	local state =
	{
...
function modifier_extended_march_tesla_stun:IsHidden()
	return true
end

modifier_extended_march_sticky_root = class({})
function modifier_extended_march_sticky_root:GetEffectName()
	return "particles/units/heroes/hero_batrider/batrider_stickynapalm_debuff.vpcf"
end

function modifier_extended_march_sticky_root:GetEffectAttachType()
	return PATTACH_POINT_FOLLOW
end

function modifier_extended_march_sticky_root:CheckState()
	local state =
	{
...
function modifier_extended_march_sticky_root:IsHidden()
	return true
end

modifier_extended_march_dismantle = class({})
function modifier_extended_march_dismantle:OnCreated(params)
	self.dismantle_dmg_pct = self:GetAbility():GetTalentSpecialValueFor("dismantle_dmg_pct") * (-1)
	self:SetStackCount(1)
end

function modifier_extended_march_dismantle:OnRefresh(params)
	self.dismantle_dmg_pct = self:GetAbility():GetTalentSpecialValueFor("dismantle_dmg_pct") * (-1)
	self:IncrementStackCount()
end

function modifier_extended_march_dismantle:IsDebuff()
	return true
end

function modifier_extended_march_dismantle:DeclareFunctions()
	local decFuncs = 
	{
...
function modifier_extended_march_dismantle:GetModifierBaseDamageOutgoing_Percentage()
	return self.dismantle_dmg_pct * self:GetStackCount()
end

modifier_extended_march_drone = class({})
function modifier_extended_march_drone:OnCreated(params)
...
function modifier_extended_march_drone:IsDebuff()
	return true
end

function modifier_extended_march_drone:OnIntervalThink()
	if IsServer() then
		local parent = self:GetParent()
...
function modifier_extended_march_drone:CheckState()
    local state = 
	{
...
--			TECHNOMANCY
-------------------------------------------
extended_tinker_technomancy = class({})

function extended_tinker_technomancy:OnUpgrade()
	local caster = self:GetCaster()
	if caster:HasAbility("extended_tinker_rearm") then
		local ability = caster:FindAbilityByName("extended_tinker_rearm")
		ability:SetLevel((self:GetLevel() + 1))
	end
end

function extended_tinker_technomancy:IsNetherWardStealable()
	return false
end

function extended_tinker_technomancy:IsStealable()
	return true
end

function extended_tinker_technomancy:IsHiddenWhenStolen()
	return true
end