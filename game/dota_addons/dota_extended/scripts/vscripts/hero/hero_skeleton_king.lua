...
--      WRAITHFIRE BLAST      --
--------------------------------
extended_wraith_king_wraithfire_blast = class({})
LinkLuaModifier("modifier_extended_wraithfire_blast_stun", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_wraithfire_blast_debuff", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_wraith_king_wraithfire_blast:IsHiddenWhenStolen()
    return false
end

function extended_wraith_king_wraithfire_blast:OnSpellStart()
    -- Ability properties
...
function extended_wraith_king_wraithfire_blast:OnProjectileHit_ExtraData(target, location, extra_data)
    ...
    local modifier_stun = "modifier_extended_wraithfire_blast_stun"
    local modifier_debuff = "modifier_extended_wraithfire_blast_debuff"
    ...
    debuff_duration = debuff_duration + caster:FindTalentValue("special_bonus_extended_skeleton_king_1")
    ...
    if caster:HasTalent("special_bonus_extended_skeleton_king_7") then
        local direction = (target:GetAbsOrigin() - caster:GetAbsOrigin()):Normalized()
        local distance = (target:GetAbsOrigin() - caster:GetAbsOrigin()):Length2D()
        local summon_point = caster:GetAbsOrigin() + direction * distance - 100
        local wraith = CreateUnitByName("npc_extended_wraith_king_wraith", summon_point, true, caster, caster, caster:GetTeamNumber())        
...
-- Stun modifier
modifier_extended_wraithfire_blast_stun = class({})

function modifier_extended_wraithfire_blast_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_wraithfire_blast_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_wraithfire_blast_stun:GetEffectAttachType()
    return PATTACH_OVERHEAD_FOLLOW
end

function modifier_extended_wraithfire_blast_stun:IsHidden() return false end
function modifier_extended_wraithfire_blast_stun:IsPurgeException() return true end
function modifier_extended_wraithfire_blast_stun:IsStunDebuff() return true end




-- Debuff modifier
modifier_extended_wraithfire_blast_debuff = class({})

function modifier_extended_wraithfire_blast_debuff:OnCreated()
    -- Ability properties
...
function modifier_extended_wraithfire_blast_debuff:IsHidden() return false end
function modifier_extended_wraithfire_blast_debuff:IsPurgable() return true end
function modifier_extended_wraithfire_blast_debuff:IsDebuff() return true end

function modifier_extended_wraithfire_blast_debuff:OnIntervalThink()
    if IsServer() then
        -- Calculate damage
...
function modifier_extended_wraithfire_blast_debuff:DeclareFunctions()
...
function modifier_extended_wraithfire_blast_debuff:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end

function modifier_extended_wraithfire_blast_debuff:OnAttackLanded(keys)
...
--      VAMPIRIC AURA      --
--------------------------------
extended_wraith_king_vampiric_aura = class({})
LinkLuaModifier("modifier_extended_vampiric_aura", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_vampiric_aura_buff", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_wraith_king_vampiric_aura:OnToggle() return nil end
function extended_wraith_king_vampiric_aura:IsStealable() return false end

function extended_wraith_king_vampiric_aura:GetIntrinsicModifierName()
    return "modifier_extended_vampiric_aura"
end

-- Aura modifier
modifier_extended_vampiric_aura = class({})

function modifier_extended_vampiric_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_vampiric_aura:AllowIllusionDuplicate() return true end
function modifier_extended_vampiric_aura:IsHidden() return true end
function modifier_extended_vampiric_aura:IsPurgable() return false end
function modifier_extended_vampiric_aura:IsDebuff() return false end    

function modifier_extended_vampiric_aura:GetAuraRadius()
    return self.radius
end

function modifier_extended_vampiric_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_INVULNERABLE
end

function modifier_extended_vampiric_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_vampiric_aura:GetAuraSearchType()
...
function modifier_extended_vampiric_aura:GetModifierAura()
    return "modifier_extended_vampiric_aura_buff"
end

function modifier_extended_vampiric_aura:IsAura()
...
-- Aura buff modifier
modifier_extended_vampiric_aura_buff = class({})

function modifier_extended_vampiric_aura_buff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_vampiric_aura_buff:OnRefresh()
    self:OnCreated()
end

function modifier_extended_vampiric_aura_buff:IsHidden() return false end
function modifier_extended_vampiric_aura_buff:IsPurgable() return false end
function modifier_extended_vampiric_aura_buff:IsDebuff() return false end

function modifier_extended_vampiric_aura_buff:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_TAKEDAMAGE}

    return decFuncs
end

function modifier_extended_vampiric_aura_buff:OnTakeDamage(keys)
    if IsServer() then
        local attacker = keys.attacker
        local damage = keys.damage
        ...
        if self.parent == attacker then
            ...
            if damage_type == DAMAGE_TYPE_PHYSICAL then
                ...
                local lifesteal_pct = self.lifesteal_pct + self.caster:FindTalentValue("special_bonus_extended_skeleton_king_3")
            ...
            -- If the damage was magical or pure, use the skeletonking particle instead, and heal using the spellsteal values
                ...
                -- #3 Talent: Vampiric Aura lifesteal/spellsteal increase
                local spellsteal_pct = self.spellsteal_pct + self.caster:FindTalentValue("special_bonus_extended_skeleton_king_3")
...
--       MORTAL STRIKE        --
--------------------------------
extended_wraith_king_mortal_strike = class({})
LinkLuaModifier("modifier_extended_mortal_strike", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_mortal_strike_buff", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_wraith_king_mortal_strike:GetIntrinsicModifierName()
    return "modifier_extended_mortal_strike"
end


-- Critical strikes modifier
modifier_extended_mortal_strike = class({})

function modifier_extended_mortal_strike:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.modifier_health = "modifier_extended_mortal_strike_buff"
...
function modifier_extended_mortal_strike:OnRefresh()
    self:OnCreated()
end

function modifier_extended_mortal_strike:IsHidden() return true end
function modifier_extended_mortal_strike:IsPurgable() return false end
function modifier_extended_mortal_strike:IsDebuff() return false end

function modifier_extended_mortal_strike:DeclareFunctions()
...
function modifier_extended_mortal_strike:GetModifierPreAttack_CriticalStrike(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.target
        ...
        if attacker == self.caster then
            ...
            local crit_chance = self.crit_chance + self.caster:FindTalentValue("special_bonus_extended_skeleton_king_2")
...
function modifier_extended_mortal_strike:OnAttackLanded(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.target
        local damage = keys.damage
        ...
        if attacker == self.caster then            
            ...
            local bonus_health_pct = self.bonus_health_pct + self.caster:FindTalentValue("special_bonus_extended_skeleton_king_5")
...
modifier_extended_mortal_strike_buff = class({})

function modifier_extended_mortal_strike_buff:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
...
function modifier_extended_mortal_strike_buff:IsHidden() return false end
function modifier_extended_mortal_strike_buff:IsPurgable() return false end
function modifier_extended_mortal_strike_buff:IsDebuff() return false end

function modifier_extended_mortal_strike_buff:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_mortal_strike_buff:OnRefresh()
    if IsServer() then
...
function modifier_extended_mortal_strike_buff:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_HEALTH_BONUS}

    return decFuncs
end

function modifier_extended_mortal_strike_buff:GetModifierHealthBonus()
...
--       REINCARNATION        --
--------------------------------
extended_wraith_king_reincarnation = class({})
LinkLuaModifier("modifier_extended_reincarnation", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_reincarnation_wraith_form_buff", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_reincarnation_wraith_form", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_wraith_king_reincarnation:GetManaCost(level)
    local caster = self:GetCaster()
    local ability = self    
    local reincarnate_mana_cost = ability:GetSpecialValueFor("reincarnate_mana_cost")

    -- #6 Talent: Reincarnation no longer needs mana
    reincarnate_mana_cost = reincarnate_mana_cost - caster:FindTalentValue("special_bonus_extended_skeleton_king_6")

    return reincarnate_mana_cost
end

function extended_wraith_king_reincarnation:GetCooldown(level)
    local caster = self:GetCaster()
    local ability = self
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #8 Talent: Reincarnate cooldown decrease
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_skeleton_king_8")

    return cooldown
end


function extended_wraith_king_reincarnation:GetIntrinsicModifierName()
    return "modifier_extended_reincarnation"
end


-- Reicarnation modifier
modifier_extended_reincarnation = class({})

function modifier_extended_reincarnation:OnCreated()    
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()    
        ...
        self.modifier_wraith = "modifier_extended_reincarnation_wraith_form"
...
function modifier_extended_reincarnation:IsHidden() return true end
function modifier_extended_reincarnation:IsPurgable() return false end
function modifier_extended_reincarnation:IsDebuff() return false end

function modifier_extended_reincarnation:OnIntervalThink()
    -- If caster has sufficent mana and the ability is ready, apply
    if (self.caster:GetMana() >= self.reincarnate_mana_cost) and (self.ability:IsCooldownReady()) and (not self.caster:HasModifier("modifier_item_extended_aegis")) then
        self.can_die = false
    else
        self.can_die = true
    end
...
function modifier_extended_reincarnation:OnRefresh()
    self:OnCreated()
end

function modifier_extended_reincarnation:DeclareFunctions()
...
function modifier_extended_reincarnation:ReincarnateTime()
    if IsServer() then  
...
function modifier_extended_reincarnation:GetModifierStackingRespawnTime()
    return self.passive_respawn_haste * (-1)
end

function modifier_extended_reincarnation:GetActivityTranslationModifiers()
...
function modifier_extended_reincarnation:OnDeath(keys)
    if IsServer() then
        ...
        if self.caster == unit then            
            ...
            if reincarnate and (not self.caster:HasModifier("modifier_item_extended_aegis")) then
...
function modifier_extended_reincarnation:GetAuraRadius()
    return self.scepter_wraith_form_radius
end

function modifier_extended_reincarnation:GetAuraEntityReject(target)
...
function modifier_extended_reincarnation:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS + DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO
end

function modifier_extended_reincarnation:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_reincarnation:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO
end

function modifier_extended_reincarnation:GetModifierAura()
    return "modifier_extended_reincarnation_wraith_form_buff"
end

function modifier_extended_reincarnation:IsAura()
...
function modifier_extended_reincarnation:IsAuraActiveOnDeath()
    return true
end
...
modifier_extended_reincarnation_wraith_form_buff = class({})

function modifier_extended_reincarnation_wraith_form_buff:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.modifier_wraith_form = "modifier_extended_reincarnation_wraith_form"
...
function modifier_extended_reincarnation_wraith_form_buff:DeclareFunctions()
...
function modifier_extended_reincarnation_wraith_form_buff:GetMinHealth()
    return 1
end

function modifier_extended_reincarnation_wraith_form_buff:OnTakeDamage(keys)
    if IsServer() then
        local attacker = keys.attacker
        local target = keys.unit 
        local damage = keys.damage
        ...
        if self.parent == target then
            ...
            if damage >= self.parent:GetHealth() then
                ...
                if self.parent:HasModifier("modifier_extended_dazzle_shallow_grave") or self.parent:HasModifier("modifier_extended_dazzle_nothl_protection") then
                    return nil
                end

                -- Check for Aegis: kill the unit normally
                if self.parent:HasModifier("modifier_item_extended_aegis") then
                    self:Destroy()
...
modifier_extended_reincarnation_wraith_form = class({})

function modifier_extended_reincarnation_wraith_form:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_reincarnation_wraith_form:DeclareFunctions()
...
function modifier_extended_reincarnation_wraith_form:GetAbsoluteNoDamageMagical()
    return 1
end

function modifier_extended_reincarnation_wraith_form:GetAbsoluteNoDamagePhysical()
    return 1
end

function modifier_extended_reincarnation_wraith_form:GetAbsoluteNoDamagePure()
    return 1
end

function modifier_extended_reincarnation_wraith_form:GetDisableHealing()
    return 1
end

function modifier_extended_reincarnation_wraith_form:GetModifierModelScale()
    return 105
end

function modifier_extended_reincarnation_wraith_form:CheckState()
...
function modifier_extended_reincarnation_wraith_form:OnDestroy()
    if IsServer() then
        -- Force kill the unit
...
function modifier_extended_reincarnation_wraith_form:GetStatusEffectName()
...
--        KINGDOM COME        --
--------------------------------
extended_wraith_king_kingdom_come = class({})
LinkLuaModifier("modifier_extended_kingdom_come", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)    
LinkLuaModifier("modifier_extended_kingdom_come_slow", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_kingdom_come_stun", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)

function extended_wraith_king_kingdom_come:IsNetherWardStealable() return false end
function extended_wraith_king_kingdom_come:IsInnateAbility()
    return true
end

function extended_wraith_king_kingdom_come:GetIntrinsicModifierName()
    return "modifier_extended_kingdom_come"
end


modifier_extended_kingdom_come = class({})

function modifier_extended_kingdom_come:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    ...
    self.modifier_slow = "modifier_extended_kingdom_come_slow"
...
function modifier_extended_kingdom_come:IsHidden() return true end
function modifier_extended_kingdom_come:IsPurgable() return false end
function modifier_extended_kingdom_come:IsDebuff() return false end

function modifier_extended_kingdom_come:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_DEATH}

    return decFuncs
end

function modifier_extended_kingdom_come:OnDeath(keys)
    if IsServer() then
        local unit = keys.unit
...
-- Kingdom Come slow
modifier_extended_kingdom_come_slow = class({})

function modifier_extended_kingdom_come_slow:OnCreated()    
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.particle_slow = "particles/units/heroes/hero_skeletonking/wraith_king_reincarnate_slow_debuff.vpcf"
    self.modifier_stun = "modifier_extended_kingdom_come_stun"
...
function modifier_extended_kingdom_come_slow:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
...
function modifier_extended_kingdom_come_slow:IsHidden() return false end
function modifier_extended_kingdom_come_slow:IsPurgable() return true end
function modifier_extended_kingdom_come_slow:IsDebuff() return true end

function modifier_extended_kingdom_come_slow:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end

function modifier_extended_kingdom_come_slow:GetModifierAttackSpeedBonus_Constant()
    return self.as_slow * (-1)
end

function modifier_extended_kingdom_come_slow:OnDestroy()    
    if IsServer() then
        ...
        if self.parent:IsRealHero() then
            ...
            local wraith = CreateUnitByName("npc_extended_wraith_king_wraith", summon_point, true, self.caster, self.caster, self.caster:GetTeamNumber())        
...
modifier_extended_kingdom_come_stun = class({})

function modifier_extended_kingdom_come_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_kingdom_come_stun:IsHidden() return false end
function modifier_extended_kingdom_come_stun:IsPurgeException() return false end 
function modifier_extended_kingdom_come_stun:IsStunDebuff() return false end

function modifier_extended_kingdom_come_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_kingdom_come_stun:GetEffectAttachType()
    return PATTACH_OVERHEAD_FOLLOW
end
...
--------------------------------
extended_wraith_king_wraith_soul_strike = class({})
LinkLuaModifier("modifier_extended_wraith_soul_strike", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)    
LinkLuaModifier("modifier_extended_wraith_soul_strike_slow", "hero/hero_skeleton_king.lua", LUA_MODIFIER_MOTION_NONE)    

function extended_wraith_king_wraith_soul_strike:GetIntrinsicModifierName()
    return "modifier_extended_wraith_soul_strike"
end


modifier_extended_wraith_soul_strike = class({})

function modifier_extended_wraith_soul_strike:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()    
        self.ability = self:GetAbility()
        self.owner = self.caster:GetOwner()
        self.modifier_slow = "modifier_extended_wraith_soul_strike_slow"
...
function modifier_extended_wraith_soul_strike:DeclareFunctions()
...
function modifier_extended_wraith_soul_strike:OnAttack(keys)
    if IsServer() then
        local target = keys.target
        local attacker = keys.attacker
...
function modifier_extended_wraith_soul_strike:OnAttackLanded(keys)    
    local target = keys.target
    local attacker = keys.attacker
    ...
    if attacker == self.caster then
        ...
        if self.owner:HasTalent("special_bonus_extended_skeleton_king_4") then
            local duration = self.owner:FindSpecificTalentValue("special_bonus_extended_skeleton_king_4", "duration")
...
modifier_extended_wraith_soul_strike_slow = class({})

function modifier_extended_wraith_soul_strike_slow:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        -- Talent properties
        self.ms_slow_pct = self.owner:FindSpecificTalentValue("special_bonus_extended_skeleton_king_4", "ms_slow_pct")
...
function modifier_extended_wraith_soul_strike_slow:IsHidden() return false end
function modifier_extended_wraith_soul_strike_slow:IsPurgable() return false end
function modifier_extended_wraith_soul_strike_slow:IsDebuff() return false end

function modifier_extended_wraith_soul_strike_slow:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_wraith_soul_strike_slow:GetModifierMoveSpeedBonus_Percentage()
