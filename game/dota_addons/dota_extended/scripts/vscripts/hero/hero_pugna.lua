...
extended_pugna_nether_blast = class({})
LinkLuaModifier("modifier_extended_nether_blast_magic_res", "hero/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)

function extended_pugna_nether_blast:GetCooldown(level)
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #1 Talent: Nether Blast cooldown decrease
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_pugna_1")

    return cooldown
end

function extended_pugna_nether_blast:GetAOERadius()
    local ability = self
...
function extended_pugna_nether_blast:IsNetherWardStealable()
    return true
end

function extended_pugna_nether_blast:IsHiddenWhenStolen()
    return false
end

function extended_pugna_nether_blast:OnSpellStart()    
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_magic_res = "modifier_extended_nether_blast_magic_res"
    ...
    damage = damage + caster:FindTalentValue("special_bonus_extended_pugna_3")
...
modifier_extended_nether_blast_magic_res = class({})

function modifier_extended_nether_blast_magic_res:OnCreated()
    -- Ability properties
    ...
    self.magic_res_reduction_pct = self.magic_res_reduction_pct + self.caster:FindTalentValue("special_bonus_extended_pugna_8")
end

function modifier_extended_nether_blast_magic_res:IsHidden() return false end
function modifier_extended_nether_blast_magic_res:IsPurgable() return true end
function modifier_extended_nether_blast_magic_res:IsDebuff() return true end

function modifier_extended_nether_blast_magic_res:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}

    return decFuncs
end

function modifier_extended_nether_blast_magic_res:GetModifierMagicalResistanceBonus()
...
extended_pugna_decrepify = class({})
LinkLuaModifier("modifier_extended_decrepify", "hero/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)

function extended_pugna_decrepify:IsHiddenWhenStolen()
    return false
end

function extended_pugna_decrepify:CastFilterResultTarget(target)
    if IsServer() then
        local caster = self:GetCaster()
        ...
        if string.find(target:GetUnitName(),"npc_dota_unit_tombstone") or string.find(target:GetUnitName(), "npc_extended_pugna_nether_ward")then
            return UF_SUCCESS
        end
...
function extended_pugna_decrepify:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget()
    local sound_cast = "Hero_Pugna.Decrepify"
    local modifier_decrep = "modifier_extended_decrepify"

    -- Ability specials
    local duration = ability:GetSpecialValueFor("duration")    

    -- #6 Talent: Decrepify duration increase
    duration = duration + caster:FindTalentValue("special_bonus_extended_pugna_6")
...
-- Decrepify modifier
modifier_extended_decrepify = class({})

function modifier_extended_decrepify:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_decrepify:IsHidden() return false end
function modifier_extended_decrepify:IsPurgable() return true end

function modifier_extended_decrepify:IsDebuff() 
...
function modifier_extended_decrepify:CheckState()
    local state = {[MODIFIER_STATE_ATTACK_IMMUNE] = true,
                    [MODIFIER_STATE_DISARMED] = true}
    return state
end

function modifier_extended_decrepify:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS,
...
function modifier_extended_decrepify:GetModifierMagicalResistanceBonus()
    if self.is_ally then
...
function modifier_extended_decrepify:GetModifierMoveSpeedBonus_Percentage()
    if self.is_ally then
        return nil
    else
...
function modifier_extended_decrepify:GetAbsoluteNoDamagePhysical()
    return 1
end

function modifier_extended_decrepify:GetEffectName()
    return "particles/units/heroes/hero_pugna/pugna_decrepify.vpcf"
end

function modifier_extended_decrepify:GetEffectAttachType()
    return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_extended_decrepify:OnTakeDamage(keys)
    if IsServer() then
...
function modifier_extended_decrepify:OnDestroy()
    if IsServer() then
        ...
        damage = damage * (1 + self.caster:FindTalentValue("special_bonus_extended_pugna_4") * 0.01)
...
--       NETHER WARD          --
--------------------------------
extended_pugna_nether_ward = class({})
function extended_pugna_nether_ward:IsHiddenWhenStolen() return false end
function extended_pugna_nether_ward:IsRefreshable() return true end
function extended_pugna_nether_ward:IsStealable() return true end
function extended_pugna_nether_ward:IsNetherWardStealable() return false end
-------------------------------------------
function extended_pugna_nether_ward:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local target_point = self:GetCursorPosition()    
    local ability = self    
    ...
    local ability_ward = "extended_pugna_nether_ward_aura"
    ...
    local nether_ward = CreateUnitByName("npc_extended_pugna_nether_ward_"..(ability_level), target_point, false, caster, caster, caster:GetTeam())
...
--        NETHER AURA         --
--------------------------------
extended_pugna_nether_ward_aura = class({})
LinkLuaModifier("modifier_extended_nether_ward_aura", "hero/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_nether_ward_degen", "hero/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)

function extended_pugna_nether_ward_aura:GetIntrinsicModifierName()
    return "modifier_extended_nether_ward_aura"
end

-- Aura modifier
modifier_extended_nether_ward_aura = class({})

function modifier_extended_nether_ward_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()        
...
function modifier_extended_nether_ward_aura:DeclareFunctions()
...
function modifier_extended_nether_ward_aura:GetModifierIncomingDamage_Percentage()
    return -100
end

function modifier_extended_nether_ward_aura:GetDisableHealing()
    return 1
end

function modifier_extended_nether_ward_aura:OnAttackLanded(keys)
...
function modifier_extended_nether_ward_aura:CheckState()
    local state = {[MODIFIER_STATE_MAGIC_IMMUNE] = true}
    return state
end

function modifier_extended_nether_ward_aura:IsHidden() return true end
function modifier_extended_nether_ward_aura:IsPurgable() return false end
function modifier_extended_nether_ward_aura:IsDebuff() return false end

function modifier_extended_nether_ward_aura:GetAuraRadius()
    return self.radius
end

function modifier_extended_nether_ward_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
end

function modifier_extended_nether_ward_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_nether_ward_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_nether_ward_aura:GetModifierAura()
    return "modifier_extended_nether_ward_degen"
end

function modifier_extended_nether_ward_aura:IsAura()
    return true
end
...
-- Degen modifier
modifier_extended_nether_ward_degen = class({})

function modifier_extended_nether_ward_degen:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()    
    ...
    if IsServer() then
        ...
        self.mana_multiplier = self.mana_multiplier + self.owner:FindTalentValue("special_bonus_extended_pugna_2")    
    end
end

function modifier_extended_nether_ward_degen:IsHidden() return false end
function modifier_extended_nether_ward_degen:IsPurgable() return false end
function modifier_extended_nether_ward_degen:IsDebuff() return true end

function modifier_extended_nether_ward_degen:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_extended_nether_ward_degen:DeclareFunctions()
...
function modifier_extended_nether_ward_degen:GetModifierTotalPercentageManaRegen()
    return self.mana_regen_reduction * (-1)
end

function modifier_extended_nether_ward_degen:OnSpentMana(keys)
    if IsServer() then
        ...
        if string.find(target:GetUnitName(), "npc_extended_pugna_nether_ward") then
            return nil
        end
        ...
        if cast_ability_name == "extended_omniknight_repel" then
       ...
       -- Shadowraze: face the caster
        if cast_ability_name == "extended_nevermore_shadowraze_close" or cast_ability_name == "extended_nevermore_shadowraze_medium" or cast_ability_name == "extended_nevermore_shadowraze_far" then
            ward:SetForwardVector((target_point - ward_position):Normalized())
        end

        -- Reqiuem of Souls: Get target's Necromastery stack count        
        if cast_ability_name == "extended_nevermore_requiem" and not ward:HasModifier("modifier_extended_necromastery_souls") and target:HasAbility("extended_nevermore_necromastery") then
            local ability_handle = ward:AddAbility("extended_nevermore_necromastery")
            ability_handle:SetLevel(7)

            -- Find target's modifier and its stacks
            if target:HasModifier("modifier_extended_necromastery_souls") then
                local stacks = target:GetModifierStackCount("modifier_extended_necromastery_souls", target)                                                                

                -- Set the ward stacks count to be the same as the caster
                if ward:HasModifier("modifier_extended_necromastery_souls") then
                    local modifier_souls_handler = ward:FindModifierByName("modifier_extended_necromastery_souls")
...
--       LIFE DRAIN           --
--------------------------------
extended_pugna_life_drain = class({})
LinkLuaModifier("modifier_extended_life_drain", "hero/hero_pugna.lua", LUA_MODIFIER_MOTION_NONE)


function extended_pugna_life_drain:IsHiddenWhenStolen()
    return false
end

function extended_pugna_life_drain:GetAssociatedSecondaryAbilities()
    return "extended_pugna_life_drain_end"
end

function extended_pugna_life_drain:OnUpgrade()
    local caster = self:GetCaster()
    local ability_cancel = "extended_pugna_life_drain_end"
...
function extended_pugna_life_drain:GetCooldown(level)
    local caster = self:GetCaster()
...
function extended_pugna_life_drain:CastFilterResultTarget(target)
    if IsServer() then
        local caster = self:GetCaster()
        local modifier_drain = "modifier_extended_life_drain"
...
function extended_pugna_life_drain:GetCustomCastErrorTarget(target)
    local caster = self:GetCaster()
    local modifier_drain = "modifier_extended_life_drain"
...
function extended_pugna_life_drain:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget()
    local sound_cast = "Hero_Pugna.LifeDrain.Cast"    
    local modifier_lifedrain = "modifier_extended_life_drain"
...
-- Life drain modifier
modifier_extended_life_drain = class({})

function modifier_extended_life_drain:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.sound_target_extended = "Extended.PugnaLifeDrainTarget"
    self.sound_target = "Hero_Pugna.LifeDrain.Target"
    self.sound_loop = "Extended.PugnaLifeDrainLoop"
    ...
    self.nether_ward = "npc_extended_pugna_nether_ward"
    ...
    self.base_slow_pct = self.base_slow_pct + self.caster:FindTalentValue("special_bonus_extended_pugna_7")

    -- Play target sounds
    EmitSoundOn(self.sound_target_extended, self.parent)
...
function modifier_extended_life_drain:OnIntervalThink()    
    if IsServer() then
        ...
        if self.is_ally then
            ...
            actual_damage = actual_damage * (1 + self.caster:FindTalentValue("special_bonus_extended_pugna_5") * 0.01)
...
function modifier_extended_life_drain:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_life_drain:GetModifierMoveSpeedBonus_Percentage()
    -- Don't slow allies
    if self.is_ally then
        return nil
    end
...
function modifier_extended_life_drain:IsHidden() return true end 
function modifier_extended_life_drain:IsPurgable() return false end 
function modifier_extended_life_drain:IsDebuff() 
    if self.is_ally then
        return false 
    else
        return true
...
function modifier_extended_life_drain:OnDestroy()    
    -- Remove particles
...
--     LIFE DRAIN CANCEL      --
--------------------------------
extended_pugna_life_drain_end = class({})

function extended_pugna_life_drain_end:IsNetherWardStealable()
    return false
end

function extended_pugna_life_drain_end:IsHiddenWhenStolen()
    return false
end

function extended_pugna_life_drain_end:GetAssociatedPrimaryAbilities()
    return "extended_pugna_life_drain"
end

function extended_pugna_life_drain_end:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local modifier_lifedrain = "modifier_extended_life_drain"
