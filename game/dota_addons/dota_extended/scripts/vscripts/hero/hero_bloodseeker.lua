...
MergeTables(LinkedModifiers,{
	["modifier_extended_bloodrage_buff_stats"] = LUA_MODIFIER_MOTION_NONE,
})
...
function extended_bloodseeker_bloodrage:OnSpellStart()
	...
	hTarget:AddNewModifier(caster, self, "modifier_extended_bloodrage_buff_stats", {duration = self:GetSpecialValueFor("duration")})
	EmitSoundOn("hero_bloodseeker.bloodRage", hTarget)
end
modifier_extended_bloodrage_buff_stats = modifier_extended_bloodrage_buff_stats or class({})

if IsServer() then
	function modifier_extended_bloodrage_buff_stats:OnCreated()
		...
		if self:GetCaster():HasTalent("special_bonus_extended_bloodseeker_1") then
			if self:GetParent():GetTeam() == self:GetCaster():GetTeam() then
				self.ampin = self.ampin - self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_1")
				self.ampout = self.ampout + self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_1")
			else
				self.ampin = self.ampin + self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_1")
				self.ampout = self.ampout -	self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_1")
			end
	...
	function modifier_extended_bloodrage_buff_stats:IsHidden() return false end
	function modifier_extended_bloodrage_buff_stats:IsPurgable() return true end	
	
	function modifier_extended_bloodrage_buff_stats:OnRefresh()
		self.ampin = self:GetAbility():GetSpecialValueFor("damage_increase_pct")
		self.ampout = self:GetAbility():GetSpecialValueFor("damage_increase_pct")
		self.damageout = self:GetAbility():GetSpecialValueFor("damage_increase_pct")
		self.damage = self:GetAbility():GetSpecialValueFor("aoe_damage")
	...
	function modifier_extended_bloodrage_buff_stats:OnIntervalThink()
	...
	function modifier_extended_bloodrage_buff_stats:DeclareFunctions()
		local funcs = {
	...
	function modifier_extended_bloodrage_buff_stats:GetModifierTotalDamageOutgoing_Percentage(params)
		if params.attacker == self:GetParent() then
			...
			if self:GetCaster():HasTalent("special_bonus_extended_bloodseeker_8") then
				local ampPct = self:GetCaster():FindSpecificTalentValue("special_bonus_extended_bloodseeker_8", "value") / self:GetCaster():FindSpecificTalentValue("special_bonus_extended_bloodseeker_8", "value2") -- find amp per pct
	...
	function modifier_extended_bloodrage_buff_stats:GetModifierIncomingDamage_Percentage(params)
	...
	function modifier_extended_bloodrage_buff_stats:OnDeath(params)
...
function modifier_extended_bloodrage_buff_stats:GetEffectName()
	return "particles/hero/bloodseeker/bloodseeker_boiling_blood.vpcf"
end

function modifier_extended_bloodrage_buff_stats:GetStatusEffectName()
	return "particles/status_fx/status_effect_bloodrage.vpcf"
end

function modifier_extended_bloodrage_buff_stats:StatusEffectPriority()
	return 8
end
-------------------------------------------
--				BLOOD RITE
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_blood_bath_buff_stats"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_blood_bath_debuff_silence"] = LUA_MODIFIER_MOTION_NONE,
})
...
function extended_bloodseeker_blood_bath:OnSpellStart()
	...
	Timers:CreateTimer(self:GetSpecialValueFor("delay"), function()
		...
		if #targets > 0 then
			local overheal = caster:AddNewModifier(caster, self, "modifier_extended_blood_bath_buff_stats", {duration = self:GetSpecialValueFor("overheal_duration")})
		end

		local rupture = false
		if caster:HasScepter() and caster:HasAbility("extended_bloodseeker_rupture") then
			rupture = caster:FindAbilityByName("extended_bloodseeker_rupture")
		end
		
		for _,target in pairs(targets) do
			local damage = self:GetSpecialValueFor("damage")
			target:AddNewModifier(caster, self, "modifier_extended_blood_bath_debuff_silence", {duration = self:GetSpecialValueFor("silence_duration")})
			if rupture then
...
modifier_extended_blood_bath_debuff_silence = modifier_extended_blood_bath_debuff_silence or class({})

if IsServer() then
	function modifier_extended_blood_bath_debuff_silence:OnCreated()
		self.cdr = 1 - self:GetAbility():GetTalentSpecialValueFor("cooldown_reduction") / 100
	end
	function modifier_extended_blood_bath_debuff_silence:OnRefresh()
		self.cdr = 1 - self:GetAbility():GetTalentSpecialValueFor("cooldown_reduction") / 100
	end
	function modifier_extended_blood_bath_debuff_silence:DeclareFunctions()
		local funcs = {
			MODIFIER_EVENT_ON_DEATH,
		}
		return funcs
	end
	function modifier_extended_blood_bath_debuff_silence:OnDeath(params)
...
function modifier_extended_blood_bath_debuff_silence:IsHidden() return false end
function modifier_extended_blood_bath_debuff_silence:IsPurgable() return true end
function modifier_extended_blood_bath_debuff_silence:IsDebuff() return true end

function modifier_extended_blood_bath_debuff_silence:CheckState()
	return {[MODIFIER_STATE_SILENCED] = true}
end

function modifier_extended_blood_bath_debuff_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silence.vpcf"
end

function modifier_extended_blood_bath_debuff_silence:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end


modifier_extended_blood_bath_buff_stats = modifier_extended_blood_bath_buff_stats or class({})

function modifier_extended_blood_bath_buff_stats:IsHidden()
	return false
end

function modifier_extended_blood_bath_buff_stats:OnCreated()
...
function modifier_extended_blood_bath_buff_stats:OnRefresh()
	self.overheal = self:GetAbility():GetSpecialValueFor("dmg_to_overheal") * 0.01
	self:SetStackCount(0)
end

function modifier_extended_blood_bath_buff_stats:DeclareFunctions()
	local funcs = {
...
function modifier_extended_blood_bath_buff_stats:OnTakeDamage(params)
...
function modifier_extended_blood_bath_buff_stats:GetModifierExtraHealthBonus(params)
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_thirst_debuff_vision"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_thirst_passive"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_thirst_haste"] = LUA_MODIFIER_MOTION_NONE,
})
...
function extended_bloodseeker_thirst:GetIntrinsicModifierName()
	return "modifier_extended_thirst_passive"
end

modifier_extended_thirst_passive = modifier_extended_thirst_passive or class({})

function modifier_extended_thirst_passive:IsHidden()
	return true
end

function modifier_extended_thirst_passive:OnCreated()	
...
function modifier_extended_thirst_passive:OnRefresh()
	self.minhp = self:GetAbility():GetSpecialValueFor("max_threshold_pct")
	self.maxhp = self:GetAbility():GetSpecialValueFor("visibility_threshold_pct")
	self.movespeed = self:GetAbility():GetSpecialValueFor("bonus_movement_speed") / (self.minhp - self.maxhp)
	self.damage = self:GetAbility():GetSpecialValueFor("bonus_damage") / (self.minhp - self.maxhp)
	...
	self.talent7 = false
	if IsServer() then
		if self:GetParent():HasTalent("special_bonus_extended_bloodseeker_7") then
			self.movespeed = self.movespeed + self:GetParent():FindSpecificTalentValue("special_bonus_extended_bloodseeker_7", "value") / (self.minhp - self.maxhp)
			self.damage = self.damage + self:GetParent():FindSpecificTalentValue("special_bonus_extended_bloodseeker_7", "value2") / (self.minhp - self.maxhp)
			CustomNetTables:SetTableValue("talents", "hero_bloodseeker_talents", {talent7_ms = self.movespeed, talent7_dmg = self.damage})
		end
	end
end

function modifier_extended_thirst_passive:OnIntervalThink()
	if IsServer() then		
		...
		for _,enemy in pairs(enemies) do			
			if self:GetCaster():PassivesDisabled() or not self:GetCaster():IsAlive() then 
				enemy:RemoveModifierByName("modifier_extended_thirst_debuff_vision")
			else
				...
				if enemy:IsAlive() or (not enemy:IsAlive() and enemy.thirstDeathTimer < self.deathstick) then
					if enemy:GetHealthPercent() < self.minhp then
						...
						if enemyHp > (self.minhp - self.maxhp) then 
							...
							enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_thirst_debuff_vision", {})
						elseif enemy:HasModifier("modifier_extended_thirst_debuff_vision") then
							enemy:RemoveModifierByName("modifier_extended_thirst_debuff_vision")
						end
...
function modifier_extended_thirst_passive:DeclareFunctions()
	local funcs = {
...
function modifier_extended_thirst_passive:GetModifierPreAttack_BonusDamage(params)
	return self:GetStackCount() * self.damage
end

function modifier_extended_thirst_passive:GetModifierMoveSpeedBonus_Percentage(params)
	return self:GetStackCount() * self.movespeed
end

function modifier_extended_thirst_passive:GetModifierMoveSpeed_Max()
	return 5000
end

function modifier_extended_thirst_passive:GetEffectName()
	return "particles/units/heroes/hero_bloodseeker/bloodseeker_thirst_owner.vpcf"
end

function modifier_extended_thirst_passive:OnTakeDamage(params)
	if IsServer() then
		if params.attacker:GetTeam() == self:GetCaster():GetTeam() and params.unit:GetTeam() ~= self:GetCaster():GetTeam() and params.attacker:IsRealHero() and params.unit:IsRealHero() then
			...
			local attackList = self:GetCaster():FindAllModifiersByName("modifier_extended_thirst_haste")
			...
			if not confirmTheKill then
				local modifier = self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_thirst_haste", {duration = duration})
...
modifier_extended_thirst_haste = modifier_extended_thirst_haste or class({})

function modifier_extended_thirst_haste:OnCreated()
...
function modifier_extended_thirst_haste:OnRefresh()
	self.movespeed = self:GetAbility():GetSpecialValueFor("bonus_movement_speed_atk")
	self.damage = self:GetAbility():GetSpecialValueFor("bonus_damage_atk")
end

function modifier_extended_thirst_haste:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_extended_thirst_haste:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE,
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,		
	}
...
function modifier_extended_thirst_haste:GetModifierPreAttack_BonusDamage(params)
	return self:GetStackCount() * self.damage
end

function modifier_extended_thirst_haste:GetModifierMoveSpeedBonus_Percentage(params)
	return self:GetStackCount() * self.movespeed
end

function modifier_extended_thirst_haste:IsHidden()
	return true
end


modifier_extended_thirst_debuff_vision = modifier_extended_thirst_debuff_vision or class({})

function modifier_extended_thirst_debuff_vision:DeclareFunctions()
	local funcs = {
...
function modifier_extended_thirst_debuff_vision:GetModifierProvidesFOWVision()
return 1
end

function modifier_extended_thirst_debuff_vision:CheckState()
	local state = {[MODIFIER_STATE_INVISIBLE] = false,}
	return state
end

function modifier_extended_thirst_debuff_vision:GetPriority()
	return MODIFIER_PRIORITY_HIGH
end

function modifier_extended_thirst_debuff_vision:GetEffectName()
	return "particles/units/heroes/hero_bloodseeker/bloodseeker_vision.vpcf"
end

function modifier_extended_thirst_debuff_vision:GetStatusEffectName()
	return "particles/status_fx/status_effect_thirst_vision.vpcf"
end

function modifier_extended_thirst_debuff_vision:StatusEffectPriority()
	return 8
end
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_rupture_debuff_dot"] = LUA_MODIFIER_MOTION_NONE,
})

extended_bloodseeker_rupture = extended_bloodseeker_rupture or class({})

function extended_bloodseeker_rupture:OnSpellStart(target)
	local hTarget = target or self:GetCursorTarget()
	local caster = self:GetCaster()
	if target then
		hTarget:AddNewModifier(caster, self, "modifier_extended_rupture_debuff_dot", {duration = 0.3})
	else
		hTarget:AddNewModifier(caster, self, "modifier_extended_rupture_debuff_dot", {duration = self:GetSpecialValueFor("duration")})
	...
	if hTarget:GetHealthPercent() > self:GetSpecialValueFor("damage_initial_pct") then
		...
		if self:GetCaster():HasTalent("special_bonus_extended_bloodseeker_3") then 
...
modifier_extended_rupture_debuff_dot = modifier_extended_rupture_debuff_dot or class({})

if IsServer() then
	function modifier_extended_rupture_debuff_dot:OnCreated()
		...
		if self:GetCaster():HasTalent("special_bonus_extended_bloodseeker_5") then
			self.attackdamage = self.attackdamage + self.attackdamage * self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_5") * 0.01
			self.castdamage = self.castdamage + self.castdamage * self:GetCaster():FindTalentValue("special_bonus_extended_bloodseeker_5") * 0.01
		end
	...
	function modifier_extended_rupture_debuff_dot:OnRefresh()
		self.movedamage = self:GetAbility():GetSpecialValueFor("movement_damage_pct") / 100
		self.attackdamage = self:GetAbility():GetSpecialValueFor("attack_damage")
		self.castdamage = self:GetAbility():GetSpecialValueFor("cast_damage")
		if self:GetCaster():HasTalent("special_bonus_extended_bloodseeker_5") then
	...
	function modifier_extended_rupture_debuff_dot:OnIntervalThink()
	...
	function modifier_extended_rupture_debuff_dot:DeclareFunctions()
        local funcs = {
	...
	function modifier_extended_rupture_debuff_dot:OnAbilityStart(params)
	...
	function modifier_extended_rupture_debuff_dot:OnAttackStart(params)
        if params.attacker == self:GetParent() then
...
function modifier_extended_rupture_debuff_dot:GetEffectName()
