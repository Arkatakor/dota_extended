...
--     PROXIMITY MINE       --
------------------------------
extended_techies_proximity_mine = class({})

function extended_techies_proximity_mine:IsHiddenWhenStolen()
    return false
end

function extended_techies_proximity_mine:IsNetherWardStealable()
    return false
end



function extended_techies_proximity_mine:CastFilterResultLocation(location)
    if IsServer() then        
        -- Ability properties
        ...
        trigger_range = trigger_range + caster:FindTalentValue("special_bonus_extended_techies_1")
        ...
        for _,unit in pairs(friendly_units) do            
            if unit:GetUnitName() == "npc_extended_techies_proximity_mine" then
                mine_found = true
                break
            end
        end
...
function extended_techies_proximity_mine:GetCustomCastErrorLocation(location)    
    return "Cannot place mine in range of other mines"    
end

function extended_techies_proximity_mine:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
...
function extended_techies_proximity_mine:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
    ...
    mine_placement_count = mine_placement_count + caster:FindTalentValue("special_bonus_extended_techies_7")
...
function PlantProximityMine(caster, ability, spawn_point)
    local mine_ability = "extended_techies_proximity_mine_trigger"

    -- Create the mine unit
    local mine = CreateUnitByName("npc_extended_techies_proximity_mine", spawn_point, true, caster, caster, caster:GetTeamNumber())
...
--     PROXIMITY MINE AI    --
------------------------------
extended_techies_proximity_mine_trigger = class({})
LinkLuaModifier("modifier_extended_proximity_mine", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_proximity_mine_building_res", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)


function extended_techies_proximity_mine_trigger:GetIntrinsicModifierName()
    return "modifier_extended_proximity_mine"
end

-- Proximity mine states modifier
modifier_extended_proximity_mine = class({})

function modifier_extended_proximity_mine:OnCreated()
    if IsServer() then
        -- Ability properties
        ...
        self.modifier_building_res = "modifier_extended_proximity_mine_building_res"
        self.modifier_electrocharge = "modifier_extended_statis_trap_electrocharge"
        self.modifier_inflammable = "modifier_extended_remote_mine_inflammable"
        self.modifier_sign = "modifier_extended_minefield_sign_detection"
        self.detonate_ability = "extended_techies_remote_mine_pinpoint_detonation"
        ...
        -- #1 Talent: Trigger range increase
        self.trigger_range = self.trigger_range + self.caster:FindTalentValue("special_bonus_extended_techies_1")
...
function modifier_extended_proximity_mine:IsHidden() return true end
function modifier_extended_proximity_mine:IsPurgable() return false end
function modifier_extended_proximity_mine:IsDebuff() return false end

function modifier_extended_proximity_mine:OnIntervalThink()
    if IsServer() then     
                    ...
                    for _,enemy in pairs(enemies) do
                        ...
                        if not enemy:HasFlyMovementCapability() then
                            ...
                            for _,remote_mine in pairs(remote_mines) do
                                if remote_mine:GetUnitName() == "npc_extended_techies_remote_mine" then
...
function modifier_extended_proximity_mine:CheckState()
    local state
...
function modifier_extended_proximity_mine:GetPriority()
    return MODIFIER_PRIORITY_NORMAL
end


-- Building fortification modifier
modifier_extended_proximity_mine_building_res = class({})

function modifier_extended_proximity_mine_building_res:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_proximity_mine_building_res:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}

    return decFuncs
end

function modifier_extended_proximity_mine_building_res:GetModifierMagicalResistanceBonus()
    return self.building_magic_resistance
end

function modifier_extended_proximity_mine_building_res:IsHidden() return true end
function modifier_extended_proximity_mine_building_res:IsPurgable() return false end
function modifier_extended_proximity_mine_building_res:IsDebuff() return false end
...
--       STASIS TRAP        --
------------------------------
extended_techies_stasis_trap = class({})

function extended_techies_stasis_trap:IsHiddenWhenStolen()
    return false
end

function extended_techies_stasis_trap:IsNetherWardStealable()
    return false
end

function extended_techies_stasis_trap:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
...
function extended_techies_stasis_trap:GetBehavior()
    local caster = self:GetCaster()

    -- #2 Talent: Stasis Traps can be placed within friendly creeps
    if caster:HasTalent("special_bonus_extended_techies_2") then
...
function extended_techies_stasis_trap:CastFilterResultTarget(target)    
    if IsServer() then
        local caster = self:GetCaster()
...
function extended_techies_stasis_trap:OnAbilityPhaseStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self  
...
function extended_techies_stasis_trap:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
    local target = self:GetCursorTarget()
    ...
    if target then
        local modifier_stasis = target:AddNewModifier(target, ability, "modifier_extended_statis_trap", {})        
        ...
        -- Plant trap
        local trap = CreateUnitByName("npc_extended_techies_stasis_trap", target_point, true, caster, caster, caster:GetTeamNumber())
        local trap_ability = "extended_techies_stasis_trap_trigger"
...
--      STATIS TRAP AI      --
------------------------------
extended_techies_stasis_trap_trigger = class({})
LinkLuaModifier("modifier_extended_statis_trap", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_statis_trap_root", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_statis_trap_electrocharge", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_statis_trap_disarmed", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)


function extended_techies_stasis_trap_trigger:GetIntrinsicModifierName()
    return "modifier_extended_statis_trap" 
end

-- Statis Trap thinker modifier
modifier_extended_statis_trap = class({})


function modifier_extended_statis_trap:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_root = "modifier_extended_statis_trap_root"
        self.modifier_electrocharge = "modifier_extended_statis_trap_electrocharge"
        self.modifier_disarmed = "modifier_extended_statis_trap_disarmed"
        self.modifier_sign = "modifier_extended_minefield_sign_detection"        
        self.modifier_inflammable = "modifier_extended_remote_mine_inflammable"
        self.detonate_ability = "extended_techies_remote_mine_pinpoint_detonation"
...
function modifier_extended_statis_trap:IsHidden() return true end
function modifier_extended_statis_trap:IsPurgable() return false end
function modifier_extended_statis_trap:IsDebuff() return false end

function modifier_extended_statis_trap:OnIntervalThink()
    if IsServer() then
        ...
        if #enemies > 0 then
            ...
            for _,mine in pairs(mines) do
                if mine:GetUnitName() == "npc_extended_techies_stasis_trap" and mine ~= self.caster then
            ...
            if self.owner and self.owner:HasTalent("special_bonus_extended_techies_4") then
                -- Find Remote Mines in the explosion radius and give them inflammable stacks
                for _,mine in pairs(mines) do
                    if mine:GetUnitName() == "npc_extended_techies_remote_mine" then
...
function modifier_extended_statis_trap:CheckState()
    if IsServer() then
        local state
...
-- Root modifier
modifier_extended_statis_trap_root = class({})

function modifier_extended_statis_trap_root:CheckState()
    local state = {[MODIFIER_STATE_ROOTED] = true}
    return state
end

function modifier_extended_statis_trap_root:IsHidden() return false end
function modifier_extended_statis_trap_root:IsPurgable() return true end
function modifier_extended_statis_trap_root:IsDebuff() return true end    

function modifier_extended_statis_trap_root:GetStatusEffectName()
    return "particles/status_fx/status_effect_techies_stasis.vpcf"
end


-- Electrocharge modifier
modifier_extended_statis_trap_electrocharge = class({})

function modifier_extended_statis_trap_electrocharge:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        if self.owner then
            self.base_magnetic_radius = self.base_magnetic_radius + self.owner:FindTalentValue("special_bonus_extended_techies_3")        
        end
...
function modifier_extended_statis_trap_electrocharge:IsHidden() return false end
function modifier_extended_statis_trap_electrocharge:IsPurgable() return true end
function modifier_extended_statis_trap_electrocharge:IsDebuff() return true end    

function modifier_extended_statis_trap_electrocharge:OnIntervalThink()
    if IsServer() then
        ...
        for _,mine in pairs(mines) do            
            if mine:GetUnitName() == "npc_extended_techies_proximity_mine" or mine:GetUnitName() == "npc_extended_techies_stasis_trap" or mine:GetUnitName() == "npc_extended_techies_remote_mine" then
...
function modifier_extended_statis_trap_electrocharge:GetTexture()
    return "techies_stasis_trap"
end


-- Disarmed Statis Traps modifier
modifier_extended_statis_trap_disarmed = class({})

function modifier_extended_statis_trap_disarmed:IsHidden() return false end
function modifier_extended_statis_trap_disarmed:IsPurgable() return false end
function modifier_extended_statis_trap_disarmed:IsDebuff() return false end
...
--        BLAST OFF!        --
------------------------------
extended_techies_blast_off = class({})
LinkLuaModifier("modifier_extended_blast_off", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier("modifier_extended_blast_off_movement", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier("modifier_extended_blast_off_silence", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_BOTH)

function extended_techies_blast_off:IsHiddenWhenStolen()
    return false
end

function extended_techies_blast_off:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
...
function extended_techies_blast_off:IsNetherWardStealable()
    return false
end

function extended_techies_blast_off:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()  
    local piggy_response = "Extended.LittlePiggyWhy"  
    local sound_cast = "Hero_Techies.BlastOff.Cast"
    local modifier_blast = "modifier_extended_blast_off"
    ...
    local pig
    if caster:HasTalent("special_bonus_extended_techies_8") then
        pig = CreateUnitByName("npc_extended_techies_blast_off_piggy", caster:GetAbsOrigin(), false, caster, caster, caster:GetTeamNumber())
...
-- Blast off modifier
modifier_extended_blast_off = class({})

function modifier_extended_blast_off:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()
        self.modifier_movement = "modifier_extended_blast_off_movement"
...
function modifier_extended_blast_off:OnIntervalThink()
...
function modifier_extended_blast_off:IsHidden() return true end
function modifier_extended_blast_off:IsPurgable() return false end
function modifier_extended_blast_off:IsDebuff() return false end

function modifier_extended_blast_off:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_ORDER}

    return decFuncs
end

function modifier_extended_blast_off:OnOrder(keys)
    if IsServer() then
...
function modifier_extended_blast_off:CheckState()
    local state = {[MODIFIER_STATE_ROOTED] = true,
                   [MODIFIER_STATE_DISARMED] = true}
    return state
end

function modifier_extended_blast_off:OnDestroy()
    if IsServer() then
        ...
        if self.parent:GetUnitName() == "npc_extended_techies_blast_off_piggy" then
            Timers:CreateTimer(1, function()
...
modifier_extended_blast_off_movement = class({})

function modifier_extended_blast_off_movement:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_silence = "modifier_extended_blast_off_silence"
        self.proximity_ability = "extended_techies_proximity_mine"
...
function modifier_extended_blast_off_movement:IsHidden() return true end
function modifier_extended_blast_off_movement:IsPurgable() return false end
function modifier_extended_blast_off_movement:RemoveOnDeath() return false end
function modifier_extended_blast_off_movement:IsDebuff() return false end
function modifier_extended_blast_off_movement:IgnoreTenacity() return true end

function modifier_extended_blast_off_movement:UpdateVerticalMotion(me, dt)
    if IsServer() then        
...
function modifier_extended_blast_off_movement:UpdateHorizontalMotion(me, dt)
    if IsServer() then
...
function modifier_extended_blast_off_movement:OnHorizontalMotionInterrupted()
    if IsServer() then
        ...
        if self.caster:HasTalent("special_bonus_extended_techies_5") then
...
function modifier_extended_blast_off_movement:CheckState()
    local state = {[MODIFIER_STATE_ROOTED] = true,
                   [MODIFIER_STATE_DISARMED] = true}
    return state
end
...
modifier_extended_blast_off_silence = class({})

function modifier_extended_blast_off_silence:IsHidden() return false end
function modifier_extended_blast_off_silence:IsPurgable() return true end
function modifier_extended_blast_off_silence:IsDebuff() return true end

function modifier_extended_blast_off_silence:CheckState()
    local state = {[MODIFIER_STATE_SILENCED] = true}
    return state
end

function modifier_extended_blast_off_silence:GetEffectName()
    return "particles/generic_gameplay/generic_silence.vpcf"
end

function modifier_extended_blast_off_silence:GetEffectAttachType()
...
--      REMOTE MINES        --
------------------------------
extended_techies_remote_mine = class({})

function extended_techies_remote_mine:IsHiddenWhenStolen()
    return false
end

function extended_techies_remote_mine:GetAssociatedSecondaryAbilities()
    return "extended_techies_focused_detonate"
end

function extended_techies_remote_mine:IsNetherWardStealable()
    return false
end

function extended_techies_remote_mine:OnUpgrade()
    local caster = self:GetCaster()
    local focused_ability = "extended_techies_focused_detonate"
...
function extended_techies_remote_mine:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
...
function extended_techies_remote_mine:OnAbilityPhaseStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
...
function extended_techies_remote_mine:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
    ...
    local mine_ability = "extended_techies_remote_mine_pinpoint_detonation"
    ...
    -- Plant mine
    local mine = CreateUnitByName("npc_extended_techies_remote_mine", target_point, false, caster, caster, caster:GetTeamNumber())
...
--    PINPOINT DETONATION   --
------------------------------
extended_techies_remote_mine_pinpoint_detonation = class({})
LinkLuaModifier("modifier_extended_remote_mine", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_remote_mine_inflammable", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)

function extended_techies_remote_mine_pinpoint_detonation:IsStealable()
    return false
end

function extended_techies_remote_mine_pinpoint_detonation:ProcsMagicStick()
    return false
end

function extended_techies_remote_mine_pinpoint_detonation:IsNetherWardStealable()
    return false
end

function extended_techies_remote_mine_pinpoint_detonation:GetIntrinsicModifierName()
    return "modifier_extended_remote_mine"
end

function extended_techies_remote_mine_pinpoint_detonation:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self  
    ...
    local modifier_inflammable = "modifier_extended_remote_mine_inflammable"
    local modifier_electrocharge = "modifier_extended_statis_trap_electrocharge"
...
modifier_extended_remote_mine = class({})


function modifier_extended_remote_mine:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_remote_mine:IsHidden() return true end
function modifier_extended_remote_mine:IsPurgable() return false end
function modifier_extended_remote_mine:IsDebuff() return false end

function modifier_extended_remote_mine:CheckState()
...
modifier_extended_remote_mine_inflammable = class({})

function modifier_extended_remote_mine_inflammable:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_remote_mine_inflammable:GetTexture()
    return "techies_remote_mines_self_detonate" 
end

function modifier_extended_remote_mine_inflammable:IsHidden() return false end
function modifier_extended_remote_mine_inflammable:IsPurgable() return false end
function modifier_extended_remote_mine_inflammable:IsDebuff() return false end

function modifier_extended_remote_mine_inflammable:OnStackCountChanged(old_count)
    local stacks = self:GetStackCount()
...
--     FOCUSED DETONATE     --
------------------------------
extended_techies_focused_detonate = class({})
LinkLuaModifier("modifier_extended_focused_detonate", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)

function extended_techies_focused_detonate:IsStealable()
    return false
end

function extended_techies_focused_detonate:IsHiddenWhenStolen()
    return false
end

function extended_techies_focused_detonate:IsNetherWardStealable()
    return false
end

function extended_techies_focused_detonate:GetAOERadius()
    local ability = self
...
function extended_techies_focused_detonate:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()    
    local detonate_ability = "extended_techies_remote_mine_pinpoint_detonation"
...
modifier_extended_focused_detonate = class({})

function modifier_extended_focused_detonate:DeclareFunctions()
    local decFuncs =
    {
...
function modifier_extended_focused_detonate:IsHidden() return true end
function modifier_extended_focused_detonate:IsPurgable() return false end
function modifier_extended_focused_detonate:IsDebuff() return false end

function modifier_extended_focused_detonate:GetModifierIgnoreCastAngle()
    return 1
end

function modifier_extended_focused_detonate:GetModifierDisableTurning()
    return 1
end

function modifier_extended_focused_detonate:IsHidden()
    return false
end

-- Do a stop order after finish casting to prevent turning to the destination point
function modifier_extended_focused_detonate:OnDestroy()
    if IsServer() then
        local stopOrder =
        {
...
--      MINEFIELD SIGN      --
------------------------------
extended_techies_minefield_sign = class({})
LinkLuaModifier("modifier_extended_minefield_sign_aura", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_minefield_sign_detection", "hero/hero_techies.lua", LUA_MODIFIER_MOTION_NONE)

function extended_techies_minefield_sign:IsHiddenWhenStolen()
    return false
end

function extended_techies_minefield_sign:IsNetherWardStealable()
    return false
end

function extended_techies_minefield_sign:IsInnateAbility()
    return true
end

function extended_techies_minefield_sign:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
    local radius = ability:GetSpecialValueFor("radius")

    -- #6 Talent: Minefield radius increase
    radius = radius + caster:FindTalentValue("special_bonus_extended_techies_6")

    return radius
end

function extended_techies_minefield_sign:OnUpgrade()
    local ability = self    
...
function extended_techies_minefield_sign:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target_point = self:GetCursorPosition()
    local sound_cast = "Hero_Techies.Sign"
    local modifier_sign = "modifier_extended_minefield_sign_aura"   
    ...
    -- Create a new sign
    local sign = CreateUnitByName("npc_extended_techies_minefield_sign", target_point, false, caster, caster, caster:GetTeamNumber())
...
-- Sign aura modifier
modifier_extended_minefield_sign_aura = class({})

function modifier_extended_minefield_sign_aura:OnCreated()
    -- Ability proprties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_minefield_sign_aura:IsHidden() return false end
function modifier_extended_minefield_sign_aura:IsPurgable() return false end
function modifier_extended_minefield_sign_aura:IsDebuff() return false end

function modifier_extended_minefield_sign_aura:CheckState()
...
function modifier_extended_minefield_sign_aura:GetAuraEntityReject(target)
    -- Only apply on mines
    if target:GetUnitName() == "npc_extended_techies_proximity_mine" or target:GetUnitName() == "npc_extended_techies_stasis_trap" or target:GetUnitName() == "npc_extended_techies_remote_mine" then
        return false
    end
...
function modifier_extended_minefield_sign_aura:GetAuraRadius()
    -- #6 Talent: Minefield radius increase
    local radius = self.radius + self.caster:FindTalentValue("special_bonus_extended_techies_6")

    return radius
end

function modifier_extended_minefield_sign_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_minefield_sign_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_minefield_sign_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_OTHER
end

function modifier_extended_minefield_sign_aura:GetModifierAura()
    return "modifier_extended_minefield_sign_detection"
end

function modifier_extended_minefield_sign_aura:IsAura()
    if IsServer() then
...
modifier_extended_minefield_sign_detection = class({})

function modifier_extended_minefield_sign_detection:CheckState()
    local state = {[MODIFIER_STATE_TRUESIGHT_IMMUNE] = true}
    return state
end

function modifier_extended_minefield_sign_detection:OnDestroy()
    if IsServer() then
        self.parent = self:GetParent()
        self.detonate_ability = "extended_techies_remote_mine_pinpoint_detonation"
        ...
        Timers:CreateTimer(FrameTime(), function()
            if self.parent:GetUnitName() == "npc_extended_techies_remote_mine" then
