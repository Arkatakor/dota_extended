...
extended_phantom_assassin_stifling_dagger = class({})

LinkLuaModifier("modifier_extended_stifling_dagger_slow", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_stifling_dagger_silence", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_stifling_dagger_bonus_damage", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_stifling_dagger_dmg_reduction", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_phantom_assassin_stifling_dagger:OnSpellStart()
	...
	if caster:HasTalent("special_bonus_extended_phantom_assassin_1") then
    	bonus_damage	=	ability:GetSpecialValueFor("bonus_damage") + self:GetCaster():FindTalentValue("special_bonus_extended_phantom_assassin_1")
  	else
		bonus_damage	=	ability:GetSpecialValueFor("bonus_damage")
	end

	--coup de grace variables
	local ability_crit = caster:FindAbilityByName("modifier_extended_coup_de_grace")
	local ps_coup_modifier = "modifier_extended_phantom_strike_coup_de_grace"
	...
	-- Secondary knives		
	if scepter or caster:HasTalent("special_bonus_extended_phantom_assassin_3") then

		local secondary_knives_thrown = 0

		-- TALENT: +1 Stifling Dagger bonus dagger (like aghs)
		if not scepter and caster:HasTalent("special_bonus_extended_phantom_assassin_3") then
			scepter_dagger_count = self:GetCaster():FindTalentValue("special_bonus_extended_phantom_assassin_3")
			secondary_knives_thrown = scepter_dagger_count
		elseif scepter and caster:HasTalent("special_bonus_extended_phantom_assassin_3") then
			scepter_dagger_count = ability:GetSpecialValueFor("scepter_dagger_count") + self:GetCaster():FindTalentValue("special_bonus_extended_phantom_assassin_3")
		else
...
function extended_phantom_assassin_stifling_dagger:OnProjectileHit( target, location )
	...
	if not target:IsMagicImmune() then
		target:AddNewModifier(caster, self, "modifier_extended_stifling_dagger_silence", {duration = silence_duration})
		target:AddNewModifier(caster, self, "modifier_extended_stifling_dagger_slow", {duration = slow_duration})
	end

	caster:AddNewModifier(caster, self, "modifier_extended_stifling_dagger_dmg_reduction", {})
	caster:AddNewModifier(caster, self, "modifier_extended_stifling_dagger_bonus_damage", {})
	...
	caster:RemoveModifierByName( "modifier_extended_stifling_dagger_bonus_damage" )
	caster:RemoveModifierByName( "modifier_extended_stifling_dagger_dmg_reduction" )
	return true
end
...
modifier_extended_stifling_dagger_slow = class({})

function modifier_extended_stifling_dagger_slow:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
...
function modifier_extended_stifling_dagger_slow:DeclareFunctions()
  local funcs = { MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE, MODIFIER_STATE_PROVIDES_VISION }
  return funcs
end

function modifier_extended_stifling_dagger_slow:GetModifierMoveSpeedBonus_Percentage()
  	return self:GetAbility():GetSpecialValueFor("move_slow");
end

function modifier_extended_stifling_dagger_slow:GetModifierProvidesFOWVision()	return true end

function modifier_extended_stifling_dagger_slow:IsDebuff()		return true end

function modifier_extended_stifling_dagger_slow:IsPurgable()	return true end

-------------------------------------------
-- Stifling Dagger silence modifier
-------------------------------------------

modifier_extended_stifling_dagger_silence = class({})

function modifier_extended_stifling_dagger_silence:OnCreated()
...
function modifier_extended_stifling_dagger_silence:CheckState()
	local states = { [MODIFIER_STATE_SILENCED] = true, }
	return states
end

function modifier_extended_stifling_dagger_silence:IsDebuff() 	return true end

function modifier_extended_stifling_dagger_silence:IsPurgable()	return true end

function modifier_extended_stifling_dagger_silence:IsHidden()		return true end

-------------------------------------------
-- Stifling Dagger bonus damage modifier
-------------------------------------------

modifier_extended_stifling_dagger_bonus_damage = class({})

function modifier_extended_stifling_dagger_bonus_damage:DeclareFunctions()
  local funcs = { MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE }
  return funcs
end

function modifier_extended_stifling_dagger_bonus_damage:GetModifierPreAttack_BonusDamage()
	local ability = self:GetAbility()
  	return ability:GetSpecialValueFor("bonus_damage");
end

function modifier_extended_stifling_dagger_bonus_damage:IsBuff()			return true end

function modifier_extended_stifling_dagger_bonus_damage:IsPurgable() 	return false end

function modifier_extended_stifling_dagger_bonus_damage:IsHidden() 	  return true end

-------------------------------------------
-- Stifling Dagger damage reduction modifier
-------------------------------------------

modifier_extended_stifling_dagger_dmg_reduction = class({})

function modifier_extended_stifling_dagger_dmg_reduction:OnCreated()
	self.ability = self:GetAbility()
	self.damage_reduction = self.ability:GetSpecialValueFor("damage_reduction")
end

function modifier_extended_stifling_dagger_dmg_reduction:DeclareFunctions()
	local decFunc = {MODIFIER_PROPERTY_BASEDAMAGEOUTGOING_PERCENTAGE}
	return decFunc
end

function modifier_extended_stifling_dagger_dmg_reduction:GetModifierBaseDamageOutgoing_Percentage()
...
extended_phantom_assassin_phantom_strike = class({})

LinkLuaModifier("modifier_extended_phantom_strike", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_phantom_strike_coup_de_grace", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
function extended_phantom_assassin_phantom_strike:IsNetherWardStealable() return false end
function extended_phantom_assassin_phantom_strike:CastFilterResultTarget(target)
    if IsServer() then
        local caster = self:GetCaster()
...
function extended_phantom_assassin_phantom_strike:GetCustomCastErrorTarget(target)
	return "dota_hud_error_cant_cast_on_self"
end

function extended_phantom_assassin_phantom_strike:OnSpellStart()
if IsServer() then
	...
	if self.caster:HasTalent("special_bonus_extended_phantom_assassin_2") then
    	self.bonus_attack_speed	=	self.ability:GetSpecialValueFor("bonus_attack_speed") + self.caster:FindTalentValue("special_bonus_extended_phantom_assassin_2")
  	else
		self.bonus_attack_speed =	self.ability:GetSpecialValueFor("bonus_attack_speed")
	end
	...
	if self.target:GetTeamNumber() ~= self.caster:GetTeamNumber() then
		...
		self.caster:AddNewModifier(self.caster, self, "modifier_extended_phantom_strike", { duration= self.buff_duration })

		-- Apply crit modifier
		self.caster:AddNewModifier(self.caster, self, "modifier_extended_phantom_strike_coup_de_grace", { duration= self.buff_duration })

		--Increased Coup de Grace chance for the next 4 attacks
		--TALENT
		if self.caster:HasTalent("special_bonus_extended_phantom_assassin_6") then
			attacks_count = self.caster:FindTalentValue("special_bonus_extended_phantom_assassin_6") + 4
		else			
			attacks_count = self.attacks
		end

		self.caster:SetModifierStackCount( "modifier_extended_phantom_strike_coup_de_grace", self.caster, attacks_count)
...
modifier_extended_phantom_strike = class({})

function modifier_extended_phantom_strike:DeclareFunctions()
  local funcs = { MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT }
  return funcs
end

function modifier_extended_phantom_strike:GetModifierAttackSpeedBonus_Constant()
    local caster = self:GetCaster()
    local ability = self:GetAbility()
...
function modifier_extended_phantom_strike:IsBuff()		  return true end

function modifier_extended_phantom_strike:IsPurgable()  return true end

-------------------------------------------
-- Phantom Strike coup de grace modifier
-------------------------------------------

modifier_extended_phantom_strike_coup_de_grace = class({})

function modifier_extended_phantom_strike_coup_de_grace:DeclareFunctions()
  local funcs = { MODIFIER_EVENT_ON_ATTACK_LANDED }
  return funcs
end

function modifier_extended_phantom_strike_coup_de_grace:OnAttackLanded(keys)
  if IsServer() then
    local caster = self:GetCaster()
    local ability = self:GetAbility()
	local owner = self:GetParent()		
	local modifier_speed = "modifier_extended_phantom_strike"		
...
function modifier_extended_phantom_strike_coup_de_grace:IsBuff()			return true end

function modifier_extended_phantom_strike_coup_de_grace:IsPurgable()	return true end
...
extended_phantom_assassin_blur = class({})

LinkLuaModifier("modifier_extended_blur", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_blur_blur", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE) --wat
LinkLuaModifier("modifier_extended_blur_speed", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_blur_opacity", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_phantom_assassin_blur:GetIntrinsicModifierName()
  return "modifier_extended_blur"
end
...
modifier_extended_blur = class({})

function modifier_extended_blur:OnCreated()
  if IsServer() then
  	-- Ability properties
  	self.caster = self:GetCaster()
  	self.ability = self:GetAbility()
  	self.parent = self:GetParent()
  	self.modifier_aura = "modifier_extended_blur_blur"
	self.modifier_blur_transparent = "modifier_extended_blur_opacity"
	self.modifier_speed = "modifier_extended_blur_speed"
...
function modifier_extended_blur:OnRefresh()
	self:OnCreated()
end

function modifier_extended_blur:OnIntervalThink()
  if IsServer() then
...
function modifier_extended_blur:DeclareFunctions()
...
function modifier_extended_blur:GetModifierEvasion_Constant()
	return self.evasion
end

function modifier_extended_blur:GetModifierIncomingDamage_Percentage()	

	--TALENT: Blur now grants +30% chance to evade any damage	
	if RollPercentage(self.caster:FindTalentValue("special_bonus_extended_phantom_assassin_8")) then
...
function modifier_extended_blur:OnAttackFail(keys)
  if IsServer() then
...
function modifier_extended_blur:IsHidden()	  return true end
function modifier_extended_blur:IsBuff()			return true end
function modifier_extended_blur:IsPurgable()  return false end


-- Evasion speed buff modifier
modifier_extended_blur_speed = class({})

function modifier_extended_blur_speed:OnCreated()    
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()        
...
function modifier_extended_blur_speed:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_blur_speed:OnRefresh()
    if IsServer() then
...
function modifier_extended_blur_speed:IsHidden() return false end
function modifier_extended_blur_speed:IsPurgable() return true end
function modifier_extended_blur_speed:IsDebuff() return false end

function modifier_extended_blur_speed:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT}

    return decFunc
end

function modifier_extended_blur_speed:GetModifierMoveSpeedBonus_Constant()
...
modifier_extended_blur_blur = class({})

function modifier_extended_blur_blur:OnCreated()
...
function modifier_extended_blur_blur:GetStatusEffectName()
	return "particles/hero/phantom_assassin/blur_status_fx.vpcf"
end

function modifier_extended_blur_blur:StatusEffectPriority()  return 11 end

function modifier_extended_blur_blur:CheckState()
	local states = { [MODIFIER_STATE_NOT_ON_MINIMAP_FOR_ENEMIES] = true, }
	return states
end

function modifier_extended_blur_blur:IsHidden()	   return true end
function modifier_extended_blur_blur:IsDebuff()	 return false end
function modifier_extended_blur_blur:IsPurgable()  return false end

-------------------------------------------
-- Blur opacity modifier
-------------------------------------------

modifier_extended_blur_opacity = class({})

function modifier_extended_blur_opacity:IsHidden()	return false end
function modifier_extended_blur_opacity:IsDebuff()	return false end
function modifier_extended_blur_opacity:IsPurgable()return false end

function modifier_extended_blur_opacity:DeclareFunctions()
    return {MODIFIER_PROPERTY_INVISIBILITY_LEVEL}
end

function modifier_extended_blur_opacity:GetModifierInvisibilityLevel()
    return 1
end

function modifier_extended_blur_opacity:IsHidden()
    return true
end
...
extended_phantom_assassin_coup_de_grace = class({})

LinkLuaModifier("modifier_extended_coup_de_grace", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_coup_de_grace_crit", "hero/hero_phantom_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_phantom_assassin_coup_de_grace:GetIntrinsicModifierName()
  return "modifier_extended_coup_de_grace"
end
...
modifier_extended_coup_de_grace = class({})

function modifier_extended_coup_de_grace:OnCreated()	
	-- Ability properties
	self.caster = self:GetCaster()
	self.ability = self:GetAbility()
	self.parent = self:GetParent()
	self.ps_coup_modifier = "modifier_extended_phantom_strike_coup_de_grace"			
	self.modifier_stacks = "modifier_extended_coup_de_grace_crit"
...
function modifier_extended_coup_de_grace:OnRefresh()
	self:OnCreated()
end

function modifier_extended_coup_de_grace:DeclareFunctions()
  local funcs = {MODIFIER_PROPERTY_PREATTACK_CRITICALSTRIKE}
  return funcs
end

function modifier_extended_coup_de_grace:GetModifierPreAttack_CriticalStrike(keys)	
	if IsServer() then		
        ...
        if self.caster:HasModifier(self.ps_coup_modifier) then
            ...
            if ps_coup_modifier_handler then
				...
				bonus_coup_de_grace_chance = bonus_coup_de_grace_chance + self:GetCaster():FindTalentValue("special_bonus_extended_phantom_assassin_4")				
        ...
        if RollPseudoRandom(crit_chance_total, self) then        	
			...
			crit_duration = crit_duration + self.caster:FindTalentValue("special_bonus_extended_phantom_assassin_7")			
			...
			local crit_bonus = self.crit_bonus + self.caster:FindTalentValue("special_bonus_extended_phantom_assassin_5")
            return crit_bonus
        end
...
function modifier_extended_coup_de_grace:IsPassive() return true end

function modifier_extended_coup_de_grace:IsHidden()	
...
modifier_extended_coup_de_grace_crit = class({})

function modifier_extended_coup_de_grace_crit:OnCreated()    
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()        
...
function modifier_extended_coup_de_grace_crit:OnIntervalThink()
    if IsServer() then
        -- Check if there are any stacks left on the table
        if #self.stacks_table > 0 then
...
function modifier_extended_coup_de_grace_crit:OnRefresh()
    if IsServer() then
        -- Insert new stack values
        table.insert(self.stacks_table, GameRules:GetGameTime())
    end
...
function modifier_extended_coup_de_grace_crit:IsHidden() return false end
function modifier_extended_coup_de_grace_crit:IsPurgable() return true end
function modifier_extended_coup_de_grace_crit:IsDebuff() return false end


function modifier_extended_coup_de_grace_crit:DeclareFunctions()
    local decFunc = {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE}

    return decFunc
end

function modifier_extended_coup_de_grace_crit:GetModifierPreAttack_BonusDamage()	
