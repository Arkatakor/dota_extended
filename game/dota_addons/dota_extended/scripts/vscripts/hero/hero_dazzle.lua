...
function extended_dazzle_poison_touch:GetCooldown()
	...
	local cooldownbonus = self:GetCaster():FindTalentValue("special_bonus_extended_dazzle_5")
...
function extended_dazzle_shallow_grave:GetManaCost()
	if self:GetCaster():HasTalent("special_bonus_extended_dazzle_1") then 
		return 0 end
			...
			if parent == victim and math.floor(health) <= 1 and not parent:FindModifierByName("modifier_extended_dazzle_shallow_grave") then
				...
				if not self.isActive and not parent:PassivesDisabled() then
					...
					Timers:CreateTimer(nothl_duration, function()
						...
						nothl_cooldown = nothl_cooldown + parent:FindTalentValue("special_bonus_extended_dazzle_7")
						print(nothl_cooldown)
...
function extended_dazzle_shadow_wave:WaveBounce(target)
	if IsServer() then
		...
		if caster:HasTalent("special_bonus_extended_dazzle_2") then
			bounceDistance = bounceDistance + caster:FindTalentValue("special_bonus_extended_dazzle_2")
		end
...
function extended_dazzle_shadow_wave:WaveHit(unit)
	if IsServer() then
		...
		if caster:HasTalent("special_bonus_extended_dazzle_3") then
			damageRadius = damageRadius + caster:FindTalentValue("special_bonus_extended_dazzle_3")
		end
		
		if caster:HasTalent("special_bonus_extended_dazzle_4") then
			damage = damage + caster:FindTalentValue("special_bonus_extended_dazzle_4")
		end
...
function extended_dazzle_weave:GetCooldown()
	local cooldown = self:GetSpecialValueFor("cooldown")
	if self:GetCaster():HasTalent("special_bonus_extended_dazzle_8") then
		cooldown = cooldown + self:GetCaster():FindTalentValue("special_bonus_extended_dazzle_8")
	end
...
function extended_dazzle_weave:OnSpellStart()
	if IsServer() then
		...
		if caster:HasTalent("special_bonus_extended_dazzle_6") then
...
function modifier_extended_dazzle_weave_buff:OnCreated()
	if IsServer() then
		...
		if not parent:IsBuilding() then
			if self:GetAbility():GetCaster():HasTalent("special_bonus_extended_dazzle_6") then
...
for i = 1,8 do
	LinkLuaModifier("modifier_special_bonus_extended_dazzle_"..i, "hero/hero_dazzle", LUA_MODIFIER_MOTION_NONE)
end

modifier_special_bonus_extended_dazzle_1 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_2 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_3 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_4 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_5 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_6 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_7 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })
	
modifier_special_bonus_extended_dazzle_8 = class({
	IsHidden      = function(self) return true  end,
	RemoveOnDeath = function(self) return false end })