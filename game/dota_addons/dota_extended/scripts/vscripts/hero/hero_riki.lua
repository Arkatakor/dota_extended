...
MergeTables(LinkedModifiers,{
	["modifier_extended_smoke_screen_debuff_miss"] = LUA_MODIFIER_MOTION_NONE,			-- Silence, miss, turn rate slow, slow
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_smoke_screen_handler"] = LUA_MODIFIER_MOTION_NONE,			-- Aura that applies the debuff
	["modifier_extended_smoke_screen_vision"] = LUA_MODIFIER_MOTION_NONE,	-- Reduces vision in comparison to smoke centre
})
extended_riki_smoke_screen = extended_riki_smoke_screen or class({})
function extended_riki_smoke_screen:GetBehavior()
	return DOTA_ABILITY_BEHAVIOR_POINT + DOTA_ABILITY_BEHAVIOR_AOE end

function extended_riki_smoke_screen:GetCooldown( nLevel )
	return self.BaseClass.GetCooldown( self, nLevel ) - self:GetCaster():FindTalentValue("special_bonus_extended_riki_2") end

function extended_riki_smoke_screen:GetAOERadius()
	return self:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_1") end
...
function extended_riki_smoke_screen:OnSpellStart()
	if IsServer() then
		...
		local aoe = self:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_1")
		local smoke_handler = "modifier_extended_smoke_screen_handler"
...
-----	Smoke Screen Aura	-----
---------------------------------
modifier_extended_smoke_screen_handler = modifier_extended_smoke_screen_handler or class({})
function modifier_extended_smoke_screen_handler:IsPurgable() return false end
function modifier_extended_smoke_screen_handler:IsHidden() return true end
function modifier_extended_smoke_screen_handler:IsAura() return true end

function modifier_extended_smoke_screen_handler:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY end

function modifier_extended_smoke_screen_handler:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO end

function modifier_extended_smoke_screen_handler:GetModifierAura()
	return "modifier_extended_smoke_screen_debuff_miss" end
	
function modifier_extended_smoke_screen_handler:GetAuraRadius()	
	local ability = self:GetAbility()
	local aoe = ability:GetSpecialValueFor("area_of_effect")
	return aoe	
end

function modifier_extended_smoke_screen_handler:OnCreated()
	if IsServer() then
...
function modifier_extended_smoke_screen_handler:OnIntervalThink()
	local ability = self:GetAbility()
	...
	for _, unit in pairs(targets) do
		...
		if not parent.afflicted[unit:entindex()] then
			local mod = unit:AddNewModifier(caster, self:GetAbility(), "modifier_extended_smoke_screen_vision", {})
			...
			local duplicateMods = unit:FindAllModifiersByName("modifier_extended_smoke_screen_vision")
...
function modifier_extended_smoke_screen_handler:OnDestroy()
	if IsServer() then
...
-----	Smoke Screen Debuff	  -----
-----------------------------------
modifier_extended_smoke_screen_debuff_miss = modifier_extended_smoke_screen_debuff_miss or class({})
function modifier_extended_smoke_screen_debuff_miss:IsPurgable() return false end
function modifier_extended_smoke_screen_debuff_miss:IsHidden() return false end
function modifier_extended_smoke_screen_debuff_miss:IsDebuff() return true end

function modifier_extended_smoke_screen_debuff_miss:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf" end

function modifier_extended_smoke_screen_debuff_miss:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW end

function modifier_extended_smoke_screen_debuff_miss:CheckState()
	local state = { [MODIFIER_STATE_SILENCED] = true}
	return state
end

function modifier_extended_smoke_screen_debuff_miss:DeclareFunctions()
...
function modifier_extended_smoke_screen_debuff_miss:GetModifierMiss_Percentage()
	local ability = self:GetAbility()
	local miss_chance = ability:GetSpecialValueFor("miss_chance")
	return miss_chance
end

function modifier_extended_smoke_screen_debuff_miss:GetModifierTurnRate_Percentage()
	local ability = self:GetAbility()
	local turn_slow = ability:GetSpecialValueFor("turn_rate_slow")*-1
	return turn_slow
end

function modifier_extended_smoke_screen_debuff_miss:GetModifierMoveSpeedBonus_Percentage()
	local ability = self:GetAbility()
...
-------------------------------------------
modifier_extended_smoke_screen_vision = modifier_extended_smoke_screen_vision or class({})
function modifier_extended_smoke_screen_vision:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_extended_smoke_screen_vision:IsPurgable() return false end
function modifier_extended_smoke_screen_vision:IsHidden() return true end
function modifier_extended_smoke_screen_vision:IsDebuff() return true end

function modifier_extended_smoke_screen_vision:DeclareFunctions()
	local funcs = {	MODIFIER_PROPERTY_BONUS_VISION_PERCENTAGE, }
	return funcs
end

function modifier_extended_smoke_screen_vision:GetBonusVisionPercentage()
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_blink_strike_debuff_turn"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_blink_strike_thinker"] = LUA_MODIFIER_MOTION_NONE,
	["modifier_extended_blink_strike_cmd"] = LUA_MODIFIER_MOTION_NONE,
})

extended_riki_blink_strike = extended_riki_blink_strike or class({})
function extended_riki_blink_strike:IsHiddenWhenStolen() return false end
function extended_riki_blink_strike:IsRefreshable() return true end
function extended_riki_blink_strike:IsStealable() return true end
function extended_riki_blink_strike:IsNetherWardStealable() return false end
-------------------------------------------

function extended_riki_blink_strike:GetCastRange(location , target)
	if IsServer() then
...
function extended_riki_blink_strike:OnAbilityPhaseStart()
	if IsServer() then
...
function extended_riki_blink_strike:OnAbilityPhaseInterrupted()
	if IsServer() then
		if self.thinker then
			local hCaster = self:GetCaster()
			self.thinker:Destroy()
			...
			self.thinker = hCaster:AddNewModifier(hCaster, self, "modifier_extended_blink_strike_thinker", {target = self.hTarget:entindex()})
		end
	end
end
...
function extended_riki_blink_strike:OnSpellStart()
	if IsServer() then
		...
		if self.tMarkedTargets then
			...
			self.hCaster:AddNewModifier(self.hCaster, self, "modifier_extended_blink_strike_cmd", {duration = jump_duration})
			...
			self.hCaster:AddNewModifier(self.hCaster, self, "modifier_extended_blink_strike_cmd", {duration = FrameTime()})
		end
		...
		Timers:CreateTimer(jump_duration, function()
			...
			if (hTarget:GetTeamNumber() ~= self.hCaster:GetTeamNumber()) then
				...
				hTarget:AddNewModifier(self.hCaster, self, "modifier_extended_blink_strike_debuff_turn", {duration = self.duration})
			end
...
function extended_riki_blink_strike:DoJumpAttack(hTarget, hNextTarget)
...
-------------------------------------------
modifier_extended_blink_strike_thinker = modifier_extended_blink_strike_thinker or class({})
function modifier_extended_blink_strike_thinker:IsDebuff() return false end
function modifier_extended_blink_strike_thinker:IsHidden() return false end
function modifier_extended_blink_strike_thinker:IsPurgable() return false end
function modifier_extended_blink_strike_thinker:IsPurgeException() return false end
function modifier_extended_blink_strike_thinker:IsStunDebuff() return false end
-------------------------------------------
function modifier_extended_blink_strike_thinker:DeclareFunctions()
    local decFuns =
    {
...
function modifier_extended_blink_strike_thinker:OnOrder(params)    
    if IsServer() then
		if params.unit == self:GetCaster() and self.created_flag then
			local activity = params.unit:GetCurrentActiveAbility()
			if activity then
				if not (activity:GetName() == "extended_riki_blink_strike") then
...
function modifier_extended_blink_strike_thinker:OnCreated(params)
	if IsServer() then
		self.hCaster = self:GetCaster()
...
function modifier_extended_blink_strike_thinker:OnIntervalThink()
	if IsServer() then
...
-----	Blink Strike Debuff	  -----
-----------------------------------
modifier_extended_blink_strike_debuff_turn = modifier_extended_blink_strike_debuff_turn or class({})
function modifier_extended_blink_strike_debuff_turn:IsPurgable() return true end
function modifier_extended_blink_strike_debuff_turn:IsHidden() return false end
function modifier_extended_blink_strike_debuff_turn:IsDebuff() return true end

function modifier_extended_blink_strike_debuff_turn:DeclareFunctions()
	local funcs = { MODIFIER_PROPERTY_TURN_RATE_PERCENTAGE, }
	return funcs
end

function modifier_extended_blink_strike_debuff_turn:OnCreated()
	self.slow_pct = self:GetAbility():GetSpecialValueFor("turn_rate_slow") * (-1)
end

function modifier_extended_blink_strike_debuff_turn:GetModifierTurnRate_Percentage()
...
---------------------------------------------------------------
modifier_extended_blink_strike_cmd = modifier_extended_blink_strike_cmd or class({})
function modifier_extended_blink_strike_cmd:IsPurgable() return false end
function modifier_extended_blink_strike_cmd:IsHidden() return true end
function modifier_extended_blink_strike_cmd:IsDebuff() return false end

function modifier_extended_blink_strike_cmd:GetPriority()
	return MODIFIER_PRIORITY_HIGH 
end

function modifier_extended_blink_strike_cmd:OnCreated()
	if IsServer() then
		local hCaster = self:GetCaster()
...
function modifier_extended_blink_strike_cmd:CheckState()
	if IsServer() then
...
function modifier_extended_blink_strike_cmd:DeclareFunctions()
    local decFuns =
    {
...
function modifier_extended_blink_strike_cmd:GetActivityTranslationModifiers()
...
function modifier_extended_blink_strike_cmd:GetModifierDisableTurning()
	return 1
end
---------------------------------------------------------------------
--------------------	  Cloak and Dagger		 --------------------
---------------------------------------------------------------------
extended_riki_cloak_and_dagger = extended_riki_cloak_and_dagger or class({})
...
LinkLuaModifier( "modifier_extended_riki_backstab_translation", "hero/hero_riki.lua", LUA_MODIFIER_MOTION_NONE )		-- Attack translate
...
----------------------------------------------------------
modifier_extended_riki_cloak_and_dagger = modifier_extended_riki_cloak_and_dagger or class({})
...
function modifier_extended_riki_cloak_and_dagger:CheckState()
	if IsServer() then
		local ability = self:GetAbility()
		local parent = self:GetParent()
		local fade_time = ability:GetSpecialValueFor("fade_time") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_6")
...
function modifier_extended_riki_cloak_and_dagger:OnAttackLanded( keys )
	if IsServer() then
		...
		if parent == attacker then
			...
			-- Get values
			local fade_time = ability:GetSpecialValueFor("fade_time") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_6")
			local agility_multiplier = ability:GetSpecialValueFor("agility_damage_multiplier") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_5")
			...
			if not parent:HasModifier("modifier_extended_riki_tricks_of_the_trade_primary") and not target:IsBuilding() and not parent:PassivesDisabled() then
				...
				if result_angle >= (180 - (ability:GetSpecialValueFor("backstab_angle") / 2)) and result_angle <= (180 + (ability:GetSpecialValueFor("backstab_angle") / 2)) then
					...
					parent:AddNewModifier(parent, self, "modifier_extended_riki_backstab_translation", {duration = parent:GetAttackSpeed()})
					
				
				-- If the attacker is not in backstab angle but the target has the smoke screen modifier
				elseif target:HasModifier("modifier_extended_smoke_screen_debuff_miss") then
...
----------------------------------------------
modifier_extended_riki_invisibility = modifier_extended_riki_invisibility or class({})
...
----------------------------------------------
modifier_extended_riki_backstab_translation = modifier_extended_riki_backstab_translation or class({})
function modifier_extended_riki_backstab_translation:IsPurgable() return false end
function modifier_extended_riki_backstab_translation:IsDebuff() return false end
function modifier_extended_riki_backstab_translation:IsHidden()	return true end

function modifier_extended_riki_backstab_translation:DeclareFunctions()
	local funcs = { MODIFIER_PROPERTY_TRANSLATE_ACTIVITY_MODIFIERS, }
	return funcs
end

function modifier_extended_riki_backstab_translation:GetActivityTranslationModifiers()
	if self:GetParent():GetName() == "npc_dota_hero_riki" then
		return "backstab"
	end
	return 0
...
function extended_riki_tricks_of_the_trade:GetChannelTime()
	return 5
end
...
function extended_riki_tricks_of_the_trade:GetCastRange()
	...
	return self:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_8")
end

function extended_riki_tricks_of_the_trade:GetAOERadius()
	return self:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_8") end
	
function extended_riki_tricks_of_the_trade:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local origin = caster:GetAbsOrigin()
		local aoe = self:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_8")
...
function modifier_extended_riki_tricks_of_the_trade_primary:GetModifierAttackRangeBonus()
	local ability = self:GetAbility()
	local aoe = ability:GetSpecialValueFor("area_of_effect") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_8")
	return aoe
end
...
function modifier_extended_riki_tricks_of_the_trade_primary:OnCreated()
	if IsServer() then
		local ability = self:GetAbility()
		local interval = ability:GetSpecialValueFor("attack_interval") + self:GetCaster():FindTalentValue("special_bonus_extended_riki_7")
