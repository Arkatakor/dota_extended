...
MergeTables(LinkedModifiers,{
	["modifier_extended_mana_break_passive"] = LUA_MODIFIER_MOTION_NONE,
})
extended_antimage_mana_break = extended_antimage_mana_break or class({})
function extended_antimage_mana_break:GetIntrinsicModifierName()
	return "modifier_extended_mana_break_passive"
end

-- Mana break modifier
modifier_extended_mana_break_passive = modifier_extended_mana_break_passive or class({})

function modifier_extended_mana_break_passive:IsHidden()
	return true
end

function modifier_extended_mana_break_passive:IsPurgable()
	return false
end

function modifier_extended_mana_break_passive:DeclareFunctions()	
...
function modifier_extended_mana_break_passive:OnCreated()
	if IsServer() then
...
function modifier_extended_mana_break_passive:OnRefresh()
	if IsServer() then
		self.ability = self:GetAbility()
		self.parent = self:GetParent()
...
function modifier_extended_mana_break_passive:OnAttackStart(keys)
	if IsServer() then
...
function modifier_extended_mana_break_passive:OnAttackLanded(keys)
	if IsServer() then
		local attacker = keys.attacker
		local target = keys.target
...
function modifier_extended_mana_break_passive:GetModifierBaseAttack_BonusDamage(params)
	if IsServer() then
...
extended_antimage_blink = extended_antimage_blink or class({})
function extended_antimage_blink:IsNetherWardStealable() return false end
...
function extended_antimage_blink:GetCooldown( nLevel )
	return self.BaseClass.GetCooldown( self, nLevel ) - self:GetCaster():FindTalentValue("special_bonus_extended_antimage_1")
end
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_spell_shield_buff_reflect"] = LUA_MODIFIER_MOTION_NONE,
})
-- Hidden Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_spell_shield_buff_passive"] = LUA_MODIFIER_MOTION_NONE,
})
extended_antimage_spell_shield = extended_antimage_spell_shield or class({})
...
function extended_antimage_spell_shield:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local ability = self
		local active_modifier = "modifier_extended_spell_shield_buff_reflect"
...
function extended_antimage_spell_shield:GetIntrinsicModifierName()
	return "modifier_extended_spell_shield_buff_passive"
end
...
local function SpellReflect(parent, params)
	...
	if ( not exception_spell[reflected_spell_name] ) and (not target:HasModifier("modifier_extended_spell_shield_buff_reflect") ) then
		local ability
...
modifier_extended_spell_shield_buff_passive = modifier_extended_spell_shield_buff_passive or class({})

function modifier_extended_spell_shield_buff_passive:IsHidden()
	return true
end

function modifier_extended_spell_shield_buff_passive:IsDebuff()
	return false
end

function modifier_extended_spell_shield_buff_passive:DeclareFunctions()	
...
function modifier_extended_spell_shield_buff_passive:OnCreated()
	if IsServer() then
...
function modifier_extended_spell_shield_buff_passive:OnRefresh()
	if IsServer() then
		self.duration = self:GetAbility():GetSpecialValueFor("active_duration")
		self.magic_resistance = self:GetAbility():GetSpecialValueFor("magic_resistance")
	end
...
function modifier_extended_spell_shield_buff_passive:GetModifierMagicalResistanceBonus(params)
	if IsServer() then
		return self.magic_resistance
	end
end

function modifier_extended_spell_shield_buff_passive:GetReflectSpell( params )
	if IsServer() then
...
function modifier_extended_spell_shield_buff_passive:GetAbsorbSpell( params )
	if IsServer() then
		local parent = self:GetParent()
		if ( parent:HasScepter() ) and ( self:GetAbility():IsCooldownReady() ) then
...
modifier_extended_spell_shield_buff_reflect = modifier_extended_spell_shield_buff_reflect or class({})

function modifier_extended_spell_shield_buff_reflect:IsHidden()
	return false
end

function modifier_extended_spell_shield_buff_reflect:IsDebuff()
	return false
end

function modifier_extended_spell_shield_buff_reflect:IsPurgable()
    return false
end

function modifier_extended_spell_shield_buff_reflect:DeclareFunctions()	
		local decFuncs = {
			MODIFIER_PROPERTY_ABSORB_SPELL,
			MODIFIER_PROPERTY_REFLECT_SPELL
						 }
...
function modifier_extended_spell_shield_buff_reflect:OnCreated( params )
    if IsServer() then

    end
end

function modifier_extended_spell_shield_buff_reflect:GetReflectSpell( params )
	if IsServer() then
		return SpellReflect(self:GetParent(), params)
	end
end

function modifier_extended_spell_shield_buff_reflect:GetAbsorbSpell( params )
	if IsServer() then
...
function modifier_extended_spell_shield_buff_passive:OnIntervalThink()
    if IsServer() then
...
-- Visible Modifiers:
MergeTables(LinkedModifiers,{
	["modifier_extended_mana_void_stunned"] = LUA_MODIFIER_MOTION_NONE,
})
extended_antimage_mana_void = extended_antimage_mana_void or class({})
...
function extended_antimage_mana_void:OnSpellStart()
	if IsServer() then
		...
		local modifier_ministun = "modifier_extended_mana_void_stunned"
