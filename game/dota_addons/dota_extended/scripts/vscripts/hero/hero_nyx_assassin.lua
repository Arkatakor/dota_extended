...
extended_nyx_assassin_impale = class({})
LinkLuaModifier("modifier_extended_impale_suffering_aura", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_impale_suffering", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_impale_stun", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_nyx_assassin_impale:IsHiddenWhenStolen()
    return false
end

function extended_nyx_assassin_impale:OnUnStolen()
    local caster = self:GetCaster()
    local modifier_aura = "modifier_extended_impale_suffering_aura"
...
function extended_nyx_assassin_impale:GetIntrinsicModifierName()
    return "modifier_extended_impale_suffering_aura"
end

function extended_nyx_assassin_impale:GetCastRange(location, target)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function extended_nyx_assassin_impale:GetCooldown(level)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local modifier_burrowed = "modifier_nyx_assassin_burrow"
...
function extended_nyx_assassin_impale:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function extended_nyx_assassin_impale:OnProjectileHit(target, location)    
    ...
    local modifier_stun = "modifier_extended_impale_stun"
    local modifier_suffering = "modifier_extended_impale_suffering"
    ...
    damage = damage + caster:FindTalentValue("special_bonus_extended_nyx_assassin_1")
...
modifier_extended_impale_suffering_aura = class({})

function modifier_extended_impale_suffering_aura:GetAuraRadius()
    return 25000 --global
end

function modifier_extended_impale_suffering_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS
end

function modifier_extended_impale_suffering_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_impale_suffering_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO
end

function modifier_extended_impale_suffering_aura:GetModifierAura()
    return "modifier_extended_impale_suffering"
end

function modifier_extended_impale_suffering_aura:IsAura()
    return true
end

function modifier_extended_impale_suffering_aura:IsAuraActiveOnDeath()
    return true
end

function modifier_extended_impale_suffering_aura:IsDebuff() return true end
function modifier_extended_impale_suffering_aura:IsHidden() return true end
function modifier_extended_impale_suffering_aura:IsPermanent() return true end
function modifier_extended_impale_suffering_aura:IsPurgable() return false end

-- Relive Suffering damage counter
modifier_extended_impale_suffering = class({})

function modifier_extended_impale_suffering:OnCreated()
    if IsServer() then
        -- Ability properties
...
function modifier_extended_impale_suffering:OnIntervalThink()
    if IsServer() then
        -- #4 Talent: Impale Relive Suffering duration increase
        if self.caster:HasTalent("special_bonus_extended_nyx_assassin_4") and not self.talent_4_applied then
            self.damage_duration = self.damage_duration + self.caster:FindTalentValue("special_bonus_extended_nyx_assassin_4")
...
function modifier_extended_impale_suffering:DeclareFunctions()
    local decFuncs = {MODIFIER_EVENT_ON_TAKEDAMAGE}

    return decFuncs
end

function modifier_extended_impale_suffering:OnTakeDamage(keys)
    if IsServer() then
...
function modifier_extended_impale_suffering:IsHidden() return true end
function modifier_extended_impale_suffering:IsPurgable() return false end
function modifier_extended_impale_suffering:IsDebuff() return true end
function modifier_extended_impale_suffering:IsPermanent() return true end


-- Impale stun modifier
modifier_extended_impale_stun = class({})

function modifier_extended_impale_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_impale_stun:IsHidden() return false end
function modifier_extended_impale_stun:IsPurgeException() return true end
function modifier_extended_impale_stun:IsStunDebuff() return true end
...
extended_nyx_assassin_mana_burn = class({})
LinkLuaModifier("modifier_extended_mana_burn_parasite", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_mana_burn_parasite_charged", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)


function extended_nyx_assassin_mana_burn:IsHiddenWhenStolen()
    return false
end

function extended_nyx_assassin_mana_burn:GetCastRange(location, target)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local modifier_burrowed = "modifier_nyx_assassin_burrow"
...
function extended_nyx_assassin_mana_burn:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_parasite = "modifier_extended_mana_burn_parasite"
    ...
    intelligence_mult = intelligence_mult + caster:FindTalentValue("special_bonus_extended_nyx_assassin_5")
...
modifier_extended_mana_burn_parasite = class({})

function modifier_extended_mana_burn_parasite:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_charged = "modifier_extended_mana_burn_parasite_charged"
        ...
        self.explosion_delay = self.explosion_delay - self.caster:FindTalentValue("special_bonus_extended_nyx_assassin_2")
...
function modifier_extended_mana_burn_parasite:OnIntervalThink()
    if IsServer() then
...
function modifier_extended_mana_burn_parasite:IsHidden() return false end
function modifier_extended_mana_burn_parasite:IsPurgable() return true end
function modifier_extended_mana_burn_parasite:IsDebuff() return true end
 

-- Mind Bug parasite charge (active) modifier
modifier_extended_mana_burn_parasite_charged = class({})

function modifier_extended_mana_burn_parasite_charged:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.parent = self:GetParent()
        self.sound_charge = "Extended.Nyx_ManaBurnCharge"
        self.sound_explosion = "Extended.Nyx_ManaBurnExplosion"
...
function modifier_extended_mana_burn_parasite_charged:OnDestroy()
    if IsServer() then
...
function modifier_extended_mana_burn_parasite_charged:IsHidden() return false end
function modifier_extended_mana_burn_parasite_charged:IsPurgable() return true end
function modifier_extended_mana_burn_parasite_charged:IsDebuff() return true end

-------------------------------------------------
--              SPIKED CARAPACE                --
-------------------------------------------------

extended_nyx_assassin_spiked_carapace = class({})
LinkLuaModifier("modifier_extended_spiked_carapace", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_spiked_carapace_stun", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_nyx_assassin_spiked_carapace:IsHiddenWhenStolen()
    return false
end

function extended_nyx_assassin_spiked_carapace:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_spikes = "modifier_extended_spiked_carapace"    
    ...
    reflect_duration = reflect_duration + caster:FindTalentValue("special_bonus_extended_nyx_assassin_7")
...
modifier_extended_spiked_carapace = class({})

function modifier_extended_spiked_carapace:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        self.vendetta_ability = self.caster:FindAbilityByName("extended_nyx_assassin_vendetta") 
        self.particle_spikes = "particles/units/heroes/hero_nyx_assassin/nyx_assassin_spiked_carapace.vpcf"
        self.modifier_stun = "modifier_extended_spiked_carapace_stun"
        self.modifier_vendetta = "modifier_extended_vendetta_charge"
        ...
        self.damage_reflection_pct = self.damage_reflection_pct + self.caster:FindTalentValue("special_bonus_extended_nyx_assassin_3")
...
function modifier_extended_spiked_carapace:IsHidden() return false end
function modifier_extended_spiked_carapace:IsPurgable() return false end
function modifier_extended_spiked_carapace:IsDebuff() return false end

function modifier_extended_spiked_carapace:DeclareFunctions()
...
function modifier_extended_spiked_carapace:GetModifierIncomingDamage_Percentage()
    return -100
end

function modifier_extended_spiked_carapace:OnTakeDamage(keys)
    if IsServer() then
        ...
        if unit == self.caster then
            ...
            if attacker:HasModifier("modifier_extended_spiked_carapace") then
                return nil
            end
...
modifier_extended_spiked_carapace_stun = class({})

function modifier_extended_spiked_carapace_stun:OnCreated()
    if IsServer() then
        self.parent = self:GetParent()
...
function modifier_extended_spiked_carapace_stun:IsHidden() return false end
function modifier_extended_spiked_carapace_stun:IsPurgeException() return true end
function modifier_extended_spiked_carapace_stun:IsStunDebuff() return true end

function modifier_extended_spiked_carapace_stun:CheckState()
    local state = {[MODIFIER_STATE_STUNNED] = true}
    return state
end

function modifier_extended_spiked_carapace_stun:GetEffectName()
    return "particles/generic_gameplay/generic_stunned.vpcf"
end

function modifier_extended_spiked_carapace_stun:GetEffectAttachType()
...
extended_nyx_assassin_vendetta = class({})
LinkLuaModifier("modifier_extended_vendetta", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_vendetta_charge", "hero/hero_nyx_assassin", LUA_MODIFIER_MOTION_NONE)

function extended_nyx_assassin_vendetta:IsNetherWardStealable() return false end
function extended_nyx_assassin_vendetta:IsHiddenWhenStolen()
    return false
end

function extended_nyx_assassin_vendetta:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    ...
    local modifier_vendetta = "modifier_extended_vendetta"
    local modifier_burrowed = "modifier_nyx_assassin_burrow"
...
-- Vendetta modifier
modifier_extended_vendetta = class({})

function modifier_extended_vendetta:OnCreated()
    -- Ability properties    
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    ...
    self.carapace_ability = "extended_nyx_assassin_spiked_carapace"
    self.modifier_charge = "modifier_extended_vendetta_charge"
    ...
    self.movement_speed_pct = self.movement_speed_pct + self.caster:FindTalentValue("special_bonus_extended_nyx_assassin_6")
...
function modifier_extended_vendetta:IsHidden() return false end
function modifier_extended_vendetta:IsPurgable() return false end
function modifier_extended_vendetta:IsDebuff() return false end

function modifier_extended_vendetta:CheckState()
    local state = {[MODIFIER_STATE_INVISIBLE] = true,
                   [MODIFIER_STATE_NO_UNIT_COLLISION]= true}
    return state
end

function modifier_extended_vendetta:DeclareFunctions()
...
function modifier_extended_vendetta:GetModifierInvisibilityLevel()
    return 1
end

function modifier_extended_vendetta:OnAbilityExecuted(keys)
...
function modifier_extended_vendetta:GetModifierMoveSpeedBonus_Percentage()
    return self.movement_speed_pct
end

function modifier_extended_vendetta:OnAttackLanded(keys)
    if IsServer() then
        local attacker = keys.attacker
...
modifier_extended_vendetta_charge = class({})

function modifier_extended_vendetta_charge:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()            
        ...
        self.maximum_vendetta_stacks = self.maximum_vendetta_stacks + self.caster:FindTalentValue("special_bonus_extended_nyx_assassin_8")
    end
end

function modifier_extended_vendetta_charge:GetTexture()
    return "nyx_assassin_vendetta"
end

function modifier_extended_vendetta_charge:IsHidden() return false end
function modifier_extended_vendetta_charge:IsPurgable() return false end
function modifier_extended_vendetta_charge:IsDebuff() return false end

function modifier_extended_vendetta_charge:OnStackCountChanged()
    if IsServer() then        
        -- Limit stack count                
