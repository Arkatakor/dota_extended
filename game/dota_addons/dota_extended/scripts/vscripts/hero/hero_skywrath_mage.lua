...
--      ARCANE BOLT       --
----------------------------
extended_skywrath_mage_arcane_bolt = class({})
LinkLuaModifier("modifier_extended_arcane_bolt_buff", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_skywrath_flying_movement", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)

function extended_skywrath_mage_arcane_bolt:IsHiddenWhenStolen()
    return false
end

function extended_skywrath_mage_arcane_bolt:GetIntrinsicModifierName()
    return "modifier_extended_skywrath_flying_movement"
end

function extended_skywrath_mage_arcane_bolt:OnUnStolen()
    local caster = self:GetCaster()

    -- Remove modifier to lessen load. Not that it would ever do something.
    caster:RemoveModifierByName("modifier_extended_skywrath_flying_movement")
end

function extended_skywrath_mage_arcane_bolt:GetManaCost(level)
    local caster = self:GetCaster()
    local mana_cost = self.BaseClass.GetManaCost(self,level)

    -- #1 Talent: Arcane Bolt mana cost decrease
    mana_cost = mana_cost - caster:FindTalentValue("special_bonus_extended_skywrath_mage_1")

    return mana_cost
end

function extended_skywrath_mage_arcane_bolt:GetCooldown(level)
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #5 Talent: Arcane Bolt cooldown decrease
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_skywrath_mage_5")

    return cooldown
end

function extended_skywrath_mage_arcane_bolt:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function LaunchArcaneBolt(caster, ability, target)
    ...
    local modifier_wrath = "modifier_extended_arcane_bolt_buff"
...
function extended_skywrath_mage_arcane_bolt:OnProjectileHit(target, location)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
...
modifier_extended_arcane_bolt_buff = class({})

function modifier_extended_arcane_bolt_buff:IsHidden() return false end
function modifier_extended_arcane_bolt_buff:IsPurgable() return false end
function modifier_extended_arcane_bolt_buff:IsDebuff() return false end


----------------------------
--    CONCUSSIVE SHOT     --
----------------------------
extended_skywrath_mage_concussive_shot = class({})
LinkLuaModifier("modifier_extended_concussive_shot_slow", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)

function extended_skywrath_mage_concussive_shot:IsHiddenWhenStolen()
    return false
end

function extended_skywrath_mage_concussive_shot:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self    
    ...
    if caster:HasTalent("special_bonus_extended_skywrath_mage_8") then
    ...
    if scepter and not caster:HasTalent("special_bonus_extended_skywrath_mage_8") then        
        local enemy_heroes = FindUnitsInRadius(caster:GetTeamNumber(),
                                               target:GetAbsOrigin(),
                                               nil,
                                               scepter_search_radius,
...
function extended_skywrath_mage_concussive_shot:OnProjectileHit_ExtraData(target, location, extra_data)
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local sound_impact = "Hero_SkywrathMage.ConcussiveShot.Target"    
    local modifier_slow = "modifier_extended_concussive_shot_slow"
...
modifier_extended_concussive_shot_slow = class({})

function modifier_extended_concussive_shot_slow:OnCreated()
    -- Ability properties
...
function modifier_extended_concussive_shot_slow:IsHidden() return false end
function modifier_extended_concussive_shot_slow:IsPurgable() return true end    
function modifier_extended_concussive_shot_slow:IsDebuff() return true end

function modifier_extended_concussive_shot_slow:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}

    return decFuncs
end

function modifier_extended_concussive_shot_slow:GetModifierMoveSpeedBonus_Percentage()
    return self.ms_slow_pct * (-1)
end

function modifier_extended_concussive_shot_slow:GetEffectName()
    return "particles/units/heroes/hero_skywrath_mage/skywrath_mage_concussive_shot_slow_debuff.vpcf"
end

function modifier_extended_concussive_shot_slow:GetEffectAttachType()
...
--      ANCIENT SEAL      --
----------------------------
extended_skywrath_mage_ancient_seal = class({})
LinkLuaModifier("modifier_extended_ancient_seal_main", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_ancient_seal_aura", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_ancient_seal_secondary", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)

function extended_skywrath_mage_ancient_seal:IsHiddenWhenStolen()
    return false
end

function extended_skywrath_mage_ancient_seal:GetCooldown(level)
    local caster = self:GetCaster()
    local cooldown = self.BaseClass.GetCooldown(self, level)

    -- #3 Talent: Ancient Seal cooldown decrease 
    cooldown = cooldown - caster:FindTalentValue("special_bonus_extended_skywrath_mage_3")

    return cooldown
end

function extended_skywrath_mage_ancient_seal:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
    local target = self:GetCursorTarget()
...
function ApplyAncientSeal(caster, ability, target)
    -- Ability properties
    local modifier_main_seal = "modifier_extended_ancient_seal_main"
    local modifier_thinker_aura = "modifier_extended_ancient_seal_aura"

    -- Ability specials
    local seal_duration = ability:GetSpecialValueFor("seal_duration")

    -- #2 Talent: Ancient Seal duration increase
    seal_duration = seal_duration + caster:FindTalentValue("special_bonus_extended_skywrath_mage_2")
...
-- Main seal modifier
modifier_extended_ancient_seal_main = class({})

function modifier_extended_ancient_seal_main:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
...
function modifier_extended_ancient_seal_main:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}

    return decFuncs
end

function modifier_extended_ancient_seal_main:GetModifierMagicalResistanceBonus()
    return self.mr_reduction_pct * (-1)
end

function modifier_extended_ancient_seal_main:IsHidden() return false end
function modifier_extended_ancient_seal_main:IsPurgable() return true end
function modifier_extended_ancient_seal_main:IsDebuff() return true end

function modifier_extended_ancient_seal_main:CheckState()
    local state = {[MODIFIER_STATE_SILENCED] = true}
    return state
end

-- Aura modifier
modifier_extended_ancient_seal_aura = class({})

function modifier_extended_ancient_seal_aura:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
    self.modifier_main_seal = "modifier_extended_ancient_seal_main"
...
function modifier_extended_ancient_seal_aura:GetAuraDuration()
    return self.aura_linger
end

function modifier_extended_ancient_seal_aura:GetAuraEntityReject(target)
...
function modifier_extended_ancient_seal_aura:GetAuraRadius()
    return self.sigil_radius
end

function modifier_extended_ancient_seal_aura:GetAuraSearchFlags()
    return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_extended_ancient_seal_aura:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_extended_ancient_seal_aura:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_ancient_seal_aura:GetModifierAura()
    return "modifier_extended_ancient_seal_secondary"
end

function modifier_extended_ancient_seal_aura:IsAura()
    return true
end

-- Secondary seal modifier
modifier_extended_ancient_seal_secondary = class({})

function modifier_extended_ancient_seal_secondary:OnCreated()
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_extended_ancient_seal_secondary:GetEffectName()
    return "particles/units/heroes/hero_skywrath_mage/skywrath_mage_ancient_seal_debuff.vpcf"
end

function modifier_extended_ancient_seal_secondary:GetEffectAttachType()
    return PATTACH_OVERHEAD_FOLLOW
end

function modifier_extended_ancient_seal_secondary:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}

    return decFuncs
end

function modifier_extended_ancient_seal_secondary:GetModifierMagicalResistanceBonus()
    return self.mr_reduction_pct * (-1)
end

function modifier_extended_ancient_seal_secondary:IsHidden() return false end
function modifier_extended_ancient_seal_secondary:IsPurgable() return true end
function modifier_extended_ancient_seal_secondary:IsDebuff() return true end

function modifier_extended_ancient_seal_secondary:CheckState()
    local state = {[MODIFIER_STATE_SILENCED] = true}
    return state
end
...
--      MYSTIC FLARE      --
----------------------------
extended_skywrath_mage_mystic_flare = class({})
LinkLuaModifier("modifier_extended_mystic_flare", "hero/hero_skywrath_mage.lua", LUA_MODIFIER_MOTION_NONE)

function extended_skywrath_mage_mystic_flare:IsHiddenWhenStolen()
    return false
end

function extended_skywrath_mage_mystic_flare:GetAOERadius()
    local caster = self:GetCaster()
    local ability = self
    local damage_radius = ability:GetSpecialValueFor("damage_radius")

    -- #4 Talent: Mystic Flare double radius
    damage_radius = damage_radius + caster:FindTalentValue("special_bonus_extended_skywrath_mage_4")

    return damage_radius
end

function extended_skywrath_mage_mystic_flare:OnSpellStart()
    -- Ability properties
    local caster = self:GetCaster()
    local ability = self
...
function ExecuteMysticFlare(caster, ability, target_point)
    -- Ability properties
    local modifier_mystic = "modifier_extended_mystic_flare"
    ...
    -- #6 Talent: Mystic Flare deals the damage in half the duration
        damage_duration = damage_duration * (1 - caster:FindTalentValue("special_bonus_extended_skywrath_mage_6") * 0.01)        
...
modifier_extended_mystic_flare = class({})

function modifier_extended_mystic_flare:OnCreated()
    if IsServer() then
        -- Ability properties
        self.caster = self:GetCaster()
        self.ability = self:GetAbility()
        ...
        self.modifier_wrath = "modifier_extended_arcane_bolt_buff"
        ...
        -- #4 Talent: Mystic Flare double radius
        self.damage_radius = self.damage_radius + self.caster:FindTalentValue("special_bonus_extended_skywrath_mage_4")

        -- #6 Talent: Mystic Flare deals the damage in half the duration
        self.damage_duration = self.damage_duration * (1 - self.caster:FindTalentValue("special_bonus_extended_skywrath_mage_6") * 0.01)
        self.damage_interval = self.damage_interval * (1 - self.caster:FindTalentValue("special_bonus_extended_skywrath_mage_6") * 0.01)
...
function modifier_extended_mystic_flare:OnIntervalThink()
    if IsServer() then
        -- Play target sound
...
--------------------------------
modifier_extended_skywrath_flying_movement = class({})

function modifier_extended_skywrath_flying_movement:CheckState()
    local caster = self:GetCaster()
    local state

    if caster:HasTalent("special_bonus_extended_skywrath_mage_7") then
...
function modifier_extended_skywrath_flying_movement:IsHidden() return true end
function modifier_extended_skywrath_flying_movement:IsPurgable() return false end
function modifier_extended_skywrath_flying_movement:IsDebuff() return false end
