...
-- Enfeeble Debuff
modifier_extended_enfeeble_debuff_stats = modifier_extended_enfeeble_debuff_stats or class({
...
-- brain_sap Debuff
modifier_extended_brain_sap_mana = modifier_extended_brain_sap_mana or class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
-- Nightmare Debuff
modifier_extended_nightmare_dot = modifier_extended_nightmare_dot or class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
modifier_extended_nightmare_invul = modifier_extended_nightmare_invul or class({
		IsDebuff                         = function(self) return true                                                                                                           end,    
		IsHidden                         = function(self) return true                                                                                                           end,
	})
modifier_extended_nightmare_vision = modifier_extended_nightmare_vision or class({
		GetBonusVisionPercentage         = function(self) return self.visionreduction                                                                                           end,    
	})
-- Fiends Grip Debuff
modifier_extended_fiends_grip_handler = modifier_extended_fiends_grip_handler or class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
LinkLuaModifier("modifier_extended_enfeeble_debuff_stats",          "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_brain_sap_mana",                 "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_nightmare_dot",                 	"hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_nightmare_invul",   				"hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_nightmare_vision", 				"hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_fiends_grip_handler",            "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
...
function extended_bane_enfeeble:OnSpellStart() 
	...
	local enfeeble_duration = getkvValues(self,"enfeeble_duration") + caster:FindTalentValue("special_bonus_extended_bane_3")
	target:AddNewModifier(caster, self, "modifier_extended_enfeeble_debuff_stats", {duration = enfeeble_duration})
...
function extended_bane_brain_sap:OnSpellStart()
	...
	local talentdurationbonus = caster:FindTalentValue("special_bonus_extended_bane_1")
	local talentdamagebonus = caster:FindTalentValue("special_bonus_extended_bane_4")	
	...
	target:AddNewModifier(caster, self, "modifier_extended_brain_sap_mana", {duration = (sapduration+talentdurationbonus)}) -- debuff enemy
...
function extended_bane_brain_sap:GetCooldown(level)
...
function extended_bane_nightmare:OnSpellStart()
	...
	local talentinvulnbonus = self:GetCaster():FindTalentValue("special_bonus_extended_bane_7")
	...
	target:AddNewModifier(caster, self, "modifier_extended_nightmare_dot",               {duration = nightmareduration}) -- im schleep
	target:AddNewModifier(caster, self, "modifier_extended_nightmare_invul", {duration = invulnduration})
...
function extended_bane_nightmare:CastFilterResultTarget(target)
	if IsServer() then
...
function extended_bane_nightmare_end:OnSpellStart()
	...
	for _,unit in pairs(units) do	
		if unit:HasModifier("modifier_extended_nightmare_invul") then -- We can't debuff an immune target! so remove this first
			unit:RemoveModifierByName("modifier_extended_nightmare_invul")
		end    

		if unit:HasModifier("modifier_extended_nightmare_dot") then  --do we have it? remove it
			unit:RemoveModifierByName("modifier_extended_nightmare_dot")
		end
	end
end
...
function extended_bane_fiends_grip:GetChannelTime()
	local talentchannelbonus = self:GetCaster():FindTalentValue("special_bonus_extended_bane_6")  -- net tables
...
function extended_bane_fiends_grip:GetCooldown(nLevel)
	local talentcooldownbonus = self:GetCaster():FindTalentValue("special_bonus_extended_bane_8")
...
function extended_bane_fiends_grip:OnSpellStart()  
	...
	local talentchannelbonus = self:GetCaster():FindTalentValue("special_bonus_extended_bane_6")
	...
	self.fiendtarget:AddNewModifier(self.fiendcaster, self, "modifier_extended_fiends_grip_handler", {duration = fiends_grip_duration+talentchannelbonus,propogated = 0, original_target = 1}) -- we are the original target, it has not spread
...
function extended_bane_fiends_grip:OnChannelFinish(bInterrupted) 
	...
	for k,v in pairs(self.fiendgriptable) do
		...
		if ((self.fiendgriptable[k]:HasModifier("modifier_extended_fiends_grip_handler")) and self.fiendgriptable[k]:FindModifierByName("modifier_extended_fiends_grip_handler").propogated == 0) then
			self.fiendgriptable[k]:FindModifierByName("modifier_extended_fiends_grip_handler"):Destroy() -- if our target has been gripped, and it has not been spread by itself, destroy it (which will reinvoke and set propogated to 1)
		end
	end
end
...
function extended_bane_fiends_grip:OnChannelThink()
	if IsServer() then
		local caster = self:GetCaster()
		if caster:HasTalent("special_bonus_extended_bane_8") then
			...
			local fiends_grip_duration = self:GetTalentSpecialValueFor("fiends_grip_duration") + caster:FindTalentValue("special_bonus_extended_bane_6")
			...
			for _, enemy in pairs(nearby_enemies) do
				...
				if not enemy:HasModifier("modifier_extended_fiends_grip_handler") then
					...
					if view_angle <= ( vision_cone / 2 ) and enemy:CanEntityBeSeenByMyTeam(caster) then
						enemy:AddNewModifier(caster, self, "modifier_extended_fiends_grip_handler", {duration = fiends_grip_duration-(GameRules:GetGameTime()-self:GetChannelStartTime()), propogated = 0})
...
function modifier_extended_enfeeble_debuff_stats:DeclareFunctions()
	local funcs = {
...
function modifier_extended_enfeeble_debuff_stats:OnCreated()
	local statreduction,damagereduction = getkvValues(self:GetAbility(),"stat_reduction","damage_reduction") 
	local talentdamagereduction = self:GetCaster():FindTalentValue("special_bonus_extended_bane_5")
...
function modifier_extended_brain_sap_mana:DeclareFunctions()
	local funcs = {
...
function modifier_extended_nightmare_dot:DeclareFunctions()
	local funcs = {
...
function modifier_extended_nightmare_dot:OnCreated()
...
function modifier_extended_nightmare_dot:CheckState()
	return{
...
function modifier_extended_nightmare_dot:OnTakeDamage(t)
	if not IsServer() then return end
...
function modifier_extended_nightmare_dot:OnAttackStart(t)
	if not IsServer() then return end
	if t.target == self:GetParent() then -- am i the target of an attack?
		...
		t.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_nightmare_dot", {duration = nightmare_duration}) -- Spread Nightmare
		t.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_nightmare_invul", {duration = nightmare_invuln_duration}) -- Apply invuln, idk if firetoad wanted allies to gain the talent so i'm leaving original functionality
...
function modifier_extended_nightmare_dot:OnIntervalThink()
	if not IsServer() then return end
...
function modifier_extended_nightmare_dot:OnDestroy()
	if not IsServer() then return end
	if self:GetCaster():GetTeamNumber() ~= self:GetParent():GetTeamNumber() then -- when nightmare is destroyed, are we an enemy?
		if not self:GetParent():IsMagicImmune() then  -- are we magic immune if we're an enemy?
			...
			local talentdurationbonus = self:GetCaster():FindTalentValue("special_bonus_extended_bane_2")
			self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_nightmare_vision", {duration = nightmare_baleful_visions_duration+talentdurationbonus}) -- blind them
		end
	end
	if self:GetParent():HasModifier("modifier_extended_nightmare_invul") then -- If we don't do this, clever players will only skill one point in nightmare and take the 6sec invuln talent
		self:GetParent():RemoveModifierByName("modifier_extended_nightmare_invul")
	end      
	EmitSoundOn("Hero_Bane.Nightmare.End", self:GetParent())
end

-- Nightmare Invulnerability
function modifier_extended_nightmare_invul:DeclareFunctions()
	local funcs = {
...
function modifier_extended_nightmare_invul:CheckState()
	return{[MODIFIER_STATE_INVULNERABLE] = true, [MODIFIER_STATE_NO_HEALTH_BAR] = true}
end

-- Nightmare Baleful Visions

function modifier_extended_nightmare_vision:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_BONUS_VISION_PERCENTAGE,
	}
	return funcs
...
function modifier_extended_nightmare_vision:OnCreated()
	self:StartIntervalThink(0)
end

function modifier_extended_nightmare_vision:OnIntervalThink()
...
function modifier_extended_fiends_grip_handler:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_OVERRIDE_ANIMATION,
	}
	return funcs  
...
function modifier_extended_fiends_grip_handler:OnCreated(var)
	if not IsServer() then return end 
...
function modifier_extended_fiends_grip_handler:OnRefresh(var)
	if not IsServer() then return end   
	self:GetParent():Interrupt()    
	self:OnIntervalThink()  
	self:StartIntervalThink(1)
...
function modifier_extended_fiends_grip_handler:OnIntervalThink()
	if not IsServer() then return end 
...
function modifier_extended_fiends_grip_handler:CheckState()
	local state = {
		[MODIFIER_STATE_STUNNED] = true,
	}
	return state
...
function modifier_extended_fiends_grip_handler:OnDestroy()  
	if not IsServer() then return end   
	...
	if not self:GetParent():IsMagicImmune() and self.propogated == 0 then -- Are we magic immune somehow(omniknight)? if not, have we already retriggered once?
		...
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_fiends_grip_handler", {duration = fiends_grip_linger_duration, propogated = 1})   
	end
end
