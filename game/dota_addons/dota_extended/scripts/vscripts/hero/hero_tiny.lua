...
function extended_tiny_rolling_stone:IsInnateAbility()
	return true
end
...
function modifier_extended_tiny_rolling_stone:OnIntervalThink()
	...
	if not self.talent8 then
		if self:GetParent():HasTalent("special_bonus_extended_tiny_8") then
			self.talent8 = true
			self:GetParent():AddNewModifier(self:GetParent(), self:GetParent():FindAbilityByName("special_bonus_extended_tiny_8"), "modifier_special_bonus_extended_tiny_8", {})
		end
	end
	if not self.talent2 then
		if self:GetParent():HasTalent("special_bonus_extended_tiny_2") then
			self.talent2 = true
			self:SetStackCount(self:GetStackCount() + self:GetParent():FindTalentValue("special_bonus_extended_tiny_2") )
		end
	end
end
...
function modifier_extended_tiny_rolling_stone:GetModifierAttackSpeedBonus_Constant()
	if not self:GetParent():HasModifier("modifier_special_bonus_extended_tiny_8") then
...
if IsServer() then
	...
	function extended_tiny_avalanche:OnProjectileHit_ExtraData(hTarget, vLocation, extradata)
		...
		if self:GetCaster():HasAbility("special_bonus_extended_tiny_3") then
			offset = self:GetCaster():FindTalentValue("special_bonus_extended_tiny_3")
...
function extended_tiny_toss:OnSpellStart()
	...
	for _, victim in pairs(tossVictims) do
		if victim ~= caster then
			...
			if not self:GetCaster():HasTalent("special_bonus_extended_tiny_7") then
				break
			end
		end
	end
...
function extended_tiny_toss:GetAOERadius()
    local ability = self
...
function modifier_tiny_toss_movement:OnRemoved()
	if IsServer() then
		...
		if caster:HasModifier("modifier_extended_tiny_rolling_stone") and caster:HasAbility("extended_tiny_grow") then
		 	radius = radius + caster:FindModifierByName("modifier_extended_tiny_rolling_stone"):GetStackCount() * caster:FindAbilityByName("extended_tiny_grow"):GetSpecialValueFor("rolling_stones_aoe")
		end
...
function modifier_tiny_toss_scepter_bounce:OnRemoved()
	if IsServer() then
		...
		local radius = self:GetAbility():GetSpecialValueFor("radius") + self.caster:FindModifierByName("modifier_extended_tiny_rolling_stone"):GetStackCount() * self.caster:FindAbilityByName("extended_tiny_grow"):GetSpecialValueFor("rolling_stones_aoe")
	...
	if IsServer() then
		if params.target == self:GetParent() then
			...
			if RollPseudoRandom(self.chance, self) then
				if self:GetParent():HasTalent("special_bonus_extended_tiny_4") then
					EmitSoundOn("Hero_Tiny.CraggyExterior", self:GetCaster())
					local radius = self:GetParent():FindTalentValue("special_bonus_extended_tiny_4")
...
function modifier_craggy_exterior_blunt:GetModifierPreAttack_BonusDamage()
	local reduction = self.reduction + self.caster:FindTalentValue("special_bonus_extended_tiny_5")	
