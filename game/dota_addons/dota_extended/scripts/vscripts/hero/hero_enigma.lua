...
extended_enigma_malefice           = extended_enigma_malefice           or class({
		IsHiddenWhenStolen         = function(self) return true                                       end,    
	})  
extended_enigma_demonic_conversion = extended_enigma_demonic_conversion or class({
		IsHiddenWhenStolen         = function(self) return true                                       end,   
	})  
extended_enigma_midnight_pulse     = extended_enigma_midnight_pulse     or class({
		IsHiddenWhenStolen         = function(self) return true                                       end,   
	})  
extended_enigma_black_hole         = extended_enigma_black_hole         or class({
		IsHiddenWhenStolen         = function(self) return true                                       end,   
	}) 
extended_enigma_gravity_well       = extended_enigma_gravity_well       or class({
		IsHiddenWhenStolen         = function(self) return true                                       end,   
		IsStealable                = function(self) return false                                      end,   
		GetIntrinsicModifierName   = function(self) return 'modifier_extended_enigma_gravity_well_stacks' end,   
		IsInnateAbility 		   = function(self) return true 									  end,
	}) 

LinkLuaModifier("modifier_extended_enigma_malefice",           "hero/hero_enigma", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_enigma_malefice_stun",      "hero/hero_enigma", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_extended_enigma_black_hole",         "hero/hero_enigma", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_enigma_gravity_well",       "hero/hero_enigma", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_enigma_gravity_well_stacks","hero/hero_enigma", LUA_MODIFIER_MOTION_NONE)
...
modifier_extended_enigma_malefice = class({  -- ticking debuff for malefice duration
		DeclareFunctions                 = function(self) return modifier_extended_enigma_malefice.funcs end,       
...
modifier_extended_enigma_malefice_stun = class({ -- actual 'stun' of malefice
		DeclareFunctions                 = function(self) return modifier_extended_enigma_malefice_stun.funcs          end, 
		IsDebuff                         = function(self) return true                                              end,    
...
modifier_extended_enigma_black_hole = class({ -- ultra hard disable, forces pull on gravity well, enigma gets perma buff per hero caught
		DeclareFunctions                 = function(self) return modifier_extended_enigma_black_hole.funcs             end,
		CheckState                                           = function(self) return self.states                   end,
		IsDebuff                         = function(self) return true                                              end,    
		IsHidden                         = function(self) return false                                             end,
		},{
...
modifier_extended_enigma_gravity_well        = class({})
modifier_extended_enigma_gravity_well_stacks = class({})
...
function extended_enigma_black_hole:GetCastRange(l,t)
	local caster = self:GetCaster()
	local cast_range = self.BaseClass.GetCastRange(self, l, t)

	-- #4 Talent: Black Hole Cast Range
	cast_range = cast_range + caster:FindTalentValue("special_bonus_extended_enigma_4")

	return cast_range
end

function extended_enigma_malefice:GetAOERadius()
	local caster = self:GetCaster()
	
	if caster:HasTalent("special_bonus_extended_enigma_2") then
		return caster:FindTalentValue("special_bonus_extended_enigma_2")
	else
		return nil
	end
end

function extended_enigma_midnight_pulse:GetAOERadius()
	local caster = self:GetCaster()
	local ability = self
	...
	radius = radius + caster:FindTalentValue("special_bonus_extended_enigma_6")
	return radius	
end

function extended_enigma_black_hole:GetAOERadius()
	local caster = self:GetCaster()
	local ability = self
...
function extended_enigma_black_hole:GetChannelTime()
	local caster = self:GetCaster()
	local channel_time = self.BaseClass.GetChannelTime(self)

	-- #7 Talent: Black Hole Duration
	channel_time = channel_time + caster:FindTalentValue("special_bonus_extended_enigma_7")

	return channel_time
end

function extended_enigma_demonic_conversion:CastFilterResultTarget(target)
	if IsServer() then
		local caster = self:GetCaster()	
		
		-- #8 Talent: Cast Eidolons on heroes
		if caster:HasTalent("special_bonus_extended_enigma_8") and target:IsRealHero() then
			return UF_SUCCESS
		end
...
function extended_enigma_demonic_conversion:IsNetherWardStealable() return false end
...
if IsServer() then	
	...
	function extended_enigma_add_gravity_source(source,vector,strength,duration)                        -- Creates a gravity source indexed by strength, returns the index
		...
		if duration then
			...
			Timers:CreateTimer(duration,function()   
					extended_enigma_remove_gravity_source(source,strength,currentindex)        
					return nil
				end)  
		end
	...
	function extended_enigma_remove_gravity_source(source,strength,index)                                   -- Remove a requested gravity source by index and strength
		if not source.gravitysources then
			source.gravitysources = {}
		end
	...
	function extended_enigma_change_gravity_source(source,vector,strength,index)                        -- Edit an existing index to not mess up our table order
		if not source.gravitysources then
			source.gravitysources = {}
		end
	...
	function extended_enigma_gravity_strength_fetcher(source)                                                           -- Precautionary performance measures
		...
		if source.resetgravity then
			...
			local singularity_stacks = (source:FindModifierByName('modifier_extended_enigma_gravity_well_stacks'):GetStackCount())
			local stack_bonus_pull = ((singularity_stacks*source:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('stack_bonus_pull')))
			table.insert(_G["gravitytable"..teamnumber],source:FindModifierByName('modifier_extended_enigma_gravity_well_stacks'):GetStackCount())
			table.insert(_G["gravitytable"..teamnumber],singularity_stacks*source:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('stack_bonus_pull'))
			table.insert(_G["gravitytable"..teamnumber],singularity_stacks*source:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('stack_bonus_radius'))   
			table.insert(_G["gravitytable"..teamnumber],source:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('radius'))			
			table.insert(_G["gravitytable"..teamnumber],source:FindAbilityByName('extended_enigma_midnight_pulse'):GetSpecialValueFor('pull_strength'))
			table.insert(_G["gravitytable"..teamnumber],source:FindAbilityByName('extended_enigma_midnight_pulse'):GetSpecialValueFor('well_pull_strength'))			
			table.insert(_G["gravitytable"..teamnumber],source:FindAbilityByName('extended_enigma_black_hole'):GetSpecialValueFor('pull_strength'))
			table.insert(_G["gravitytable"..teamnumber],(source:FindAbilityByName('extended_enigma_black_hole'):GetSpecialValueFor('well_pull_strength') + stack_bonus_pull) )
		end
		source.resetgravity = nil
		return unpack(_G["gravitytable"..source:GetTeamNumber()])
	end 

	function extended_enigma_find_strongest_source(source,target,base_extra_pull,strength)  -- Find our strongest source in 900 radius
		...
		black_hole_well_strength = extended_enigma_gravity_strength_fetcher(source) -- we don't actually need a bunch of these but i'm lazy and will fix it later, the extra table keys won't hurt too much
		...
		local talent_percent_bonus = (source:FindTalentValue("special_bonus_extended_enigma_5") * 0.01) + mass_stack_bonus_percent		
	...
	function extended_enigma_gravity_pull(target,strongest_source,pullstrength,radius)          -- Perform actual gravity pull
	...
	function extended_enigma_gravity_pull_area(source,gtarget,radius,strength)                          -- iterate through a group near a ground target
		...
		for _,target in pairs(nearby_enemies) do
			...
			local strongest_source,pullstrength = extended_enigma_find_strongest_source(source,target,0,strength)       
			if strongest_source then
				extended_enigma_gravity_pull(target,strongest_source,pullstrength,radius)
				ResolveNPCPositions(strongest_source,120)
			end
		end
	end

	function extended_enigma_gravity_pull_single_enemy(source,target,bonuspull)                         -- For targets, searches for a nearby gravity source
		if not source.gravitysources or source:PassivesDisabled() then return end
		local strongest_source,pullstrength = extended_enigma_find_strongest_source(source,target,bonuspull)
	...
	function extended_enigma_malefice:OnSpellStart()
		...
		malefice_duration = malefice_duration + caster:FindTalentValue("special_bonus_extended_enigma_1")
		
		-- #2 Talent: Malefice Effect Radius
		if caster:HasTalent("special_bonus_extended_enigma_2") then
			local radius = caster:FindTalentValue("special_bonus_extended_enigma_2")
			...
			for _,enemy in pairs(enemies) do
				enemy:AddNewModifier(caster, self, "modifier_extended_enigma_malefice", {duration = malefice_duration})             
			end
		else
			if target:TriggerSpellAbsorb(self) then return end
			target:AddNewModifier(caster, self, "modifier_extended_enigma_malefice", {duration = malefice_duration})  
		end

	end

	function extended_enigma_demonic_conversion:OnSpellStart()
		local target,caster = findtarget(self)
		caster.resetgravity = true  
		...
		extended_enigma_create_eidolons(target,caster,self,count,duration)

	end

	function extended_enigma_create_eidolons(target,caster,abilitysource,number_of_eidolons,duration)
		local base_health,shard_percentage = getkvValues(abilitysource,"base_health","shard_percentage")		
		local eidolon_name = "npc_extended_enigma_eidolon_"..math.min(abilitysource:GetLevel(),4)
		...
		shard_percentage = (shard_percentage + caster:FindTalentValue("special_bonus_extended_enigma_3")) * 0.01		
	...
	function extended_enigma_midnight_pulse:OnSpellStart() 
		...
		extended_enigma_add_gravity_source(self:GetCaster(),gtarget,2,duration+0.2)
		...
		extended_enigma_gravity_pull_area(caster,gtarget,radius,2)
	end 

	function extended_enigma_black_hole:OnSpellStart()
		local gtarget,caster = findgroundtarget(self)
		if caster:HasScepter() and caster:FindAbilityByName('extended_enigma_midnight_pulse') then
			caster:FindAbilityByName('extended_enigma_midnight_pulse'):OnSpellStart()
		end
		...
		if caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks') then
			local singularity_stacks = caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks'):GetStackCount()
			local well_radius_bonus  = caster:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('stack_bonus_radius')        
		end

		-- #6 Talent: Black Hole radius
		radius = radius + self.caster:FindTalentValue("special_bonus_extended_enigma_6")

		self.radius = radius
		self.pull_distance = (base_pull_distance + stack_pull_distance * singularity_stacks)
		self.channel_time = self:GetChannelTime()
		GridNav:DestroyTreesAroundPoint(gtarget,radius,false)
		self.gravityindex = extended_enigma_add_gravity_source(self:GetCaster(),gtarget,1)
	...
	function extended_enigma_black_hole:OnChannelThink()		
		extended_enigma_gravity_pull_area(self:GetCaster(),self.gtarget,self.baseradius,1)
		...
		for _,enemy in pairs(self.nearby_enemies) do
			...
			if distance > self.radius then
				...
				if not enemy:FindModifierByName("modifier_extended_enigma_black_hole") then
					enemy:AddNewModifier(self.caster, self, "modifier_extended_enigma_black_hole", {duration = (self.channel_time-(GameRules:GetGameTime()-self:GetChannelStartTime()))})
					if self.caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks') and enemy:IsHero() and not enemy:IsIllusion() then
						self.caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks'):IncrementStackCount()
					end
	...
	function extended_enigma_black_hole:OnChannelFinish(bInterrupted)
		extended_enigma_remove_gravity_source(self:GetCaster(),1,self.gravityindex)
		for k,v in pairs(self.black_hole_targets) do
			if self.black_hole_targets[k]:HasModifier("modifier_extended_enigma_black_hole") then
				self.black_hole_targets[k]:FindModifierByName("modifier_extended_enigma_black_hole"):Destroy() -- gracefully end flail on interrupt
			end
		end
	...
	function extended_enigma_gravity_well:OnSpellStart()
		...
		if not self:GetCursorTarget() then -- if we are targetting nothing, then we are probably targetting the ground
			...
			extended_enigma_add_gravity_source(self:GetCaster(),gtarget,3,gravity_well_duration) -- 3 is the weakest source
			...
			local target,caster = findtarget(self)
			if target:TriggerSpellAbsorb(self) then return end 
			self.gravityindex = extended_enigma_add_gravity_source(self:GetCaster(),target:GetAbsOrigin(),3,gravity_well_duration) 
			target:AddNewModifier(self:GetCaster(), self, "modifier_extended_enigma_gravity_well", {duration = gravity_well_duration})
		end
	end

	function modifier_extended_enigma_malefice:OnCreated()
		...
		self.modifier_parent:AddNewModifier(self.caster,self.ability,"modifier_extended_enigma_malefice_stun",{duration=self.stunduration})
		local singularity_stacks,well_pull_bonus
		if self.caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks') then 
			singularity_stacks = self.caster:FindModifierByName('modifier_extended_enigma_gravity_well_stacks'):GetStackCount() 
			well_pull_bonus = self.caster:FindAbilityByName('extended_enigma_gravity_well'):GetSpecialValueFor('stack_bonus_pull')  
		else
	...
	function modifier_extended_enigma_malefice:CheckState()
		return extended_enigma_gravity_pull_single_enemy(self:GetCaster(),self:GetParent(),self.well_pull_strength)
	end

	function modifier_extended_enigma_malefice:OnIntervalThink()
		self.modifier_parent:AddNewModifier(self.caster,self.ability,"modifier_extended_enigma_malefice_stun",{duration=self.stunduration})
	end

	function modifier_extended_enigma_malefice_stun:OnCreated()
		self.stunduration,self.damage,self.well_pull_strength = getkvValues(self:GetAbility(),"stun_duration","tick_damage","well_pull_strength")
		self.caster          = self:GetCaster()
		self.ability         = self:GetAbility()
		self.modifier_parent = self:GetParent()
	...
	function modifier_extended_enigma_malefice_stun:OnDestroy()
		...
		if IsServer() then
			if vars.attacker == self:GetParent() then
				...
				if self.attacks >= self.attacks_to_split then 
					...
					extended_enigma_create_eidolons(self:GetParent(),self:GetCaster(),self:GetAbility(),1,duration)
				end
	...
	function modifier_extended_enigma_black_hole:OnCreated()
		self.parent = self:GetParent()
	...
	function modifier_extended_enigma_black_hole:OnDestroy()
		self.parent:FadeGesture(ACT_DOTA_FLAIL)
	end

	function modifier_extended_enigma_gravity_well:OnCreated()
		...
		self.gravityindex = extended_enigma_add_gravity_source(self:GetCaster(),self:GetParent():GetAbsOrigin(),3,duration)
	...
	function modifier_extended_enigma_gravity_well:OnIntervalThink()
		extended_enigma_change_gravity_source(self:GetCaster(),self:GetParent():GetAbsOrigin(),3,self.gravityindex)
	end

	function modifier_extended_enigma_gravity_well:OnDestroy()
		local target = self:GetParent():GetAbsOrigin()
		local caster = self:GetCaster()  
		StopSoundOn("Enigma.GravityWellCast",self:GetParent())
		extended_enigma_remove_gravity_source(caster,3,self.gravityindex)
		...
		if remainingduration > 1 then
			self.gravityindex = extended_enigma_add_gravity_source(self:GetCaster(),target,3,remainingduration)
			...
			ParticleManager:SetParticleControl( self.gravity_wellGFX, 1, Vector(250, 0, 0))
			extended_enigma_add_gravity_source(caster,gtarget,3) -- 3 is the weakest source
	...
	function midnight_pulse_pull_instance:OnIntervalThink()
		extended_enigma_gravity_pull_area(self.caster,self.gtarget,self.radius,2)
	end
