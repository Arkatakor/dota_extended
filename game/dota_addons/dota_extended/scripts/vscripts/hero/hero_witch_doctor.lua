...
function extended_witch_doctor_paralyzing_cask:IsHiddenWhenStolen() return false end
function extended_witch_doctor_paralyzing_cask:IsRefreshable() return true end
function extended_witch_doctor_paralyzing_cask:IsStealable() return true end
function extended_witch_doctor_paralyzing_cask:IsNetherWardStealable() return true end
-------------------------------------------
...
function extended_witch_doctor_paralyzing_cask:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if hTarget then
		if hTarget:IsRealHero() then
			if hTarget:GetTeamNumber() ~= hCaster:GetTeamNumber() then
				...
				local heal = ExtraData.hero_damage + hCaster:FindTalentValue("special_bonus_extended_witch_doctor_1")
			...
			if hTarget:GetTeamNumber() ~= hCaster:GetTeamNumber() then
				...
				local heal = ExtraData.creep_damage + hCaster:FindTalentValue("special_bonus_extended_witch_doctor_1")
				hTarget:Heal(heal, hCaster)
				SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, hTarget, heal, nil)
			end
		end
...
LinkLuaModifier("modifier_extended_voodoo_restoration", "hero/hero_witch_doctor", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_voodoo_restoration_heal", "hero/hero_witch_doctor", LUA_MODIFIER_MOTION_NONE)

function extended_witch_doctor_voodoo_restoration:GetCastRange()
	return self:GetSpecialValueFor("radius")
end

function extended_witch_doctor_voodoo_restoration:GetManaCost( hTarget )
	if self:GetCaster():HasTalent("special_bonus_extended_witch_doctor_6") then
		return 0
	else
...
function extended_witch_doctor_voodoo_restoration:OnToggle()
	...
	if self:GetToggleState() then
		...
		hCaster:AddNewModifier(hCaster, self, "modifier_extended_voodoo_restoration", {})
		...
		hCaster:RemoveModifierByName("modifier_extended_voodoo_restoration")
	end
end

modifier_extended_voodoo_restoration = class({})
function modifier_extended_voodoo_restoration:OnCreated()
	if IsServer() then
...
function modifier_extended_voodoo_restoration:OnDestroy()
	if IsServer() then
...
function modifier_extended_voodoo_restoration:OnIntervalThink()
	local hCaster = self:GetCaster()
	...
	if self.cleanse_counter >= self.cleanse_interval then
		...
		for _,hAlly in pairs(allies) do
			...
			if hAlly == hCaster then
				EmitSoundOn("Extended.WitchDoctorDispel", hCaster)
			end
		end
	end
	-- Drain mana if no talent
	if not hCaster:HasTalent("special_bonus_extended_witch_doctor_6") then
...
function modifier_extended_voodoo_restoration:IsAura()
	return true
end

function modifier_extended_voodoo_restoration:IsAuraActiveOnDeath()
	return false
end

function modifier_extended_voodoo_restoration:GetAuraRadius()
	return self.radius
end

function modifier_extended_voodoo_restoration:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_extended_voodoo_restoration:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_extended_voodoo_restoration:GetModifierAura()
	return "modifier_extended_voodoo_restoration_heal"
end

function modifier_extended_voodoo_restoration:IsHidden()
	return true
end
-------------------------------------------
modifier_extended_voodoo_restoration_heal = class({})
function modifier_extended_voodoo_restoration_heal:IsDebuff() return false end
function modifier_extended_voodoo_restoration_heal:IsHidden() return false end
function modifier_extended_voodoo_restoration_heal:IsPurgable() return false end
function modifier_extended_voodoo_restoration_heal:IsPurgeException() return false end
function modifier_extended_voodoo_restoration_heal:IsStunDebuff() return false end
function modifier_extended_voodoo_restoration_heal:RemoveOnDeath() return true end
function modifier_extended_voodoo_restoration_heal:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
-------------------------------------------
function modifier_extended_voodoo_restoration_heal:OnCreated()
	if IsServer() then
		local hAbility = self:GetAbility()
...
function modifier_extended_voodoo_restoration_heal:OnIntervalThink()
	local hCaster = self:GetCaster()
...
--				MALEDICT
-------------------------------------------
LinkLuaModifier("modifier_extended_maledict", "hero/hero_witch_doctor", LUA_MODIFIER_MOTION_NONE)

extended_witch_doctor_maledict = class({})
function extended_witch_doctor_maledict:IsHiddenWhenStolen() return false end
function extended_witch_doctor_maledict:IsRefreshable() return true end
function extended_witch_doctor_maledict:IsStealable() return true end
function extended_witch_doctor_maledict:IsNetherWardStealable() return true end
-------------------------------------------
...
function extended_witch_doctor_maledict:OnSpellStart()
	...
	if #enemies > 0 then
		...
		for _, enemy in pairs(enemies) do
			enemy:AddNewModifier(hCaster, self, "modifier_extended_maledict", {duration = duration+FrameTime()})
		end
	else
...
-------------------------------------------
modifier_extended_maledict = class({})
function modifier_extended_maledict:IsDebuff() return true end
function modifier_extended_maledict:IsHidden() return false end
function modifier_extended_maledict:IsPurgable() return false end
function modifier_extended_maledict:IsPurgeException() return false end
function modifier_extended_maledict:IsStunDebuff() return false end
function modifier_extended_maledict:RemoveOnDeath() return true end
-------------------------------------------
function modifier_extended_maledict:OnCreated()
	local hAbility = self:GetAbility()
	local hParent = self:GetParent()
...
function modifier_extended_maledict:OnRefresh()
	local hAbility = self:GetAbility()
	self.bonus_damage_pct = hAbility:GetSpecialValueFor("bonus_damage_pct") * 0.01
	self.tick_time_sec = hAbility:GetSpecialValueFor("tick_time_sec")
	self.tick_time_main = hAbility:GetSpecialValueFor("tick_time_main")
...
function modifier_extended_maledict:OnDestroy()
	if IsServer() then
		self:StartIntervalThink(-1)
	end
...
function modifier_extended_maledict:OnIntervalThink()
	local hParent = self:GetParent()
...
function modifier_extended_maledict:GetStatusEffectName()
	return "particles/status_fx/status_effect_maledict.vpcf"
end

function modifier_extended_maledict:StatusEffectPriority()
	return 5
end

function modifier_extended_maledict:DeclareFunctions()
	local funcs =
	{
...
function modifier_extended_maledict:GetModifierHealAmplify_Percentage()
	return self.heal_reduce_pct
end

function modifier_extended_maledict:OnDeath(params)
...
function modifier_extended_maledict:GetHealthRegenAmp()
	return self.heal_reduce_pct
end

function modifier_extended_maledict:DealHPBurstDamage(hTarget)
...
--				DEATH WARD
-------------------------------------------
LinkLuaModifier("modifier_extended_death_ward", "hero/hero_witch_doctor", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_death_ward_caster", "hero/hero_witch_doctor", LUA_MODIFIER_MOTION_NONE)

extended_witch_doctor_death_ward = class({})
function extended_witch_doctor_death_ward:IsHiddenWhenStolen() return false end
function extended_witch_doctor_death_ward:IsRefreshable() return true end
function extended_witch_doctor_death_ward:IsStealable() return true end
function extended_witch_doctor_death_ward:IsNetherWardStealable() return false end
-------------------------------------------
...
function extended_witch_doctor_death_ward:OnSpellStart()
	if IsServer() then
		local hCaster = self:GetCaster()
		local vPosition = self:GetCursorPosition()
		if hCaster:HasTalent("special_bonus_extended_witch_doctor_8") then
			-- Calculate where the wards shall spawn
			local distance = hCaster:FindTalentValue("special_bonus_extended_witch_doctor_8")
		...
		if (math.random(1,100) <= 15) then
			self.dance = true
			self.death_ward:EmitSound("Extended.WitchDoctorSingsASong")
		else
...
function extended_witch_doctor_death_ward:CreateWard(vPosition)
	local hCaster = self:GetCaster()
	local damage = self:GetSpecialValueFor("damage") + hCaster:GetIntellect()*self:GetSpecialValueFor("int_to_dmg_pct")/100
	local death_ward = CreateUnitByName("extended_witch_doctor_death_ward", vPosition, true, hCaster, nil, hCaster:GetTeam())
	...
	local death_ward_mod = death_ward:AddNewModifier(hCaster, self, "modifier_extended_death_ward", {duration = self:GetChannelTime()}) 
	local exceptionList = 
	{
	...
	-- For Attack-order
	self.mod_caster = hCaster:AddNewModifier(hCaster, self, "modifier_extended_death_ward_caster", {duration = self:GetChannelTime()})
...
function extended_witch_doctor_death_ward:GetChannelAnimation()
...
function extended_witch_doctor_death_ward:OnChannelFinish()
	if IsServer() then
		...
		StopSoundEvent("Extended.WitchDoctorSingsASong", self.death_ward)
...
-------------------------------------------
modifier_extended_death_ward_caster = class({})
function modifier_extended_death_ward_caster:IsDebuff() return false end
function modifier_extended_death_ward_caster:IsHidden() return true end
function modifier_extended_death_ward_caster:IsPurgable() return false end
function modifier_extended_death_ward_caster:IsPurgeException() return false end
function modifier_extended_death_ward_caster:IsStunDebuff() return false end
function modifier_extended_death_ward_caster:RemoveOnDeath() return true end
-------------------------------------------
modifier_extended_death_ward = class({})
function modifier_extended_death_ward:IsDebuff() return false end
function modifier_extended_death_ward:IsHidden() return true end
function modifier_extended_death_ward:IsPurgable() return false end
function modifier_extended_death_ward:IsPurgeException() return false end
function modifier_extended_death_ward:IsStunDebuff() return false end
function modifier_extended_death_ward:RemoveOnDeath() return true end
-------------------------------------------

function modifier_extended_death_ward:OnCreated()
...
function modifier_extended_death_ward:OnDestroy()
	if IsServer() then
		self:GetAbility()[self:GetParent().index] = nil
	end
end

function modifier_extended_death_ward:OnIntervalThink()
	if IsServer() then
		local hCaster = self:GetCaster()
		local hParent = self:GetParent()
		local hAbility = self:GetAbility()
		...
		local range = hCaster:GetAttackRange() + hCaster:FindTalentValue("special_bonus_extended_witch_doctor_4")
...
function modifier_extended_death_ward:CheckState()
	local state = {
