...
local function PopulateGenericExtendedTalentTableValues()
    -- Allow client access to EXTENDED_GENERIC_TALENT_LIST (javascript access)
    CustomNetTables:SetTableValue("extended_talent_manager", "extended_generic_talent_info", EXTENDED_GENERIC_TALENT_LIST)
end
...
local function PopulateHeroTalentList(hero)
    ...
    for i=0,7 do
        ...
        local inner_value = {}
        if i%2 == 0 then
            local areExtendedTalents = false
            local currentAbilityIndex = (endAbilityIndex - i)
            -- Special talents
            -- Extended talents only allow 2 selection. Non Extended talents allow 8 selection
            local ability1 = hero:GetAbilityByIndex(currentAbilityIndex-1)
            if ability1 then
                local abilityName = ability1:GetAbilityName()
                if abilityName:find("special_bonus_extended_") == 1 then
                    areExtendedTalents = true
                end

                if abilityName:find("special_bonus_extended_") == 1 or abilityName:find("special_bonus_unique_") == 1 then
                    -- Only add to selection if it is a extended or unique talent
            ...
            if not areExtendedTalents then
                -- Copy table
                for k,v in pairs(EXTENDED_HERO_TALENTS_LIST[heroName]) do 
                    inner_value[k] = v
                end
            end
        else
            -- Stat talents (Pick either of the 6)
            inner_value = EXTENDED_HERO_TALENTS_LIST[heroName]
        end

        hero_talent_list[current_level] = inner_value
    end

    CustomNetTables:SetTableValue( "extended_talent_manager", "hero_talent_list_"..hero:entindex(), hero_talent_list )
end
...
local function PopulateHeroTalentChoice(hero)
    ...
    CustomNetTables:SetTableValue( "extended_talent_manager", "hero_talent_choice_"..hero:entindex(), talent_choices )
end
...
local function PopulateHeroTalentLinkedAbilities(hero)
    local existing_table = CustomNetTables:GetTableValue( "extended_talent_manager", "talent_linked_abilities" )
    ...
    CustomNetTables:SetTableValue( "extended_talent_manager", "talent_linked_abilities", existing_table )
end
...
local function HasNotPopulatedValues(hero_id)
    return (CustomNetTables:GetTableValue( "extended_talent_manager", "hero_talent_list_"..hero_id) == nil)
end

function PopulateHeroExtendedTalents(hero)
...
function HandlePlayerUpgradeExtendedTalent(unused, kv)
    ...
    if hero and IsValidEntity(hero) then
        ...
        if ownerPlayerID == thisPlayerID or (PlayerResource:GetUnitShareMaskForPlayer(ownerPlayerID, thisPlayerID) % 2 == 1) then
            local hero_talent_list = CustomNetTables:GetTableValue( "extended_talent_manager", "hero_talent_list_"..heroID )
            local hero_talent_choices = CustomNetTables:GetTableValue( "extended_talent_manager", "hero_talent_choice_"..heroID )
            ...
            if hero:GetLevel() >= level and
                ...
                if talent_name ~= nil then
                    ...
                    if string.find(talent_name, "extended_generic_talent") == 1 then
                    ...
                    --Update table choice
                    CustomNetTables:SetTableValue( "extended_talent_manager", "hero_talent_choice_"..heroID, hero_talent_choices )
                else
...
function InitPlayerHeroExtendedTalents()
    -- Populate net table "extended_talent_manager"
    PopulateGenericExtendedTalentTableValues()

    -- Register for event when user select a talent to upgrade
    CustomGameEventManager:RegisterListener("upgrade_extended_talent", HandlePlayerUpgradeExtendedTalent)
end