...
function ChangeAttackProjectileExtended(unit)
	...
	if unit:HasModifier("modifier_item_extended_desolator") or unit:HasModifier("modifier_item_extended_desolator_2") then
		if unit:HasModifier("modifier_item_extended_skadi") then
	...
	elseif unit:HasModifier("modifier_item_extended_skadi") then
		unit:SetRangedProjectileName(particle_skadi)

	-- If the unit has any form of lifesteal, use the lifesteal projectile
	elseif unit:HasModifier("modifier_extended_morbid_mask") or unit:HasModifier("modifier_extended_mask_of_madness") or unit:HasModifier("modifier_extended_satanic") or unit:HasModifier("modifier_item_extended_vladmir") or unit:HasModifier("modifier_item_extended_vladmir_blood") then		
...
function CreateEmptyTalents(hero)
	for i=1,8 do
		...
		local class = "modifier_special_bonus_extended_"..hero.."_"..i.." = class({IsHidden = function(self) return true end, RemoveOnDeath = function(self) return false end, AllowIllusionDuplicate = function(self) return true end})"  
		load(class)()
	end
end
...
function CDOTA_BaseNPC:DropRapier(hItem, sNewItemName)
	...
	if sName == "item_extended_rapier" then
		hRapier:GetContainer():SetRenderColor(230,240,35)
	elseif sName == "item_extended_rapier_2" then
	...
	elseif sName == "item_extended_rapier_magic" then
		hRapier:GetContainer():SetRenderColor(35,35,240)
	elseif sName == "item_extended_rapier_magic_2" then
		...
		ParticleManager:SetParticleControl(hRapier.rapier_pfx, 0, vLocation + vRandomVector)
	elseif sName == "item_extended_rapier_cursed" then
...
function CDOTA_BaseNPC:AddRangeIndicator(hCaster, hAbility, sAttribute, iRange, iRed, iGreen, iBlue, bShowOnCooldown, bShowAlways, bWithCastRangeIncrease, bRemoveOnDeath)
	local modifier = self:AddNewModifier(hCaster or self,hAbility, "modifier_extended_range_indicator", {
